...
import { loadAuxiliaryRows, loadIngestedSheetRows, readDashboardSheetRows, shouldUseIngestForDashboard } from "./dataProvider.js";
import { getIngestMetaForOfficePeriod, listIngestPeriodsForOffice } from "./leadsStore.js";
import {
...
import { currentMonthKey, getMonthFile, listMonthFiles, monthFilterFromKey, parseMonthKey } from "./monthlyReports.js";
...
// Google returns this when a spreadsheet ID is invalid, deleted, or otherwise
// missing (e.g. an office/month mapped to a placeholder or removed sheet).
function isGoogleEntityNotFoundError(error) {
  const message = String(error?.message || error || "").toLowerCase();
  return (
    message.includes("requested entity was not found") ||
    message.includes("unable to parse range") ||
    (message.includes("not found") && message.includes("entity")) ||
    error?.code === 404 ||
    error?.status === 404
  );
}
...
function collectOfficeScopedMonthRecords(officeMap, officeScope = "") {
  const resolvedOffice = resolveOfficeNameForMonthMap(officeScope, officeMap);
  if (!resolvedOffice) {
    return [];
  }
  if (Array.isArray(officeMap?.byOffice?.[resolvedOffice])) {
    return officeMap.byOffice[resolvedOffice];
  }
  const normalizedScope = normalizeText(resolvedOffice);
...
function monthRecordFromIngestPeriod(period = "", officeName = "", ingestMeta = null) {
  ...
  return {
    ...
    sheet_id: String(ingestMeta?.spreadsheetId || "").trim(),
    active: true,
    ingest: true,
...
function mergeOfficeMonthRecords(officeMap, officeScope = "", ingestPeriods = []) {
  const resolvedOffice = resolveOfficeNameForMonthMap(officeScope, officeMap);
  const byKey = new Map();
  for (const record of collectOfficeScopedMonthRecords(officeMap, officeScope)) {
    ...
    byKey.set(key, {
      ...record,
      office_name: String(record?.office_name || resolvedOffice || "").trim(),
    });
  }
  for (const period of ingestPeriods || []) {
    if (byKey.has(period)) {
      const current = byKey.get(period);
      if (!String(current?.sheet_id || "").trim()) {
        byKey.set(period, { ...current, ingest: true });
      }
      continue;
    }
    const synthetic = monthRecordFromIngestPeriod(period, resolvedOffice);
    if (synthetic) {
      byKey.set(period, synthetic);
    }
  }
  return [...byKey.values()].sort((left, right) => String(right.key || "").localeCompare(String(left.key || "")));
}

function isLoadableMonthRecord(record = {}, ingestPeriods = new Set()) {
  if (record?.legacy) {
    return false;
  }
  if (String(record?.sheet_id || "").trim()) {
    return true;
  }
  const key = String(record?.key || "").trim();
  return Boolean(record?.ingest || ingestPeriods.has(key));
}

async function buildIngestPeriodsByOffice(officeMap, officeScopes = []) {
  const ingestPeriodsByOffice = new Map();
  if (!(await shouldUseIngestForDashboard())) {
    return ingestPeriodsByOffice;
  }
  for (const office of officeScopes) {
    const resolvedOffice = resolveOfficeNameForMonthMap(office, officeMap);
    if (!resolvedOffice || ingestPeriodsByOffice.has(resolvedOffice)) {
      continue;
    }
    const periods = await listIngestPeriodsForOffice(resolvedOffice, "leads");
    ingestPeriodsByOffice.set(resolvedOffice, new Set(periods));
  }
  return ingestPeriodsByOffice;
}

async function enrichOfficeMonthRecords(officeMap, officeScope = "", ingestPeriods = new Set()) {
  const resolvedOffice = resolveOfficeNameForMonthMap(officeScope, officeMap);
  const records = mergeOfficeMonthRecords(officeMap, officeScope, [...ingestPeriods]);
  if (!(await shouldUseIngestForDashboard()) || !ingestPeriods.size) {
    return records;
  }
  const enriched = [];
  for (const record of records) {
    const key = String(record?.key || "").trim();
    if (!key || String(record?.sheet_id || "").trim() || !ingestPeriods.has(key)) {
      enriched.push(record);
      continue;
    }
    const ingestMeta = await getIngestMetaForOfficePeriod(resolvedOffice, key, "leads");
    enriched.push({
      ...record,
      sheet_id: String(ingestMeta?.spreadsheetId || record.sheet_id || "").trim(),
      ingest: true,
    });
  }
  return enriched.sort((left, right) => String(right.key || "").localeCompare(String(left.key || "")));
}
...
async function readLeadsRowsForBenchmarkBaseline({
  ...
  try {
    ...
    if (isGoogleEntityNotFoundError(error)) {
      const ingestedFallback = await loadIngestedSheetRows({
        tabKey: "leads",
        office: officeScope || monthRecord.office_name || "",
        period: monthRecord.key || monthRecord.period || "",
      }).catch(() => null);
      rawRows = Array.isArray(ingestedFallback) ? ingestedFallback : [];
    } else {
      throw error;
    }
  }
...
// Counts distinct non-empty values for a field, including the synthetic
// zero-activity "working" agent rows injected by buildWorkingAgentZeroRows.
// Those rows carry no lead `id`, so the default uniqueValues() (which requires
// an id) skips them even though they ARE rendered in the results table. Without
// this, the "Total Agent" / desk / team leader summary cards under-report
// versus the rows the user actually sees (e.g. card says 9 while the table
// lists 13 agents).
export function uniqueValuesIncludingSyntheticAgents(rows, tabConfig, fieldKey) {
  const fieldName = getFieldName(tabConfig, fieldKey);
  const idField = getFieldName(tabConfig, "id");
  const values = new Set();
  for (const row of rows) {
    if (!row?.__syntheticWorkingAgent && !isPresent(getRowValue(row, idField))) {
      continue;
    }
...
function buildDashboardStats(rows, tabConfig, infoContext, dateFilter, now = new Date()) {
  ...
  return {
    totalAgent: uniqueValuesIncludingSyntheticAgents(rows, tabConfig, "agentNames"),
    teamLeaderTotal: uniqueValuesIncludingSyntheticAgents(rows, tabConfig, "teamLeader"),
    deskTotal: uniqueValuesIncludingSyntheticAgents(rows, tabConfig, "office"),
...
async function readMonthData({
  ...
  const legacyInfoRowsPromise = includeLegacyInfoTargets
    ...
    : Promise.resolve([]);
  const kycFtdCustomerIdSetPromise = includeKycFtd
    ? (() => {
        notifyProgress("FTD");
        return readDashboardSheetRows("ftd", {
  ...
  let rawRows = [];
  // Tracks whether the month's leads source was actually reachable. A valid but
  // empty sheet is "available" (0 leads is legitimate); a missing/invalid sheet
  // with no ingest fallback is "unavailable" and should be surfaced clearly.
  let leadsSourceUnavailable = false;
  const hasSheetId = Boolean(String(monthScope.spreadsheetId || "").trim());
  ...
  try {
    ...
    // No mapped sheet and the ingest lookup returned nothing → no source at all.
    if (!hasSheetId && (!Array.isArray(rawRows) || rawRows.length === 0)) {
      leadsSourceUnavailable = true;
    }
  } catch (error) {
    if (isGooglePermissionDeniedError(error)) {
      const serviceAccountEmail = getGoogleCredentialConfig().email;
    ...
    // The mapped spreadsheet is missing/invalid (e.g. Offices tab has a stale or
    // placeholder ID). Prefer ingested Redis rows for this office/month; if none
    // exist, render an empty month instead of failing the whole report.
    if (isGoogleEntityNotFoundError(error)) {
      const ingestedFallback = await loadIngestedSheetRows({
        tabKey: "leads",
        office: monthScope.office,
        period: monthScope.period,
      }).catch(() => null);
      if (Array.isArray(ingestedFallback)) {
        rawRows = ingestedFallback;
      } else {
        rawRows = [];
        leadsSourceUnavailable = true;
      }
  ...
  // Targets for the roster merge must come from the FULL Info Agents tab, not
  // the lead-scoped subset. Otherwise a working agent with a target but zero
  // leads (e.g. Hamza Has, target 10) is dropped from the lead-scoped info and
  // his target falls back to 0 — even though he still appears as a zero-activity
  // row. The roster is already permission-scoped, so looking up targets from the
  // full info tab only assigns them to agents the user is allowed to see.
  const legacyInfoTargetsByAgent = buildInfoAgentsContext(legacyInfoRows).targetsByAgent;
  const mergedInfoRows = mergedInfoRowsFromRoster(rosterRows, legacyInfoTargetsByAgent);
...
function dimensionValueForRow(row, tabConfig, dimension, options = {}) {
  ...
  let value = cleanSpreadsheetText(getRowValue(row, fieldName) || "");
  if (!value && dimension.key === "desk") {
    value = cleanSpreadsheetText(row?.__scopeOfficeName || "");
  }
    ...
    const tableRows = groupedEntries
      .map((entry) => {
        ...
        for (const columnValue of columnValuesWithGrandTotal) {
          ...
          if (legacyOverrides && columnDimension === "month" && columnValue !== COLUMN_GRAND_TOTAL_KEY) {
...
// Filter keys that scope to lead activity. When any of these are applied, we do
// not inject zero-activity working agents (there is no activity to match).
const ACTIVITY_FILTER_KEYS = ["country", "brand", "campaign", "subCampaign", "placement", "status", "date", "hour"];

// Builds synthetic zero-activity lead rows for agents who are marked "working"
// (Argentina uses "active", which normalizes to working) and have a target, but
// produced no leads/FTD in the selected month(s). Without these rows such agents
// disappear from the dashboard even though they should be listed with zeros.
export function buildWorkingAgentZeroRows({
  selectedMonthData = [],
  existingRows = [],
  tabConfig,
  query = {},
  ...
  // Respect activity-based filters: zero-activity agents cannot match them.
  const hasActivityFilter = ACTIVITY_FILTER_KEYS.some((key) => parseCsvSelection(query[key]).length > 0);
  if (hasActivityFilter) {
    return [];
  }
  const deskFilter = parseCsvSelection(query.desk).map((value) => normalizeText(value)).filter(Boolean);
  const teamLeaderFilter = parseCsvSelection(query.teamLeader).map((value) => normalizeText(value)).filter(Boolean);
  const agentFilter = parseCsvSelection(query.agent).map((value) => normalizeAgentName(value)).filter(Boolean);
  const deskFilterSet = new Set(deskFilter);
  const teamLeaderFilterSet = new Set(teamLeaderFilter);
  const agentFilterSet = new Set(agentFilter);
  ...
  const scopeOffice = selectedOfficeScopes[0] || "";
  const rows = [];
  ...
  for (const item of selectedMonthData) {
    ...
    const monthLabel = String(item?.monthRecord?.month_label || monthKey).trim();
    const officeName = String(item?.monthRecord?.office_name || scopeOffice || "").trim();
    const records = Array.isArray(item?.infoContext?.records) ? item.infoContext.records : [];
    if (!records.length) {
      continue;
    }
    // Agents already present (with lead rows) in this month must not be duplicated.
    ...
    for (const row of existingRows) {
      ...
      const normalizedAgent = normalizeAgentName(getRowValue(row, agentField));
      if (normalizedAgent) {
        presentAgents.add(normalizedAgent);
      }
    }
    ...
    for (const record of records) {
      ...
      const normalizedAgent = String(record?.normalized_name || "").trim();
      if (!normalizedAgent || presentAgents.has(normalizedAgent)) {
        continue;
      }
      if (isExcludedNormalizedAgent(normalizedAgent)) {
        continue;
      }
      if (agentFilterSet.size && !agentFilterSet.has(normalizedAgent)) {
        continue;
      }
      if (teamLeaderFilterSet.size && !teamLeaderFilterSet.has(normalizeText(record?.team_leader || ""))) {
        continue;
      }
      const deskValue = String(record?.office || "").trim();
      if (deskFilterSet.size && !deskFilterSet.has(normalizeText(deskValue || officeName))) {
        continue;
      }
      rows.push({
        [agentField]: record?.agent_name || normalizedAgent,
        [officeField]: deskValue || officeName,
        [teamLeaderField]: record?.team_leader || "",
        Leads: 0,
        [crTargetField]: "",
...
async function buildLegacyAugmentation({
  ...
  for (const office of offices) {
    ...
    if (isLast4 && !benchmarkMode) {
      const combined = resolveLast4MonthKeysForOffice(office, liveMonthKeys || []);
      monthKeysToInclude = legacyMonthKeysInWindow(combined);
    }
...
export async function loadDashboardReport(accessContext, query = {}, options = {}) {
  ...
  const ingestPeriodsByOffice = await buildIngestPeriodsByOffice(officeMap, selectedOfficeScopes);
  const primaryResolvedOffice = resolveOfficeNameForMonthMap(primaryOfficeScope, officeMap);
  const primaryIngestPeriods = ingestPeriodsByOffice.get(primaryResolvedOffice) || new Set();
  const rawMonthRecords = mergedMonthRecords(officeMap);
  const scopedMonthRecords = (
    await Promise.all(
      selectedOfficeScopes.map(async (office) => {
        const resolvedOffice = resolveOfficeNameForMonthMap(office, officeMap);
        const ingestPeriods = ingestPeriodsByOffice.get(resolvedOffice) || new Set();
        const liveRecords = (await enrichOfficeMonthRecords(officeMap, office, ingestPeriods)).filter(
          (record) => record.active !== false,
        );
        return officeMonthRecordsWithLegacy(office, liveRecords).map((record) => ({
          ...record,
          office_name: office,
        }));
      }),
  ...
  const isLast4Quick = ["1", "true", "yes", "on"].includes(normalizeText(query.last4QuickMode));
  const primaryOfficeMonthRecords = (await enrichOfficeMonthRecords(officeMap, primaryOfficeScope, primaryIngestPeriods)).filter(
    (record) => record.active !== false,
  );
  const officeLiveMonthKeys = primaryOfficeMonthRecords
    .filter((record) => isLoadableMonthRecord(record, primaryIngestPeriods))
  ...
  let selectedMonthKeys = uniqueSelection((requestedMonthKeys.length ? requestedMonthKeys : [fallbackMonthKey]).filter(Boolean));
  if (isLast4Quick) {
    selectedMonthKeys = resolveLast4MonthKeysForOffice(primaryOfficeScope, officeLiveMonthKeys);
  }

  const firstLoadableMonthKey =
    selectedMonthKeys.find((monthKey) => {
      const scopedMatch = primaryOfficeMonthRecords.find((record) => String(record.key || "") === String(monthKey || ""));
      return scopedMatch && isLoadableMonthRecord(scopedMatch, primaryIngestPeriods);
    }) ||
    selectedMonthKeys[0] ||
    fallbackMonthKey;
  let monthRecord =
    primaryOfficeMonthRecords.find((record) => String(record.key || "") === String(firstLoadableMonthKey || "")) ||
    resolveMonthRecord(firstLoadableMonthKey, primaryOfficeScope, officeMap, now);
  if (!monthRecord || !isLoadableMonthRecord(monthRecord, primaryIngestPeriods)) {
    throw new Error("No active month mapping found.");
  }
  if (!String(monthRecord.sheet_id || "").trim() && primaryIngestPeriods.has(String(monthRecord.key || "").trim())) {
    const ingestMeta = await getIngestMetaForOfficePeriod(primaryResolvedOffice, monthRecord.key, "leads");
    monthRecord = {
      ...monthRecord,
      sheet_id: String(ingestMeta?.spreadsheetId || "").trim(),
      ingest: true,
  ...
  if (reportMode === "last4") {
    ...
    const primaryScopedMonthRecords = primaryOfficeMonthRecords
      .filter((record) => isLoadableMonthRecord(record, primaryIngestPeriods))
      .sort((left, right) => String(right.key || "").localeCompare(String(left.key || "")));
  ...
  const selectedMonthRecords = (
    await Promise.all(
      selectedOfficeScopes.map(async (office) => {
        const resolvedOffice = resolveOfficeNameForMonthMap(office, officeMap);
        const ingestPeriods = ingestPeriodsByOffice.get(resolvedOffice) || new Set();
        const officeRecords = (await enrichOfficeMonthRecords(officeMap, office, ingestPeriods))
          .filter((record) => record.active !== false && isLoadableMonthRecord(record, ingestPeriods))
          .sort((left, right) => String(right.key || "").localeCompare(String(left.key || "")));
        const effectiveMonthKeys = isLast4Quick
          ? resolveLast4MonthKeysForOffice(
              office,
              officeRecords.map((record) => String(record.key || "").trim()).filter(Boolean),
            ).filter((monthKey) => officeRecords.some((record) => String(record.key || "") === monthKey))
          : selectedMonthKeys.length
            ? selectedMonthKeys
            : [officeRecords[0]?.key].filter(Boolean);
        return Promise.all(
          effectiveMonthKeys.map(async (monthKey) => {
            const matched = officeRecords.find((record) => String(record.key || "") === String(monthKey || ""));
            if (!matched) {
              return null;
            }
            if (!String(matched.sheet_id || "").trim() && ingestPeriods.has(String(monthKey || "").trim())) {
              const ingestMeta = await getIngestMetaForOfficePeriod(resolvedOffice, monthKey, "leads");
              return {
                ...matched,
                sheet_id: String(ingestMeta?.spreadsheetId || "").trim(),
                ingest: true,
  ...
  if (legacyReportEligible(reportMode, query, benchmarkMode)) {
    ...
    if (legacyAugmentation.rows.length) {
      modeRowsCombined = modeRowsCombined.concat(legacyAugmentation.rows);
      legacyOverrides = legacyAugmentation.overrides;
    }
  }

  // Working agents (Argentina: "active") with a target but zero leads/FTD must
  // still appear in the dashboard. Inject synthetic zero rows after filtering so
  // the not-working filter never drops them and activity filters are respected.
  // Only for months whose leads source actually loaded — an unavailable source
  // (missing/unsynced sheet) must not be masked by a full roster of zero rows.
  const availableMonthData = selectedMonthData.filter((item) => !item?.leadsSourceUnavailable);
  if (reportMode === "specific" && selectedSpecificDimensions(query).includes("agent")) {
    const workingAgentZeroRows = buildWorkingAgentZeroRows({
      selectedMonthData: availableMonthData,
      existingRows: modeRowsCombined,
      tabConfig,
      query,
      selectedOfficeScopes,
    });
    if (workingAgentZeroRows.length) {
      modeRowsCombined = modeRowsCombined.concat(workingAgentZeroRows);
    }
  }

  // Surface a clear notice when one or more selected months have no reachable
  // data source (e.g. the Offices tab maps a month to a missing spreadsheet and
  // nothing is synced to Redis), so an all-zero table is not mistaken for a bug.
  const unavailableMonthLabels = selectedMonthData
    .filter((item) => item?.leadsSourceUnavailable)
    .map((item) => String(item?.monthRecord?.month_label || item?.monthRecord?.key || "").trim())
    .filter(Boolean);
  let dataNotice = "";
  if (unavailableMonthLabels.length) {
    const officeLabel = selectedOfficeScopes.join(", ") || primaryOfficeScope;
    const uniqueLabels = [...new Set(unavailableMonthLabels)];
    dataNotice =
      `No data source is available for ${officeLabel} — ${uniqueLabels.join(", ")}. ` +
      `Map this month to a valid spreadsheet in Bot Authority → Offices (shared with the service account), ` +
      `or sync it into Redis, then reload.`;
  }
