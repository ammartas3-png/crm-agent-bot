...
export async function loadDashboardReport(accessContext, query = {}, options = {}) {
  ...
  const diagnosticsEnabled = ["1", "true", "yes", "on"].includes(normalizeText(query.debugDiagnostics));
  const stageTimings = [];
  ...
  const useFastOptions = reportMode === "specific" && (selectedMonthRecords.length > 2 || combinedRows.length > 5000);
