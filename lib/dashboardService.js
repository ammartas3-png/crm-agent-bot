...
// A row represents a lead (Lead Date) and, when it converted, an FTD (FTD DATE).
// The FTD can happen on a different day/hour than the lead was registered. To make
// "today's FTDs from older leads" land on the FTD day (and not on the lead's older
// day), the date/hour dimensions bucket the row by two separate "events":
//   - lead event:  Lead Date day / Created hour
//   - ftd event:   FTD DATE day / FTD DATE hour (only when the row has an FTD)
// Returns true when the row has a meaningful FTD signal.
function rowHasFtdSignalForBucket(row, tabConfig) {
  if (ftdContributionFromRow(row, tabConfig) > 0) {
    return true;
  }
  return Boolean(String(getRowValue(row, getFieldName(tabConfig, "ftdMaker")) || "").trim());
}

// Mirrors calculations.dateValueForField("ftdDate"): prefer the explicit FTD DATE,
// otherwise fall back to Created / Lead Date so an FTD without an explicit date is
// still attributed to a day rather than being dropped.
function rowFtdDateRawValue(row, tabConfig) {
  const explicit = getRowValue(row, getFieldName(tabConfig, "ftdDate"));
  if (String(explicit || "").trim() && parseDateValue(explicit)) {
    return explicit;
  }
...
function dimensionValueForEvent(row, tabConfig, dimension, eventKind, options = {}) {
  if (dimension.key === "date") {
    if (eventKind === "ftd") {
      return formatDateStamp(rowFtdDateRawValue(row, tabConfig));
    }
    const leadDateField = getFieldName(tabConfig, "leadDate");
    const createdField = getFieldName(tabConfig, "created");
    return formatDateStamp(getRowValue(row, leadDateField) || getRowValue(row, createdField));
  ...
  if (dimension.key === "hour") {
    if (eventKind === "ftd") {
      return formatHourStamp(rowFtdDateRawValue(row, tabConfig));
    }
    const createdField = getFieldName(tabConfig, "created");
    const leadDateField = getFieldName(tabConfig, "leadDate");
    return formatHourStamp(getRowValue(row, createdField) || getRowValue(row, leadDateField));
  }
  return dimensionValueForRow(row, tabConfig, dimension, options);
}

function columnDimensionValueForEvent(row, tabConfig, columnDimension, eventKind) {
  if (eventKind === "ftd" && (columnDimension === "date" || columnDimension === "hour")) {
    const ftdDateRaw = rowFtdDateRawValue(row, tabConfig);
    return columnDimension === "date" ? formatDateStamp(ftdDateRaw) : formatHourStamp(ftdDateRaw);
  }
  return columnDimensionValueForRow(row, tabConfig, columnDimension);
}
...
export function specificBuilderTable(rows, tabConfig, infoContext, monthFilter, query = {}, now = new Date(), options = {}) {
  ...
  // Date/Hour dimensions attribute leads by Lead Date but FTDs by FTD DATE. When such a
  // dimension is present we bucket each row by both its lead event and (when applicable)
  // its FTD event, then compute every bucket's leads/FTD via a per-bucket date/hour filter
  // so cross-day FTDs (e.g. an older lead converting today) land on the FTD day.
  const rowDimensionsHaveDateHour = orderedDimensions.some(
    (dimension) => dimension.key === "date" || dimension.key === "hour",
  );
  const columnIsDateHour = columnDimension === "date" || columnDimension === "hour";
  const dateAwareGrouping = rowDimensionsHaveDateHour || columnIsDateHour;
  const dateDimensionInRows = orderedDimensions.find((dimension) => dimension.key === "date");
  const hourDimensionInRows = orderedDimensions.find((dimension) => dimension.key === "hour");
  const singleDayDateFilter = (dayValue) => {
    const raw = String(dayValue || "").trim();
    if (!/^\d{4}-\d{2}-\d{2}$/.test(raw)) {
      return null;
    }
    return { type: "range", start: raw, end: raw };
  };
  const singleHourRange = (hourValue) => {
    const match = String(hourValue || "").trim().match(/^(\d{1,2}):/);
    if (!match) {
      return null;
    }
  ...
  const bucketDateHourFilter = (values = {}, columnValue = null) => {
    let dateFilter = monthFilter || null;
    let hourRange = null;
    if (dateDimensionInRows) {
      const day = singleDayDateFilter(values[dateDimensionInRows.key]);
      if (day) {
        dateFilter = day;
      }
    }
    if (hourDimensionInRows) {
      const range = singleHourRange(values[hourDimensionInRows.key]);
      if (range) {
        hourRange = range;
      }
    }
    if (columnValue !== null && columnValue !== COLUMN_GRAND_TOTAL_KEY) {
      if (columnDimension === "date") {
        const day = singleDayDateFilter(columnValue);
        if (day) {
          dateFilter = day;
        }
      } else if (columnDimension === "hour") {
        const range = singleHourRange(columnValue);
        if (range) {
          hourRange = range;
        }
      }
    ...
    if (hourRange) {
      filter.hourRange = hourRange;
    }
    return filter;
  };
  // Summary for a date/hour bucket: leads counted by Lead Date and FTDs by FTD DATE within
  // the bucket's day/hour. FTD targets keep their month-scoped semantics (a daily column
  // should still be measured against the agent's monthly target).
  const dateAwareBucketSummary = (bucketRows, values = {}, columnValue = null) => {
    const filter = bucketDateHourFilter(values, columnValue);
    const baseSummary = calculateSummary(bucketRows, tabConfig, filter, now);
    const ftdRowsInBucket = getFtdRowsByDateRange(bucketRows, tabConfig, filter, now);
    let ftdTarget = 0;
    if (needsTargetAggregation) {
      const targetAggregation = targetAggregationForScope({
        rows: bucketRows,
        ...
        scope: {
          groupField: "agentNames",
          onlyWorkingAgents: true,
          ...targetScopeFromDimensionValues(values),
        },
        now,
      });
      ftdTarget = Number(targetAggregation?.includedTarget || 0);
    ...
    return {
      ...baseSummary,
      kycFtd: kycFtdCountFromRows(ftdRowsInBucket, tabConfig),
      ftdTarget,
      ftdTargetReach: targetReachPercent(Number(baseSummary.totalFtd || 0), ftdTarget),
    };
  };
  // Only split a row into lead+ftd events where the matching summary path applies the
  // per-bucket date/hour filter; otherwise FTDs would be double-counted across buckets.
  const eventKindsForRow = (row, active) =>
    active && rowHasFtdSignalForBucket(row, tabConfig) ? ["lead", "ftd"] : ["lead"];
  ...
  if (columnDimension) {
    ...
    for (const row of rows) {
      ...
      for (const eventKind of eventKindsForRow(row, columnIsDateHour)) {
        const values = {};
        for (const dimension of effectiveRowDimensions) {
          values[dimension.key] = dimensionValueForEvent(row, tabConfig, dimension, eventKind, {
        ...
        const columnValue = columnDimensionValueForEvent(row, tabConfig, columnDimension, eventKind);
        const groupKey = effectiveRowDimensions.length
          ...
          : "__all__";
        const bucketKey = `${groupKey}||${columnValue}`;
    ...
    const tableRows = groupedEntries
      .map((entry) => {
        ...
        const allEntryRows = [];
        const seenEntryRows = columnIsDateHour ? new Set() : null;
        for (const rowsForColumn of entry.byColumn.values()) {
          if (seenEntryRows) {
            for (const entryRow of rowsForColumn) {
              if (!seenEntryRows.has(entryRow)) {
                seenEntryRows.add(entryRow);
                allEntryRows.push(entryRow);
              }
            }
          } else {
        ...
        for (const columnValue of columnValuesWithGrandTotal) {
          ...
          if (!summary && columnIsDateHour) {
            summary = dateAwareBucketSummary(bucketRows, entry.values, columnValue);
          }
          if (!summary) {
  ...
  for (const row of rows) {
    ...
    for (const eventKind of eventKindsForRow(row, rowDimensionsHaveDateHour)) {
      const values = {};
  ...
  let detailRows = groupedEntries
    .map((entry) => {
      const summary = rowDimensionsHaveDateHour
        ? dateAwareBucketSummary(entry.rows, entry.values)
        : scopedSummary(entry.rows, entry.values);
      const metricValues = {
        ...metricValuesFromSummary(summary),
        ...(needsBuilderExtraMetrics
  ...
  if (totalDimensionKeys.length) {
    for (const totalDimensionKey of totalDimensionKeys) {
      ...
      for (const detailRow of detailRows) {
        ...
        if (!groupsAtDepth.has(prefixKey)) {
          ...
          groupsAtDepth.set(prefixKey, {
            values: prefixValues,
            rows: [],
            // When date/hour events split a row across buckets the same row may appear in
            // several detail rows; dedupe so subtotals never double-count it.
            rowSet: rowDimensionsHaveDateHour ? new Set() : null,
          });
        }
        const group = groupsAtDepth.get(prefixKey);
        if (group.rowSet) {
          for (const sourceRow of detailRow.sourceRows) {
            if (!group.rowSet.has(sourceRow)) {
              group.rowSet.add(sourceRow);
              group.rows.push(sourceRow);
            }
          }
        } else {
          group.rows.push(...detailRow.sourceRows);
        }
      }
      ...
      for (const [prefixKey, group] of groupsAtDepth.entries()) {
        ...
        const summary = rowDimensionsHaveDateHour
          ? dateAwareBucketSummary(group.rows, group.values)
          : scopedSummary(group.rows, group.values);
