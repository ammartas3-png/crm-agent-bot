...
const SPECIFIC_METRICS = [
  ...
  { key: "avgFtdByAgent", label: "Avg FTD by Agent", type: "number" },
  { key: "avgFtdByAgentDaily", label: "Avg FTD by Agent Daily", type: "number" },
...
function builderExtraMetricValues(rows = [], tabConfig, summary = {}) {
  const agentField = getFieldName(tabConfig, "agentNames");
  const leadDateField = getFieldName(tabConfig, "leadDate");
  const createdField = getFieldName(tabConfig, "created");
  ...
  const avgFtdByAgent = agentCount > 0 ? totalFtd / agentCount : 0;
  return {
    agentCount,
    avgLeadByAgent,
    avgLeadByAgentDaily: dayCount > 0 ? avgLeadByAgent / dayCount : 0,
    avgFtdByAgent,
    avgFtdByAgentDaily: dayCount > 0 ? avgFtdByAgent / dayCount : 0,
  };
}
...
function specificBuilderTable(rows, tabConfig, infoContext, monthFilter, query = {}, now = new Date()) {
  ...
  if (columnDimension) {
    ...
    const effectiveRowDimensions = rowDimensions.length
      ? rowDimensions
      : [{ key: "group", label: "Group", type: "text", kind: "dimension", synthetic: true }];
    const grouped = new Map();
    const columnValues = new Set();
    for (const row of rows) {
      const values = {};
      for (const dimension of effectiveRowDimensions) {
        values[dimension.key] = dimension.synthetic ? "All" : dimensionValueForRow(row, tabConfig, dimension);
      }
      const columnValue = columnDimensionValueForRow(row, tabConfig, columnDimension);
      const groupKey = effectiveRowDimensions.map((dimension) => normalizeText(values[dimension.key])).join("::");
        ...
        for (const columnValue of orderedColumnValues) {
          ...
          for (const metric of metrics) {
            ...
            if (metric.key === "leadShare") {
              ...
              payload[key] = Number(metricValues[metric.key] || 0);
            }
          }
        }
        return payload;
