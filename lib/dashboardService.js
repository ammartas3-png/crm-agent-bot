...
function pruneReportCache() {
  if (reportResultCache.size <= REPORT_RESULT_CACHE_MAX) {
    return;
  }
  const sorted = [...reportResultCache.entries()].sort(
    (left, right) => Number(left[1]?.ts || 0) - Number(right[1]?.ts || 0),
  );
  while (reportResultCache.size > REPORT_RESULT_CACHE_MAX && sorted.length) {
    const [cacheKey] = sorted.shift();
    reportResultCache.delete(cacheKey);
  }
}

function getCachedReport(cacheKey = "") {
  const cached = reportResultCache.get(cacheKey);
  if (!cached) {
    return null;
  }
  if (Date.now() - Number(cached.ts || 0) > REPORT_RESULT_CACHE_TTL_MS) {
    reportResultCache.delete(cacheKey);
    return null;
  }
...
function setCachedReport(cacheKey = "", value = null) {
  if (!cacheKey || !value) {
    return;
  }
  reportResultCache.set(cacheKey, { ts: Date.now(), value: cloneSerializable(value) });
  pruneReportCache();
}
...
export async function loadDashboardReport(accessContext, query = {}, options = {}) {
  ...
  const resultCacheKey = useResultCache ? reportCacheKey(accessContext, query) : "";
  if (useResultCache) {
    const cached = getCachedReport(resultCacheKey);
    if (cached) {
      return cached;
    }
  }
  ...
  if (reportMode === "last4") {
    ...
    if (useResultCache) {
      setCachedReport(resultCacheKey, responsePayload);
    }
