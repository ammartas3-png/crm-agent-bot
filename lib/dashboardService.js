...
import { createDashboardReportCache } from "./dashboardReportCache.js";
import { getGoogleCredentialConfig, updateSheetValues } from "./googleSheets.js";
import { getDashboardDataProvider } from "./dashboardDataProvider.js";
...
const REPORT_RESULT_CACHE_TTL_MS = Math.max(0, Number(process.env.REPORT_RESULT_CACHE_TTL_MS || 45 * 1000));
const REPORT_RESULT_CACHE_MAX = Math.max(10, Number(process.env.REPORT_RESULT_CACHE_MAX || 180));
const REPORT_MAX_RESPONSE_ROWS = 2000;

let deskLanguageMapCache = null;
const reportResultCache = createDashboardReportCache({
  ttlMs: REPORT_RESULT_CACHE_TTL_MS,
  maxEntries: REPORT_RESULT_CACHE_MAX,
});

export function clearDashboardReportCache() {
  reportResultCache.clear();
}
...
function normalizeOfficeScopeOptions(officeMap = {}, permissionFilters = {}) {
  ...
  const matched = allOffices
    .filter((office) => allowedSet.has(normalizeText(office)))
    .sort((left, right) => left.localeCompare(right));
  if (matched.length) {
    return matched;
  }
  // Fail-open for scope labels when office mapping sheet is temporarily unavailable.
  return uniqueValues(normalizeStringList(permissionFilters.office)).sort((left, right) => left.localeCompare(right));
}
...
async function readOfficeDeskLanguageSnapshot(options = {}) {
  ...
  const dataProvider = options.dataProvider || getDashboardDataProvider();
  ...
  try {
    const rows = await dataProvider.readSheetRows("officeDeskLanguage", {
...
async function readOfficeDeskLanguageMap(options = {}) {
  const snapshot = await readOfficeDeskLanguageSnapshot(options);
...
async function readLeadsRowsForBenchmarkBaseline({
  ...
  let rawRows = [];
  try {
    rawRows = await dataProvider.readSheetRows("leads", {
      tabConfig,
...
export async function refreshOfficeDeskLanguageBenchmarks(options = {}) {
  ...
  const dataProvider = options.dataProvider || getDashboardDataProvider();
  const updateSheetValuesFn = typeof options.updateSheetValues === "function" ? options.updateSheetValues : updateSheetValues;
  ...
  const officeMap = await dataProvider.getOfficeMonthMap().catch(() => ({
    offices: [],
    byOffice: {},
  }));
  ...
  const resolveOfficeRosterRows = async (office = "") => {
    ...
    if (!rosterRowsByOffice.has(normalizedOffice)) {
      rosterRowsByOffice.set(
        normalizedOffice,
        await readOfficeAgentRosterRows(normalizedOffice, { dataProvider }),
      );
    }
  ...
  const languageSnapshot = await readOfficeDeskLanguageSnapshot({ forceRefresh: true, dataProvider });
...
async function readOfficeAgentRosterRows(officeScope = "", options = {}) {
  ...
  try {
    const rows = await dataProvider.readSheetRows("officeAgentRoster", {
...
async function readMonthData({
  ...
  const rosterRowsRawPromise = Array.isArray(officeRosterRows)
    ...
    : (() => {
        ...
        return readOfficeAgentRosterRows(officeScope || monthRecord.office_name || "", { dataProvider });
      })();
  const legacyInfoRowsPromise = includeLegacyInfoTargets
    ? (() => {
        notifyProgress("Info Agents");
        return dataProvider.readSheetRows("infoAgents", {
  ...
  const kycFtdCustomerIdSetPromise = includeKycFtd
    ? (() => {
        notifyProgress("FTD");
        return dataProvider.readSheetRows("ftd", {
...
export async function loadDashboardReport(accessContext, query = {}, options = {}) {
  ...
  const useResultCache = REPORT_RESULT_CACHE_TTL_MS > 0 && !progressHandler && !options.skipResultCache;
  const resultCacheKey = useResultCache ? reportResultCache.key(accessContext, query) : "";
  if (useResultCache) {
    const cached = reportResultCache.get(resultCacheKey);
    if (cached) {
      return cached;
    }
  }
  ...
  const resolveOfficeRosterRows = async (officeName = "") => {
    ...
    if (!rosterRowsPromiseByOffice.has(office)) {
      rosterRowsPromiseByOffice.set(
        office,
        readOfficeAgentRosterRows(office, { dataProvider })
          .then((rows) => {
  ...
  const readMonthDataCached = (params = {}) => {
    const cacheKey = JSON.stringify({
      provider: dataProvider.name || "unknown",
    ...
    if (!monthDataRequestCache.has(cacheKey)) {
      monthDataRequestCache.set(cacheKey, readMonthData({ ...params, dataProvider }));
    }
  ...
  if (reportMode === "last4") {
    ...
    if (useResultCache) {
      reportResultCache.set(resultCacheKey, responsePayload);
    }
  ...
  let deskLanguageSnapshot =
    reportMode === "specific"
      ? await readOfficeDeskLanguageSnapshot({ dataProvider })
  ...
  if (shouldHydrateBenchmarkCacheNow) {
    try {
      ...
      deskLanguageSnapshot = await readOfficeDeskLanguageSnapshot({ forceRefresh: true, dataProvider });
      deskBenchmarkCache =
        deskLanguageSnapshot?.benchmarkCache instanceof Map ? deskLanguageSnapshot.benchmarkCache : new Map();
    } catch {
