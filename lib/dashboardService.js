import { getTabConfig, sheetRange } from "../config/sheetsConfig.js";
import {
  calculateSummary,
  dateMatches,
...
import { getGoogleCredentialConfig, getSheetTitles, readSheetRows, readSheetValues, updateSheetValues } from "./googleSheets.js";
import { lastIngestSyncAt, loadAuxiliaryRows, readDashboardSheetRows, shouldUseIngestForDashboard } from "./dataProvider.js";
import {
...
import { buildDeskLanguageLabelMap, buildTeamRosterReport } from "./teamRoster.js";
...
export function mergedInfoRowsFromRoster(rosterRows = [], targetByAgent = new Map(), monthStatusByAgent = new Map()) {
  const rows = [];
  for (const row of rosterRows) {
    ...
    // Month-report semantics: the month's Info Agents tab is authoritative for
    // that month's status. When it explicitly marks the agent working/not
    // working, prefer it over the roster's current status (an agent who worked
    // in the month but has since left the roster should still count that month).
    const monthStatus = monthStatusByAgent.get(normalizedAgent);
    const workingStatus =
      monthStatus === "working"
        ? "Working"
        : monthStatus === "not_working"
          ? "Not Working"
          : rosterWorkingStatusValue(row?.["Working Status"] || row?.Status || "");
    rows.push({
...
function latestMonthKeyFromMonthData(monthData = []) {
  return monthData.reduce((max, item) => {
...
function deriveLatestStatusByAgent(monthData = []) {
  const latestKey = latestMonthKeyFromMonthData(monthData);
  const map = new Map();
  if (!latestKey) {
    return map;
  }
...
function buildTargetMetrics(rows, tabConfig, infoContext, dateFilter, now = new Date()) {
  ...
  return {
    ftdTarget: Number(targetAggregation?.rawTarget || 0),
  };
}
...
// Last N-day window (default 7) of a single selected month, for the "left the
// job right at month end" exception in the target-achieved rate.
function monthLastDaysWindow(dateFilter, windowDays = 7) {
  if (!dateFilter || dateFilter.type !== "month") {
    return null;
  }
  const year = Number(dateFilter.year);
  const month = Number(dateFilter.month); // 0-indexed
  if (!Number.isFinite(year) || !Number.isFinite(month)) {
    return null;
  }
  const lastDay = new Date(Date.UTC(year, month + 1, 0)).getUTCDate();
  return { year, month, lastDay, thresholdDay: Math.max(1, lastDay - (windowDays - 1)) };
}

function leftInLastDays(endDateRaw, window) {
  if (!window) {
    return false;
  }
  const parsed = parseDateValue(endDateRaw);
  if (!parsed) {
    return false;
  }
  return (
    parsed.getUTCFullYear() === window.year &&
    parsed.getUTCMonth() === window.month &&
    parsed.getUTCDate() >= window.thresholdDay
  );
}
...
export function buildDashboardStats(rows, tabConfig, infoContext, dateFilter, now = new Date()) {
  const aggregation = targetAggregationForScope({
    ...
    scope: {
      groupField: "agentNames",
      // Include not-working agents too; the target-achieved rule below decides
      // which of them count (achieved, or left in the last 7 days of the month).
      onlyWorkingAgents: false,
      // Restrict the population to the agents actually in the filtered/scoped
      // view (not the whole office).
      restrictToRows: true,
    },
    now,
  });
  // Team leaders sometimes also appear as an "agent" in the data. Exclude them
  // from the agent counts and from the target-achieved calculations, so those
  // metrics reflect only real agents (not the TLs themselves).
  const teamLeaderNameSet = new Set();
  const addTeamLeaderName = (raw) => {
    const value = String(raw || "").trim();
    if (!value) {
      return;
    }
    teamLeaderNameSet.add(normalizeText(value));
    teamLeaderNameSet.add(normalizeAgentName(value));
  };
  const teamLeaderField = getFieldName(tabConfig, "teamLeader");
  // Month-specific team leaders only: take them from the reported month's rows
  // (the leads' Team Leader field). Do NOT add infoContext.teamLeaders, which is
  // the current roster — otherwise an agent who was promoted to team leader in a
  // later month is wrongly dropped from this month's agent counts even though
  // they were a regular agent (with a target) during the reported month.
  for (const row of rows) {
    addTeamLeaderName(getRowValue(row, teamLeaderField));
  }
  const isTeamLeaderName = (raw) => {
    const value = String(raw || "").trim();
    if (!value) {
      return false;
    }
    return teamLeaderNameSet.has(normalizeText(value)) || teamLeaderNameSet.has(normalizeAgentName(value));
  };

  const isNotTeamLeader = (item) =>
    !teamLeaderNameSet.has(item.normalized_name) && !isTeamLeaderName(item.agent_name);
  // All agents in view (real agents, not team leaders).
  const agentDetails = (aggregation?.details || []).filter(isNotTeamLeader);
  const totalAgent = new Set(agentDetails.map((item) => item.normalized_name)).size;
  // Currently working / active agents.
  const activeAgent = agentDetails.filter((item) => normalizeText(item.working_status || "") === "working").length;

  const lastDaysWindow = monthLastDaysWindow(dateFilter);
  const endDateByAgent = infoContext?.endDateByAgent instanceof Map ? infoContext.endDateByAgent : new Map();
  // Rate Of Target Achieved population: agents with a positive target, minus
  // agents who LEFT the job without reaching target — unless they left in the
  // last 7 days of the month (then they still count). An agent who reached
  // target and left still counts (positive).
  const targetAgents = agentDetails.filter((item) => {
    if (!(Number(item.target || 0) > 0)) {
      return false;
    }
    const status = normalizeText(item.working_status || "");
    if (status !== "not_working") {
      return true; // working (or unknown) -> always counts
    }
    const achieved = Number(item.ftd || 0) >= Number(item.target || 0);
    if (achieved) {
      return true; // reached target then left -> counts
    }
    return leftInLastDays(endDateByAgent.get(item.normalized_name), lastDaysWindow);
  });
  ...
  return {
    ...
    agentsWithTarget: targetAgents.length,
    totalTargetAchieved: targetAchievedCount,
    rateOfTargetAchieved: targetAchievedRate,
  };
}

// Target Result report: from the standard agent builder rows (Desk / Team
// Leader / Agent with Target / FTD / Target Reach), keep only agents that have a
// target, drop team leaders who did not reach their target, sort by Target Reach
// descending, and summarise how many reached target (Reach >= 100%).
export function buildTargetResultReport(builder, infoContext) {
  const sourceRows = (Array.isArray(builder?.table) ? builder.table : []).filter((row) => row?.__rowKind !== "total");
  const teamLeaderNameSet = new Set();
  for (const teamLeader of infoContext?.teamLeaders || []) {
    const value = String(teamLeader || "").trim();
    if (value) {
      teamLeaderNameSet.add(normalizeText(value));
      teamLeaderNameSet.add(normalizeAgentName(value));
    }
  }
  const isTeamLeader = (agent) => {
  ...
  const isNotWorking = (agent) => {
    const key = normalizeAgentName(agent);
    if (!key) {
      return false;
    }
    const raw = latestStatusByAgent?.get?.(key) || byAgent?.get?.(key)?.working_status || "";
    // Only exclude agents with an explicit "not working" status. A blank/unknown
    // status is not treated as not-working here, so agents whose status we could
    // not resolve are still shown (avoids wrongly wiping rows).
    if (!String(raw).trim()) {
      return false;
    }
    return normalizeWorkingStatusValue(raw) === "not_working";
  };
  const rows = sourceRows
    .filter((row) => {
      if (!(Number(row.ftdTarget || 0) > 0)) {
        return false; // no target -> excluded entirely
      }
      const reach = Number(row.ftdTargetReach || 0);
      if (isTeamLeader(row.agent) && reach < 100) {
        return false; // team leader who did not reach target -> excluded
      }
      if (isNotWorking(row.agent) && reach < 100) {
        return false; // not-working agent who did not reach target -> excluded
      }
      return true;
    })
    .map((row) => ({
      ...
      ftdTargetReach: Number(row.ftdTargetReach || 0),
    }))
    .sort((left, right) => right.ftdTargetReach - left.ftdTargetReach);
  const total = rows.length;
  const reached = rows.filter((row) => row.ftdTargetReach >= 100).length;
  return {
    rows,
    columns: [
      ...
      { key: "ftdTarget", label: "Target", type: "number" },
      { key: "ftd", label: "FTD", type: "number" },
      { key: "ftdTargetReach", label: "Target Reach", type: "percent" },
    ],
    summary: {
      total,
      reached,
      notReached: Math.max(0, total - reached),
      rate: total > 0 ? (reached / total) * 100 : 0,
    },
  };
}
...
async function readMonthData({
  ...
  // Month-specific working status from this month's Info Agents tab; it wins over
  // the roster's current status when merging (option A).
  const monthStatusByAgent = new Map();
  for (const [normalized, record] of legacyInfoContext.byAgent) {
    if (record?.working_status === "working" || record?.working_status === "not_working") {
      monthStatusByAgent.set(normalized, record.working_status);
    }
  }
  const mergedInfoRows = mergedInfoRowsFromRoster(rosterRows, legacyInfoContext.targetsByAgent, monthStatusByAgent);
  ...
  // Fired/exit dates for the WHOLE office roster (All Office Agents), not pruned
  // to agents that have leads this month. Roster-only Not-Working agents (e.g. a
  // fired agent with no leads in the month) are injected into the standard agent
  // report but their fired date lives only on the roster, so keep an unpruned map
  // to resolve their Exit Date at injection time.
  const rosterEndDateByAgent = collectAgentEndDateMap(rosterRowsRaw, infoAgentsTabConfig, new Set());
  // Starting dates for the WHOLE office roster (All Office Agents), not pruned to
  // agents that have leads this month. Roster-only agents (e.g. a brand-new agent
  // with no leads yet) are injected into the standard agent report, but their
  // Starting Date lives on the roster; keep an unpruned map so their Start Date
  // resolves at injection time instead of falling back to the team leader's.
  const rosterStartDateByAgent = collectAgentStartDateMap(rosterRowsRaw, infoAgentsTabConfig, new Set());
  ...
  // Full Info-Agents roster for this month (NOT pruned to agents that have
  // leads), so the standard agent report can also list roster-only agents.
  const rosterAgents = buildInfoAgentsContext(legacyInfoRows).records.filter(
    (record) => record?.normalized_name && !isExcludedNormalizedAgent(record.normalized_name),
  );
  return {
    monthRecord,
    rows: permissionRows,
    ...
    infoContext,
    statusByAgent,
    rosterEndDateByAgent,
    rosterStartDateByAgent,
    rosterAgents,
  };
}
...
export function kycFtdCountFromRows(rows = [], tabConfig, options = {}) {
  ...
  // restrictToRows without an explicit agent/team/desk scope (i.e. the Grand
  // Total over a filtered view) must count only the KYC FTDs of the agents
  // actually present in the filtered leads rows — otherwise it counts the whole
  // office's KYC FTDs (e.g. 2186 instead of the ~373 the visible rows add up
  // to). Scopes that already name an agent/team/desk are left untouched.
  const hasExplicitScope =
    (Array.isArray(scope.agent) && scope.agent.length) ||
    (Array.isArray(scope.teamLeader) && scope.teamLeader.length) ||
    (Array.isArray(scope.office) && scope.office.length);
  if (scope.restrictToRows && !hasExplicitScope) {
    const agentField = getFieldName(tabConfig, "agentNames");
    const agentSet = new Set();
    for (const row of rows || []) {
      const agentValue = String(getRowValue(row, agentField) || "").trim();
      if (agentValue) {
        agentSet.add(agentValue);
      }
    }
    if (agentSet.size) {
      effectiveScope = { ...scope, agent: [...agentSet] };
    }
  }
...
function workTimeValuesFromRows(rows = [], tabConfig, infoContext, now = new Date(), options = {}) {
  ...
  let startDateRaw =
    infoContext?.startDateByAgent?.get(normalizedAgent) || infoContext?.byAgent?.get(normalizedAgent)?.start_date || "";
  // Only fall back to the team leader's start date when this row actually
  // represents the team leader themselves (a team-leader-grouped row, or a
  // leader who also appears as their own agent). A regular agent must never
  // inherit their leader's start date — doing so made brand-new agents (e.g.
  // Seifallah Ho, started 31/08/2026) show their leader's much older start date
  // and a wildly inflated tenure.
  if (!startDateRaw && rowTeamLeader && (!rowAgent || rowAgent === rowTeamLeader)) {
    startDateRaw =
  ...
  const rosterEndRaw = infoContext?.endDateByAgent?.get(normalizedAgent) || "";
  let statusRaw =
    latestStatusMap?.get(normalizedAgent) ||
    normalizeWorkingStatusValue(infoContext?.byAgent?.get(normalizedAgent)?.working_status || "");
  // A real roster fired date that falls before the first day of the latest
  // selected month means the agent had already left by the time the viewed
  // period started, so they are Not Working — even if an earlier month's Info
  // Agents tab still carried a stale "Active" status that leaked through here.
  // When the fired date lands within (or after) the latest selected month the
  // agent still counts as working that month, so the resolved status is left
  // untouched (matches the end-of-month "still working in August" case).
  const latestMonthKey = options.latestMonthKey || infoContext?.latestMonthKey || "";
  if (statusRaw === "working" && rosterEndRaw && latestMonthKey) {
    const parsedEnd = parseDateValue(rosterEndRaw);
    const monthMatch = /^(\d{4})-(\d{2})$/.exec(String(latestMonthKey).trim());
    if (parsedEnd && monthMatch) {
      const latestMonthStartMs = Date.UTC(Number(monthMatch[1]), Number(monthMatch[2]) - 1, 1);
      const endMs = Date.UTC(
        parsedEnd.getUTCFullYear(),
        parsedEnd.getUTCMonth(),
        parsedEnd.getUTCDate(),
      );
      if (endMs < latestMonthStartMs) {
        statusRaw = "not_working";
      }
    }
  }
  // Exit Date only applies to agents who are Not Working in the selected period.
  // A Working/Active agent may still carry an (often end-of-month) fired date on
  // the roster, but surfacing it is misleading — they are still working this
  // month. Suppress it (and let their tenure count to today) unless they are Not
  // Working. Not-working agents keep their roster fired date as the exit date.
  const endDateRaw = statusRaw === "working" ? "" : rosterEndRaw;
...
export function specificBuilderTable(rows, tabConfig, infoContext, monthFilter, query = {}, now = new Date(), options = {}) {
  ...
  if (columnDimension) {
    ...
    // Drop invalid / nonsense column buckets: missing dates ("-"), typo years
    // (e.g. "206-08-W3" from a "0206" lead date), and — for a single selected
    // month — any bucket whose date falls outside that month (the reason an
    // August report used to show July columns). Applies per level, so it also
    // covers Week/Date sub-columns nested under a Month/Week primary.
    const isSingleMonth = Boolean(monthFilter && monthFilter.type === "month");
    const columnPartIsAcceptable = (dim, rawPart) => {
      const part = String(rawPart == null ? "" : rawPart).trim();
      if (!part || part === "-") {
        return false;
      }
      if (dim === "hour") {
        return /^\d{2}:00$/.test(part);
      }
      let firstOfPeriod = "";
      if (dim === "month") {
        const matched = part.match(/^(\d{4})-(\d{2})$/);
        if (!matched || Number(matched[1]) < 2000 || Number(matched[1]) > 2100) {
          return false;
        }
        firstOfPeriod = `${part}-01`;
      } else if (dim === "week") {
        const matched = part.match(/^(\d{4})-(\d{2})-W\d$/);
        if (!matched || Number(matched[1]) < 2000 || Number(matched[1]) > 2100) {
          return false;
        }
        firstOfPeriod = `${matched[1]}-${matched[2]}-01`;
      } else if (dim === "date") {
        const matched = part.match(/^(\d{4})-(\d{2})-(\d{2})$/);
        if (!matched || Number(matched[1]) < 2000 || Number(matched[1]) > 2100) {
          return false;
        }
        firstOfPeriod = part;
      ...
      return isSingleMonth ? dateMatches(firstOfPeriod, monthFilter, now) : true;
    };
    const orderedColumnValues = orderedColumnValuesRaw.filter((columnValue) => {
      if (columnValue === COLUMN_GRAND_TOTAL_KEY) {
        return true;
      }
      const parts = columnDimension2 ? String(columnValue).split(COLUMN_LEVEL_SEPARATOR) : [columnValue];
      const dims = columnDimension2 ? [columnDimension, columnDimension2] : [columnDimension];
      return parts.every((part, index) => columnPartIsAcceptable(dims[index], part));
    });
  ...
  const wantsFullDetail = ["1", "true", "yes", "on"].includes(normalizeText(query.comparisonMode)) ||
    ["1", "true", "yes", "on"].includes(normalizeText(query.fullDetailRows));
  ...
  // Mid-month transfer handling: when an agent worked under more than one team in
  // the month they appear on one row per team (their leads/FTD stay where they
  // were made). Their agent-scoped metrics (FTD Target, KYC FTD) would otherwise
  // repeat in full on every row, so split them:
  //   - FTD Target: each non-latest team keeps a target equal to the FTD made
  //     there; the latest (current) team carries the remainder (total - others).
  //   - KYC FTD: attributed entirely to the latest (current) team.
  // Every such agent's rows are flagged (__transferAgent) so the UI/export can
  // highlight them.
  const metricsHasTarget = metrics.some((metric) => metric.key === "ftdTarget");
  const metricsHasReach = metrics.some((metric) => metric.key === "ftdTargetReach");
  const metricsHasKyc = metrics.some((metric) => metric.key === "kycFtd");
  if (dimensionKeys.includes("agent") && (metricsHasTarget || metricsHasKyc)) {
    const teamKeyOf = (values = {}) =>
      `${normalizeText(values.desk || "")}||${normalizeText(values.teamLeader || "")}`;
    const latestHierarchy = buildLatestAgentHierarchyByMonth(rows, tabConfig);
    const rowsByAgent = new Map();
    for (const detailRow of detailRows) {
      const agentKey = normalizeAgentName(detailRow.values?.agent);
      ...
      rowsByAgent.get(agentKey).push(detailRow);
    }
    for (const [agentKey, group] of rowsByAgent) {
      if (group.length < 2) {
        continue;
      }
      const ftdByRow = group.map(
        (detailRow) =>
          Number(
            calculateSummary(
              detailRow.sourceRows,
              tabConfig,
              monthFilter ? { date: monthFilter } : {},
              now,
            ).totalFtd,
      ...
      const totalTarget = Math.max(...group.map((detailRow) => Number(detailRow.payload?.ftdTarget) || 0));
      const totalKyc = Math.max(...group.map((detailRow) => Number(detailRow.payload?.kycFtd) || 0));
      const latest = latestHierarchy.get(agentKey);
      const latestKey = latest ? `${normalizeText(latest.desk || "")}||${normalizeText(latest.teamLeader || "")}` : "";
      let latestIndex = group.findIndex((detailRow) => teamKeyOf(detailRow.values) === latestKey);
      if (latestIndex < 0) {
        const maxFtd = Math.max(...ftdByRow);
        latestIndex = ftdByRow.indexOf(maxFtd);
        if (latestIndex < 0) {
          latestIndex = group.length - 1;
        }
      }
      const sumNonLatestFtd = ftdByRow.reduce((sum, value, index) => (index === latestIndex ? sum : sum + value), 0);
      group.forEach((detailRow, index) => {
        detailRow.payload.__transferAgent = true;
        if (metricsHasTarget) {
          const splitTarget = index === latestIndex ? Math.max(0, totalTarget - sumNonLatestFtd) : ftdByRow[index];
          detailRow.payload.ftdTarget = splitTarget;
          if (metricsHasReach) {
            detailRow.payload.ftdTargetReach = targetReachPercent(ftdByRow[index], splitTarget);
          }
        }
        if (metricsHasKyc) {
          detailRow.payload.kycFtd = index === latestIndex ? totalKyc : 0;
        }
      });
    }
  }
  ...
  // recomputed via the standard summary path. restrictToRows keeps the FTD
  // Target population to the agents actually in the filtered view — otherwise
  // the target was summed over every working agent in the whole office (e.g.
  // 2375 instead of the ~340 the visible rows add up to), which made the grand
  // total FTD Target and FTD Target Reach nonsensical for a filtered report.
  const grandTotalFlatSummary = scopedSummary(rows, {}, { restrictToRows: true });
  ...
  // FTD Target (and its reach) show the raw assigned target per agent regardless
  // of working status, so the grand total must equal the sum of the visible rows
  // rather than the working-only aggregate — otherwise the total is smaller than
  // the column adds up to (not-working agents with a target are in the column).
  const metricKeySet = new Set(metrics.map((metric) => metric.key));
  if (metricKeySet.has("ftdTarget")) {
    const summedFtdTarget = detailRows.reduce((sum, detailRow) => sum + (Number(detailRow.payload?.ftdTarget) || 0), 0);
    grandTotalFlatRow.ftdTarget = summedFtdTarget;
    if (metricKeySet.has("ftdTargetReach")) {
      grandTotalFlatRow.ftdTargetReach = targetReachPercent(Number(grandTotalFlatSummary.totalFtd || 0), summedFtdTarget);
    }
  }
  // KYC FTD is attributed to a single team per agent (see transfer split), so the
  // grand total equals the sum of the visible rows.
  if (metricKeySet.has("kycFtd")) {
    grandTotalFlatRow.kycFtd = detailRows.reduce((sum, detailRow) => sum + (Number(detailRow.payload?.kycFtd) || 0), 0);
  }
...
export async function dashboardBootstrap(accessContext, options = {}) {
  ...
  const lastSyncAt = await lastIngestSyncAt().catch(() => null);
  return {
...
// Detect the HR-code directory tab in an office spreadsheet. Offices name it
// inconsistently ("Agent ID", "AGENT ID", "HR code", ...) and Sheets tab names
// are case-sensitive in A1 ranges, so we resolve the real title per sheet.
function resolveAgentDirectoryTabTitle(titles = []) {
  ...
  return (
    list.find((title) => {
      ...
      return (
        ...
        normalized === "hr codes" ||
        normalized === "hrcode" ||
        (normalized.includes("agent") && normalized.includes("id")) ||
        normalized.includes("hr code")
      );
    }) || ""
  );
}

// Build an agent-name -> HR code map for ALL selected offices/months. Each
// office spreadsheet holds its own directory tab (col A = agent name, col B = HR
// code); they are merged so the HR Code column works for every office, not just
// Turkey.
async function loadAgentHrCodeMap(selectedMonthRecords = []) {
  ...
  await Promise.all(
    uniqueSheetIds.map(async (sheetId) => {
      try {
        const titles = await getSheetTitles(sheetId).catch(() => []);
        const title = resolveAgentDirectoryTabTitle(titles);
        if (!title) {
          return;
        }
        const perSheetConfig = {
          ...directoryConfig,
          name: title,
          range: sheetRange(title, "A:B"),
        };
        const rows = await readSheetRows("agentDirectory", {
          tabConfig: perSheetConfig,
        ...
        // Ignore offices without an HR-code tab (e.g. not set up yet).
      }
    }),
  );
  return map;
...
async function loadDashboardReportUncached(accessContext, query = {}, options = {}) {
  ...
  const isTargetResultMode = ["1", "true", "yes", "on"].includes(normalizeText(query.targetResult));
  const isTeamRosterMode = ["1", "true", "yes", "on"].includes(normalizeText(query.teamRoster));
  const includeKycFtd =
    !isLeadSplitterMode &&
  ...
  const needsLegacyInfoTargets =
    reportMode !== "specific" ||
    specificMetricKeys.includes("ftdTarget") ||
    specificMetricKeys.includes("ftdTargetReach") ||
    // Agent-dimension reports also list roster-only agents, which needs the
    // Info Agents tab loaded.
    parseCsvSelection(query.rowDimensions).map((key) => normalizeText(key)).includes("agent");
  const officeMap = await getOfficeMonthMap().catch(() => ({
    offices: [],
    byOffice: {},
  }));
  ...
  // Team Roster: a pure roster/headcount snapshot for the selected office(s).
  // It needs only the office agent roster tab + the Desk->Lang map, so we build
  // it here and return early, skipping the entire leads/FTD pipeline.
  if (isTeamRosterMode) {
    const rosterRowsAll = (await Promise.all(selectedOfficeScopes.map((office) => resolveOfficeRosterRows(office)))).flat();
    const rosterPermTabConfig = { fields: { agentNames: "Agent", teamLeader: "Team Leader", desk: "Desk", office: "Office" } };
    const scopedRosterRows = accessContext?.authorityScope?.unrestricted
      ? rosterRowsAll
      : filterRowsByPermission(rosterRowsAll, rosterPermTabConfig, accessContext.permissionFilters || {});
    const languageSnapshot = await readOfficeDeskLanguageSnapshot().catch(() => ({ rows: [] }));
    const deskLangMap = buildDeskLanguageLabelMap(languageSnapshot?.rows || []);
    // Team Roster always counts only active (Working) agents.
    const teamRosterReport = buildTeamRosterReport(scopedRosterRows, {
      deskLangMap,
      workingFilter: "working",
    });
    const teamRosterOfficeLabel = selectedOfficeScopes.length === 1 ? selectedOfficeScopes[0] : `${selectedOfficeScopes.length} Offices`;
    emitProgress("Returning Results", 100);
    return {
      reportMode: "specific",
      specificType: "",
      tableType: "teamroster",
      teamRoster: { ...teamRosterReport, office: teamRosterOfficeLabel },
      month: { key: "", label: "", office_name: teamRosterOfficeLabel },
      summary: {},
      table: [],
      tableTitle: "Team Roster",
      pagination: { page: 1, rowLimit: 0, totalRows: 0, totalPages: 0, truncated: false },
      builder: null,
      stats: {},
      filters: { officeScope: selectedOfficeScopes.join(",") },
      options: {},
    };
  }
  ...
  // HR Code column is opt-in (off by default), so only read the per-office
  // directory tabs when the request actually asks for it — avoids extra Sheets
  // reads (and cost) on every report.
  const wantsHrCode = ["1", "true", "yes", "on"].includes(normalizeText(query.showHrCode));
  const agentHrCodeByAgent = wantsHrCode
    ? await loadAgentHrCodeMap(selectedMonthRecords).catch(() => new Map())
    : new Map();
  ...
  combinedInfoContext.latestMonthKey = latestMonthKeyFromMonthData(selectedMonthData);
  ...
  // Standard agent report: also list roster (Info Agents) agents that have no
  // leads/FTD this month, so working/not-working agents still appear (with
  // zeros) using their roster desk/team/status. Applies to the plain
  // agent-grouped builder AND the Month column-pivot (agents show up with zero
  // month columns even with no leads) — but not comparison/traffic/leadsplitter/
  // benchmark/last-4/agent-plan, nor finer column pivots (date/week/hour) or a
  // secondary column pivot.
  const truthyQuery = (value) => ["1", "true", "yes", "on"].includes(normalizeText(value));
  const rowDimensionKeys = parseCsvSelection(query.rowDimensions).map((key) => normalizeText(key));
  const columnDimensionValue = normalizeText(query.columnDimension);
  const columnDimension2Value = normalizeText(query.columnDimension2);
  const injectableColumnPivot =
    !columnDimensionValue || (columnDimensionValue === "month" && !columnDimension2Value);
  const isStandardAgentReport =
    reportMode === "specific" &&
    normalizeText(query.specificType) === "builder" &&
    rowDimensionKeys.includes("agent") &&
    !truthyQuery(query.comparisonMode) &&
    !truthyQuery(query.leadSplitter) &&
    !isTrafficPriorityMode &&
    !benchmarkMode &&
    !truthyQuery(query.last4QuickMode) &&
    !truthyQuery(query.agentProductivityPlanMode) &&
    injectableColumnPivot;
  if (isStandardAgentReport) {
    const agentField = getFieldName(tabConfig, "agentNames");
    const deskField = getFieldName(tabConfig, "office");
    const teamLeaderField = getFieldName(tabConfig, "teamLeader");
    const idField = getFieldName(tabConfig, "id");
    const hideNotWorking = shouldHideNotWorkingFromQuery(query);
    const filterDesks = parseCsvSelection(query.desk).map((value) => normalizeText(value));
    const filterTeamLeaders = parseCsvSelection(query.teamLeader).map((value) => normalizeText(value));
    const filterAgents = parseCsvSelection(query.agent).map((value) => normalizeAgentName(value));
    // Full roster across the selected months (prefer a working record).
    const rosterByAgent = new Map();
    const rosterEndDateByAgentCombined = new Map();
    const rosterStartDateByAgentCombined = new Map();
    for (const item of selectedMonthData) {
      for (const record of item?.rosterAgents || []) {
        const normalized = record?.normalized_name;
        if (!normalized) {
          continue;
        }
        const previous = rosterByAgent.get(normalized);
        if (!previous || (previous.working_status !== "working" && record.working_status === "working")) {
          rosterByAgent.set(normalized, record);
        }
      }
      const endDateMap = item?.rosterEndDateByAgent;
      if (endDateMap instanceof Map) {
        for (const [normalized, endDate] of endDateMap.entries()) {
          if (!endDate) {
            continue;
          }
          rosterEndDateByAgentCombined.set(
            normalized,
            preferLaterDateString(rosterEndDateByAgentCombined.get(normalized) || "", endDate),
          );
        }
      }
      const startDateMap = item?.rosterStartDateByAgent;
      if (startDateMap instanceof Map) {
        for (const [normalized, startDate] of startDateMap.entries()) {
          if (!startDate) {
            continue;
          }
          rosterStartDateByAgentCombined.set(
            normalized,
            preferEarlierDateString(rosterStartDateByAgentCombined.get(normalized) || "", startDate),
          );
        }
      }
    }
    const presentAgents = new Set(
      modeRowsCombined.map((row) => normalizeAgentName(getRowValue(row, agentField))).filter(Boolean),
    );
    let rosterOnlyRows = [];
    const rosterRecordByRowId = new Map();
    for (const record of rosterByAgent.values()) {
      const normalized = record.normalized_name;
      if (presentAgents.has(normalized) || isExcludedNormalizedAgent(normalized)) {
        continue;
      }
      if (hideNotWorking && record.working_status === "not_working") {
        continue;
      }
      const desk = record.office || "";
      const teamLeader = record.team_leader || "";
      if (filterDesks.length && !filterDesks.includes(normalizeText(desk))) {
        continue;
      }
      if (filterTeamLeaders.length && !filterTeamLeaders.includes(normalizeText(teamLeader))) {
        continue;
      }
      if (filterAgents.length && !filterAgents.includes(normalized)) {
        continue;
      }
      const rowId = `roster-${normalized}`;
      rosterRecordByRowId.set(rowId, record);
      rosterOnlyRows.push({
        [idField]: rowId,
        [agentField]: record.agent_name || normalized,
        [deskField]: desk,
        [teamLeaderField]: teamLeader,
        __scopeOfficeName: desk || primaryOfficeScope,
        __rosterOnly: true,
      });
    }
    // Enforce the caller's permission scope (office/desk/team/agent) on the
    // injected roster rows so scoped users never see agents outside their scope.
    if (hasPermissionRestrictions && rosterOnlyRows.length) {
      rosterOnlyRows = filterRowsByPermission(rosterOnlyRows, tabConfig, permissionFilters);
    }
    if (rosterOnlyRows.length) {
      // Make target / working status resolve for these roster-only agents by
      // adding them to the (leads-scoped) info context.
      for (const row of rosterOnlyRows) {
        const record = rosterRecordByRowId.get(String(getRowValue(row, idField)));
        const normalized = record?.normalized_name;
        if (!record || !normalized || combinedInfoContext.byAgent?.has?.(normalized)) {
          continue;
        }
        combinedInfoContext.byAgent?.set?.(normalized, record);
        combinedInfoContext.records?.push?.(record);
        combinedInfoContext.targetsByAgent?.set?.(normalized, Number(record.target || 0));
        // Resolve the Start Date for roster-only agents (no leads this month) from
        // the office roster's "Starting Date" column, so a brand-new agent shows
        // their own start date instead of inheriting the team leader's.
        const rosterStart = rosterStartDateByAgentCombined.get(normalized);
        if (rosterStart) {
          if (!(combinedInfoContext.startDateByAgent instanceof Map)) {
            combinedInfoContext.startDateByAgent = new Map();
          }
          combinedInfoContext.startDateByAgent.set(
            normalized,
            preferEarlierDateString(combinedInfoContext.startDateByAgent.get(normalized) || "", rosterStart),
          );
          if (!record.start_date) {
            record.start_date = rosterStart;
          }
        }
        // Resolve the Exit Date for roster-only Not-Working agents (no leads this
        // month) from the office roster's "Working Month /Fired Date" column.
        const rosterEnd = rosterEndDateByAgentCombined.get(normalized);
        if (rosterEnd) {
          if (!(combinedInfoContext.endDateByAgent instanceof Map)) {
            combinedInfoContext.endDateByAgent = new Map();
          }
          if (!combinedInfoContext.endDateByAgent.get(normalized)) {
            combinedInfoContext.endDateByAgent.set(normalized, rosterEnd);
          }
        }
      }
      modeRowsCombined = modeRowsCombined.concat(rosterOnlyRows);
    }
  }
  ...
  if (isTrafficPriorityMode) {
    ...
    builder = specificBuilderTable(modeRowsCombined, tabConfig, combinedInfoContext, combinedMonthFilter, query, now, {
      ...
      deskBenchmarkCache,
      useOfficeSheetBenchmarksOnly,
      // Collapse an agent into their most-recent team for Target Result and
      // multi-month reports. The standard single-month agent report keeps the
      // per-team split (an agent transferred mid-month stays on each team's row
      // for their leads/FTD); the transfer-split post-pass below then divides the
      // target/KYC correctly so the sums still match the Grand Total.
      useLatestAgentHierarchy: isTargetResultMode || selectedMonthKeys.length > 1,
  ...
  // Keep the top summary band's FTD Target in sync with the builder Grand Total
  // (both are the sum of the visible rows' assigned targets). Without this the
  // band still used the working-only aggregate and diverged from the grand total.
  if (builder && !builder.columnDimension && builder.grandTotalRow && builder.grandTotalRow.ftdTarget !== undefined) {
    const grandTotalTarget = Number(builder.grandTotalRow.ftdTarget);
    if (Number.isFinite(grandTotalTarget)) {
      summary = {
        ...summary,
        ftdTarget: grandTotalTarget,
        ftdTargetReach: targetReachPercent(Number(summary.totalFtd || 0), grandTotalTarget),
      };
    }
  }
  ...
  const trafficPriorityReport = isTrafficPriorityMode
    ...
    : null;
  const targetResultReport = isTargetResultMode
    ? buildTargetResultReport(builder, combinedInfoContext)
    : null;

  return {
    reportMode: reportMode === "specific" ? "specific" : "monthly",
    specificType: reportMode === "specific" ? "builder" : "",
    tableType: isTrafficPriorityMode
      ? "trafficpriority"
      : isLeadSplitterMode
        ? "leadsplitter"
        : isTargetResultMode
    ...
    targetResult: targetResultReport,
    month: {
