...
function specificBuilderTable(rows, tabConfig, infoContext, monthFilter, query = {}, now = new Date(), options = {}) {
  ...
  const hrCodeHidden = ["1", "true", "yes", "on"].includes(normalizeText(query.hideHrCode));
  const includeHrCode =
    !hrCodeHidden && dimensionKeys.includes("agent") && Boolean(agentHrCodeByAgent) && agentHrCodeByAgent.size > 0;
...
function reportCacheKey(accessContext, query) {
  ...
  return JSON.stringify({ scope, admin, query: query || {} });
}
