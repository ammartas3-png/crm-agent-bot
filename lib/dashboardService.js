...
const LEAD_SPLITTER_OTHER_DESK = "Other";
...
function leadSplitterRowIsFtd(value) {
...
// derived from the Department field, Leads = row count, FTD = count of rows whose
// FTD cell is 1, CR = FTD / Leads. Rows are assumed to be already office/month/
// authority/filter scoped by loadDashboardReport.
export function buildLeadSplitterReport(rows = [], tabConfig) {
  ...
  const grand = { leads: 0, ftd: 0, crtSum: 0, crtCount: 0 };

  for (const row of Array.isArray(rows) ? rows : []) {
    const desk = deskCodeFromDepartment(getRowValue(row, departmentField)) || LEAD_SPLITTER_OTHER_DESK;
    ...
    const agent =
      ...
      "Unknown";
    const isFtd = leadSplitterRowIsFtd(getRowValue(row, ftdField));
    const crTargetValue = toPercentNumber(getRowValue(row, crTargetField));
    ...
    if (!agentMap.has(agent)) {
      agentMap.set(agent, { leads: 0, ftd: 0, crtSum: 0, crtCount: 0 });
    }
    ...
    if (crTargetValue !== null && Number.isFinite(crTargetValue)) {
      bucket.crtSum += crTargetValue;
      bucket.crtCount += 1;
      grand.crtSum += crTargetValue;
      grand.crtCount += 1;
    }
  }

  const crOf = (ftd, leads) => (leads > 0 ? (ftd / leads) * 100 : 0);
  const crTargetOf = (crtSum, crtCount) => (crtCount > 0 ? crtSum / crtCount : 0);
  const crReachOf = (cr, crTarget) => (crTarget > 0 ? (cr / crTarget) * 100 : 0);
  const metricsFor = ({ leads, ftd, crtSum, crtCount }) => {
    const cr = crOf(ftd, leads);
    const crTarget = crTargetOf(crtSum, crtCount);
    return { leads, ftd, cr, crTarget, crTargetReach: crReachOf(cr, crTarget) };
  };
  ...
  for (const desk of desks) {
    const countryMap = deskMap.get(desk);
    const deskAgg = { leads: 0, ftd: 0, crtSum: 0, crtCount: 0 };
    const countries = [...countryMap.entries()].map(([country, agentMap]) => {
      const agents = [...agentMap.entries()]
        .map(([agent, value]) => ({ agent, ...value }))
        .sort((left, right) => right.leads - left.leads || String(left.agent).localeCompare(String(right.agent)));
      const agg = agents.reduce(
        (sum, item) => ({
          leads: sum.leads + item.leads,
          ftd: sum.ftd + item.ftd,
          crtSum: sum.crtSum + item.crtSum,
          crtCount: sum.crtCount + item.crtCount,
        }),
        { leads: 0, ftd: 0, crtSum: 0, crtCount: 0 },
      );
      return { country, agents, agg };
    });
    countries.sort((left, right) => right.agg.leads - left.agg.leads || String(left.country).localeCompare(String(right.country)));
    for (const entry of countries) {
      for (const agent of entry.agents) {
        outRows.push({ kind: "agent", desk, country: entry.country, agent: agent.agent, ...metricsFor(agent) });
      }
      outRows.push({
        kind: "countryTotal",
        desk,
        country: entry.country,
        label: `${entry.country} Total`,
        ...metricsFor(entry.agg),
      });
      deskAgg.leads += entry.agg.leads;
      deskAgg.ftd += entry.agg.ftd;
      deskAgg.crtSum += entry.agg.crtSum;
      deskAgg.crtCount += entry.agg.crtCount;
    }
    outRows.push({ kind: "deskTotal", desk, country: "", label: `${desk} Total`, ...metricsFor(deskAgg) });
  }

  return {
    rows: outRows,
    summary: metricsFor(grand),
  };
}
...
export async function loadDashboardReport(accessContext, query = {}, options = {}) {
  ...
  const leadSplitterReport = isLeadSplitterMode ? buildLeadSplitterReport(modeRowsCombined, tabConfig) : null;

  return {
    reportMode: reportMode === "specific" ? "specific" : "monthly",
    specificType: reportMode === "specific" ? "builder" : "",
    tableType: isLeadSplitterMode ? "leadsplitter" : tableType,
