...
async function readMonthData({ monthRecord, officeScope, tabConfig, infoAgentsTabConfig, permissionFilters }) {
  ...
  const infoRowsByAgentScope = filterInfoRowsByAllowedAgents(infoRows, infoAgentsTabConfig, allowedAgents);
  const infoContext = buildInfoAgentsContext(infoRowsByAgentScope);
  const startDateByAgent = collectAgentStartDateMap(infoRows, infoAgentsTabConfig, allowedAgents);
...
function buildDeskFtdAverages(rows = [], tabConfig, infoContext, now = new Date()) {
  ...
  for (const row of rows) {
    ...
    if (normalizedAgent) {
      if (!entry.agents.has(normalizedAgent)) {
        entry.agents.set(normalizedAgent, { rows: [], monthKeys: new Set() });
      }
      const agentEntry = entry.agents.get(normalizedAgent);
      agentEntry.rows.push(row);
  ...
  for (const [desk, entry] of grouped.entries()) {
    const summary = calculateSummary(entry.rows, tabConfig, {}, now);
    ...
    for (const [normalizedAgent, agentEntry] of entry.agents.entries()) {
      const agentSummary = calculateSummary(agentEntry.rows, tabConfig, {}, now);
      const activeMonthCount = agentEntry.monthKeys.size || 1;
      const agentMonthlyFtd = Number(agentSummary.totalFtd || 0) / activeMonthCount;
      const startDateRaw =
    ...
    output.set(desk, {
      avgFtdByAgent: agentCount > 0 ? Number(summary.totalFtd || 0) / agentCount : 0,
      avgFtdByLongTerm: {
...
function builderExtraMetricValues(rows = [], tabConfig, summary = {}, options = {}) {
  ...
  const uniqueAgent = uniqueAgents.size === 1 ? [...uniqueAgents][0] : "";
  const startDateRaw = uniqueAgent
    ? infoContext?.startDateByAgent?.get(uniqueAgent) || infoContext?.byAgent?.get(uniqueAgent)?.start_date || ""
    : "";
  const workProfile = workProfileFromStartDateRaw(startDateRaw, now);
  const deskAverage =
  ...
  const agentActiveMonthCount = uniqueAgent ? uniqueMonths.size || 1 : 0;
  const agentAvgFtdByMonth = uniqueAgent && agentActiveMonthCount > 0 ? totalFtd / agentActiveMonthCount : 0;
...
function workTimeValuesFromRows(rows = [], tabConfig, infoContext, now = new Date()) {
  const agentField = getFieldName(tabConfig, "agentNames");
  const uniqueAgents = new Set();
  for (const row of rows) {
    const normalizedAgent = normalizeAgentName(getRowValue(row, agentField));
  ...
  const normalizedAgent = [...uniqueAgents][0];
  const startDateRaw =
    infoContext?.startDateByAgent?.get(normalizedAgent) || infoContext?.byAgent?.get(normalizedAgent)?.start_date || "";
...
function specificBuilderTable(rows, tabConfig, infoContext, monthFilter, query = {}, now = new Date()) {
  ...
  if (columnDimension) {
    ...
    const overallSummary = summaryWithTargets(rows, tabConfig, infoContext, monthFilter, now);
    ...
    const tableRows = [...grouped.values()]
      .map((entry) => {
        ...
        if (includeWorkTime) {
          Object.assign(payload, workTimeValuesFromRows([...entry.byColumn.values()].flat(), tabConfig, infoContext, now));
        }
        return payload;
      })
      .sort((left, right) => {
  ...
  const detailRows = [...grouped.values()]
    .map((entry) => {
      ...
      if (includeWorkTime) {
        Object.assign(payload, workTimeValuesFromRows(entry.rows, tabConfig, infoContext, now));
      }
      return {
        payload,
  ...
  if (totalDimensionKeys.length) {
    for (const totalDimensionKey of totalDimensionKeys) {
      ...
      for (const [prefixKey, group] of groupsAtDepth.entries()) {
        ...
        if (includeWorkTime) {
          Object.assign(payload, workTimeValuesFromRows(group.rows, tabConfig, infoContext, now));
        }
