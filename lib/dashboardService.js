...
export async function loadDashboardReport(accessContext, query = {}, options = {}) {
  ...
  const hasPermissionRestrictions = Object.values(accessContext.permissionFilters || {}).some(
    (value) => normalizeStringList(value).length > 0,
  );
  if (reportMode === "specific" && benchmarkMode && hasPermissionRestrictions) {
    const benchmarkOfficeScopes =
      Array.isArray(officeMap?.offices) && officeMap.offices.length ? officeMap.offices : selectedOfficeScopes;
    const benchmarkMonthRecords = benchmarkOfficeScopes
      .flatMap((office) => {
        const officeRecords = collectOfficeScopedMonthRecords(officeMap, office)
          .filter((record) => record.active !== false)
          .sort((left, right) => String(right.key || "").localeCompare(String(left.key || "")));
    ...
    if (benchmarkMonthRecords.length) {
      await Promise.all(benchmarkOfficeScopes.map((office) => resolveOfficeRosterRows(office)));
      const benchmarkMonthData = await Promise.all(
        benchmarkMonthRecords.map((record) =>
          readMonthData({
            monthRecord: record,
            officeScope: record.office_name || primaryOfficeScope,
            tabConfig,
