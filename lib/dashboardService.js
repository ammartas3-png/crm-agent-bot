...
function buildDeskFtdAverages(rows = [], tabConfig, now = new Date()) {
  const deskField = getFieldName(tabConfig, "office");
  const agentField = getFieldName(tabConfig, "agentNames");
  const grouped = new Map();
  for (const row of rows) {
    ...
    if (normalizedAgent) {
      entry.agents.add(normalizedAgent);
    }
  }
...
function builderExtraMetricValues(rows = [], tabConfig, summary = {}, options = {}) {
  ...
  const deskAverage =
    deskFtdAverages.get(normalizedDeskFromValues || normalizedDeskFromRows)?.avgFtdByAgent ??
...
function specificBuilderTable(rows, tabConfig, infoContext, monthFilter, query = {}, now = new Date()) {
  ...
  const deskFtdAverages = buildDeskFtdAverages(rows, tabConfig, now);
