...
function workTimeValuesFromRows(rows = [], tabConfig, infoContext, now = new Date(), options = {}) {
  ...
  const endDateRaw =
    statusRaw === "working" ? "" : infoContext?.endDateByAgent?.get(normalizedAgent) || "";
...
function buildLast4AgentMatrix(monthData = [], tabConfig, query = {}, now = new Date(), options = {}) {
  ...
  for (const item of monthData) {
    ...
    for (const record of item.infoContext?.records || []) {
      // Last 4 Months mirrors the monthly quick report: every agent listed for a
      // selected month is included regardless of working status or whether they
      // produced any leads/targets. The Hide Not Working toggle is still honoured
      // by the final row filter below.
