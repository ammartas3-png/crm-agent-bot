...
export function buildDashboardStats(rows, tabConfig, infoContext, dateFilter, now = new Date()) {
  ...
  const targetAgents = (aggregation?.details || []).filter(
    (item) =>
      Number(item.target || 0) > 0 &&
      !teamLeaderNameSet.has(item.normalized_name) &&
      !isTeamLeaderName(item.agent_name),
  );
  ...
  const totalAgent = uniqueValues(rows, tabConfig, "agentNames").filter((name) => !isTeamLeaderName(name)).length;
  return {
    totalAgent,
