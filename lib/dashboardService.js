...
import { dashboardPerfLog, hashStableValue, isGoogle429Error, isGoogleTimeoutError, safeCacheKeyPart } from "./dashboardPerf.js";
...
const STATIC_SHEET_CACHE_TTL_SECONDS = 30 * 60;
const MONTHLY_SHEET_CACHE_TTL_SECONDS = 10 * 60;
...
const ESTIMATED_ROWS_PER_MONTH_OFFICE = 45_000;
const PRELOAD_ESTIMATED_ROWS_LIMIT = 420_000;
const PRELOAD_MONTH_OFFICE_LIMIT = 14;

let deskLanguageMapCache = null;
const monthDataInflight = new Map();
...
function createPerformanceState(query = {}) {
  return {
    startedAt: Date.now(),
    sheetsLoaded: 0,
    rowsLoaded: 0,
    googleSheetsStartedAt: Date.now(),
    googleSheetsFinishedAt: 0,
    reportBuildStartedAt: 0,
    reportBuildFinishedAt: 0,
    reportMode: String(query.reportMode || "monthly").trim().toLowerCase(),
  };
}

function markGoogleSheetsDone(state) {
  if (!state) {
    return;
  }
  if (!state.googleSheetsFinishedAt) {
    state.googleSheetsFinishedAt = Date.now();
    dashboardPerfLog("GOOGLE_SHEETS_LOAD_TIME", {
      reportMode: state.reportMode,
      ms: state.googleSheetsFinishedAt - state.googleSheetsStartedAt,
    });
  }
}

function markReportBuildStart(state) {
  if (!state || state.reportBuildStartedAt) {
    return;
  }
  state.reportBuildStartedAt = Date.now();
}

function markReportBuildDone(state) {
  if (!state) {
    return;
  }
  if (!state.reportBuildFinishedAt) {
    state.reportBuildFinishedAt = Date.now();
    dashboardPerfLog("REPORT_BUILD_TIME", {
      reportMode: state.reportMode,
      ms: state.reportBuildFinishedAt - Number(state.reportBuildStartedAt || state.reportBuildFinishedAt),
    });
    dashboardPerfLog("TOTAL_EXECUTION_TIME", {
      reportMode: state.reportMode,
      ms: state.reportBuildFinishedAt - state.startedAt,
    });
  }
}

function trackLoadedSheet(state, sheetName = "", rows = 0) {
  if (!state) {
    return;
  }
  state.sheetsLoaded += 1;
  state.rowsLoaded += Number(rows || 0);
  dashboardPerfLog("SHEET_NAME", { sheet: sheetName });
  dashboardPerfLog("ROWS_LOADED", { sheet: sheetName, rows: Number(rows || 0) });
}

function logSheetTotals(state) {
  if (!state) {
    return;
  }
  dashboardPerfLog("TOTAL_SHEETS_REQUESTS", { count: state.sheetsLoaded });
  dashboardPerfLog("TOTAL_ROWS_LOADED", { count: state.rowsLoaded });
}

function monthlyPermissionHash(permissionFilters = {}) {
  return hashStableValue(permissionFilters || {}).slice(0, 16);
}

function monthlySheetKey(sheetPrefix = "", monthKey = "", officeScope = "") {
  return `sheets:${sheetPrefix}:${safeCacheKeyPart(monthKey)}:${safeCacheKeyPart(officeScope)}`;
}

function maybeLogSheetsError(error, context = {}) {
  if (isGoogle429Error(error)) {
    dashboardPerfLog("GOOGLE_SHEETS_429", context);
  }
  if (isGoogleTimeoutError(error)) {
    dashboardPerfLog("GOOGLE_SHEETS_TIMEOUT", context);
  }
}

async function readOfficeDeskLanguageMap(options = {}) {
  const performanceState = options.performanceState || null;
  ...
  const redisKey = "sheets:language:global";
  try {
    let rows = await readCachedJson(redisKey, {
      cacheScope: "sheet-static",
      cacheLabel: "language",
      memoryTtlSeconds: 30,
    });
    if (!Array.isArray(rows)) {
      dashboardPerfLog("GOOGLE_SHEETS_READ_START", {
        sheet: "Language",
        tab: "Language",
      ...
      dashboardPerfLog("GOOGLE_SHEETS_READ_DONE", {
        sheet: "Language",
        tab: "Language",
        ms: Date.now() - readStartedAt,
        rows: rows.length,
      });
      trackLoadedSheet(performanceState, "Language", rows.length);
      await writeCachedJson(redisKey, rows, STATIC_SHEET_CACHE_TTL_SECONDS, {
        cacheScope: "sheet-static",
        cacheLabel: "language",
      });
    }
    ...
    maybeLogSheetsError(error, { sheet: "Language" });
...
async function readOfficeAgentRosterRows(officeScope = "", options = {}) {
  const performanceState = options.performanceState || null;
  const tabName = rosterTabNameForOffice(officeScope);
  if (!tabName) {
    return [];
  }
  const redisKey = `sheets:office-rosters:${safeCacheKeyPart(officeScope)}`;
  try {
    let rows = await readCachedJson(redisKey, {
      cacheScope: "sheet-static",
      cacheLabel: "officeRosters",
      memoryTtlSeconds: 30,
    });
    if (!Array.isArray(rows)) {
      dashboardPerfLog("GOOGLE_SHEETS_READ_START", {
      ...
      trackLoadedSheet(performanceState, tabName, rows.length);
      await writeCachedJson(redisKey, rows, STATIC_SHEET_CACHE_TTL_SECONDS, {
        cacheScope: "sheet-static",
        cacheLabel: "officeRosters",
      });
      await writeCachedJson(`sheets:agent-mappings:${safeCacheKeyPart(officeScope)}`, rows, STATIC_SHEET_CACHE_TTL_SECONDS, {
        cacheScope: "sheet-static",
        cacheLabel: "agentMappings",
      });
      await writeCachedJson(`sheets:targets:${safeCacheKeyPart(officeScope)}`, rows, STATIC_SHEET_CACHE_TTL_SECONDS, {
        cacheScope: "sheet-static",
        cacheLabel: "targets",
      });
    }
    ...
    maybeLogSheetsError(error, { sheet: tabName });
    return [];
  }
}
...
async function readMonthData({
  ...
  const monthKey = String(monthRecord?.key || "").trim();
  const permissionHash = monthlyPermissionHash(permissionFilters || {});
  const inflightKey = `${safeCacheKeyPart(monthRecord?.sheet_id || monthKey)}:${safeCacheKeyPart(scopedOffice)}:${permissionHash}`;
  if (monthDataInflight.has(inflightKey)) {
    return monthDataInflight.get(inflightKey);
  }
  const pending = (async () => {
    const rosterRowsPromise = Array.isArray(officeRosterRows)
      ? Promise.resolve(officeRosterRows)
      : readOfficeAgentRosterRows(scopedOffice, { performanceState });
    const infoRowsCacheKey = monthlySheetKey("infoAgents", monthKey, scopedOffice);
    const leadsRowsCacheKey = monthlySheetKey("leads", monthKey, scopedOffice);

    const legacyInfoRowsPromise = (async () => {
      try {
        let infoRows = await readCachedJson(infoRowsCacheKey, {
          cacheScope: "sheet-monthly",
          cacheLabel: "infoAgents",
          memoryTtlSeconds: 20,
        });
        if (!Array.isArray(infoRows)) {
          dashboardPerfLog("GOOGLE_SHEETS_READ_START", {
            sheet: `${monthKey}:Info Agents`,
            tab: String(infoAgentsTabConfig?.name || "Info Agents"),
            officeScope: scopedOffice,
          ...
          dashboardPerfLog("GOOGLE_SHEETS_READ_DONE", {
            ...
            officeScope: scopedOffice,
            ms: Date.now() - readStartedAt,
            rows: infoRows.length,
          });
          trackLoadedSheet(performanceState, `${monthKey}:Info Agents`, infoRows.length);
          await writeCachedJson(infoRowsCacheKey, infoRows, MONTHLY_SHEET_CACHE_TTL_SECONDS, {
            cacheScope: "sheet-monthly",
            cacheLabel: "infoAgents",
          });
        }
        return mapRowsWithScope(infoRows, scopedOffice);
      } catch (error) {
        maybeLogSheetsError(error, { sheet: `${monthKey}:Info Agents`, officeScope: scopedOffice });
        dashboardPerfLog("GOOGLE_SHEETS_READ_FAILED", {
          sheet: `${monthKey}:Info Agents`,
          tab: String(infoAgentsTabConfig?.name || "Info Agents"),
          officeScope: scopedOffice,
    ...
    let rawRows = [];
    try {
      let leadsRows = await readCachedJson(leadsRowsCacheKey, {
        cacheScope: "sheet-monthly",
        cacheLabel: "leads",
        memoryTtlSeconds: 20,
      });
      if (!Array.isArray(leadsRows)) {
        dashboardPerfLog("GOOGLE_SHEETS_READ_START", {
          sheet: `${monthKey}:Leads`,
          tab: String(tabConfig?.name || "Leads"),
          officeScope: scopedOffice,
          spreadsheetId: monthRecord.sheet_id,
        });
        const readStartedAt = Date.now();
        leadsRows = await readSheetRows("leads", {
          tabConfig,
          spreadsheetId: monthRecord.sheet_id,
        });
        dashboardPerfLog("GOOGLE_SHEETS_READ_DONE", {
          ...
          officeScope: scopedOffice,
          ms: Date.now() - readStartedAt,
          rows: leadsRows.length,
        });
        trackLoadedSheet(performanceState, `${monthKey}:Leads`, leadsRows.length);
        await writeCachedJson(leadsRowsCacheKey, leadsRows, MONTHLY_SHEET_CACHE_TTL_SECONDS, {
          cacheScope: "sheet-monthly",
          cacheLabel: "leads",
        });
      }
      ...
      maybeLogSheetsError(error, { sheet: `${monthKey}:Leads`, officeScope: scopedOffice });
      dashboardPerfLog("GOOGLE_SHEETS_READ_FAILED", {
        sheet: `${monthKey}:Leads`,
        tab: String(tabConfig?.name || "Leads"),
        officeScope: scopedOffice,
    ...
    if (monthDataInflight.get(inflightKey) === pending) {
      monthDataInflight.delete(inflightKey);
    }
  });
  monthDataInflight.set(inflightKey, pending);
  return pending;
}
...
function specificBuilderTable(rows, tabConfig, infoContext, monthFilter, query = {}, now = new Date(), options = {}) {
  ...
  const deskFtdAverages = needsBuilderExtraMetrics
...
export async function loadDashboardReport(accessContext, query = {}, options = {}) {
  ...
  const performanceState = options.performanceState || createPerformanceState(query);
  const returnWithPerformance = (payload) => {
    markReportBuildDone(performanceState);
    return payload;
  };
  const officeMap = await getOfficeMonthMap().catch(() => ({
    offices: [],
  ...
  const resolveOfficeRosterRows = async (officeName = "") => {
    ...
    if (!rosterRowsPromiseByOffice.has(office)) {
      rosterRowsPromiseByOffice.set(
        office,
        readOfficeAgentRosterRows(office, { performanceState })
          .then((rows) => {
  ...
  if (reportMode === "last4") {
    ...
    markGoogleSheetsDone(performanceState);
    logSheetTotals(performanceState);
    markReportBuildStart(performanceState);
  ...
  const estimatedMonthOfficeReads = selectedMonthRecords.length;
  const estimatedRowsToLoad = estimatedMonthOfficeReads * ESTIMATED_ROWS_PER_MONTH_OFFICE;
  if (
    estimatedMonthOfficeReads > PRELOAD_MONTH_OFFICE_LIMIT ||
    estimatedRowsToLoad > PRELOAD_ESTIMATED_ROWS_LIMIT
  ) {
    throw reportTooHeavyError(
      "Selected filters may load too many rows from Google Sheets. Please narrow months, offices, or dimensions and retry.",
      {
        stage: "Loading Google Sheets",
        estimatedRowsToLoad,
        estimatedMonthOfficeReads,
      },
    );
  }
  ...
  if (reportMode === "specific") {
    deskLanguageMap = await readOfficeDeskLanguageMap({ performanceState });
