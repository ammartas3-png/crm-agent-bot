...
function kycFtdRowIdentityKey(row = {}, tabConfig) {
  const agentField = getFieldName(tabConfig, "agentNames");
  const idField = getFieldName(tabConfig, "id");
  return [
    normalizeAgentName(getRowValue(row, agentField)),
    normalizeAgentName(row?.__rawFtdAgent || ""),
    String(getRowValue(row, idField) || ""),
    String(row?.__sourceMonthKey || ""),
  ].join("|");
}

function mergeKycFtdRowsUnique(rows = [], tabConfig) {
  const seen = new Set();
  const merged = [];
  for (const row of rows) {
    const key = kycFtdRowIdentityKey(row, tabConfig);
    if (seen.has(key)) {
      continue;
    }
    seen.add(key);
...
function filterKycFtdRowsForPermission(kycFtdRows = [], tabConfig, permissionFilters = {}, allowedAgents = new Set()) {
  ...
  return mergeKycFtdRowsUnique([...permissionFiltered, ...agentMatched], tabConfig);
}
