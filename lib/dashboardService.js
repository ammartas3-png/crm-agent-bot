...
const DESK_BENCHMARK_EXTRA_METRIC_KEYS = new Set([
  "avgFtdByAgent",
  "avgFtdByAgentDaily",
  "agentAvgFtdPerWorkedMonth",
  "avgFtdByDeskLongTerm",
...
const OFFICE_DESK_LANGUAGE_COLUMNS = ["Desk", "Lang"];
...
const FAST_OPTIONS_ROW_THRESHOLD = 25_000;
const REPORT_MAX_SAFE_DURATION_MS = 260 * 1000;
const REPORT_MAX_RESPONSE_ROWS = 2000;
const BENCHMARK_MAX_UNFILTERED_MONTHS = 4;
const REPORT_TOO_HEAVY_ROW_LIMIT = 320_000;
const REPORT_TOO_HEAVY_COMPLEXITY_LIMIT = 32_000_000;
...
function reportTooHeavyError(message = "Selected report is too heavy to calculate.", details = {}) {
...
function hasNarrowBenchmarkFilters(query = {}) {
  return [
    "date",
    "hour",
    "desk",
    ...
    "teamLeader",
    "agent",
  ].some((key) => parseCsvSelection(query[key]).length > 0);
}

function logDashboardTiming(event = "", payload = {}) {
  try {
    console.info("[dashboard-timing]", JSON.stringify({ event, ...payload }));
  } catch {
    // no-op: logging should never break report execution
  }
}
...
function officeDeskLanguageTabConfig() {
  ...
  return {
    ...
    name: safeTabName,
    range: `'${safeTabName.replace(/'/g, "''")}'!A:B`,
...
async function readOfficeDeskLanguageMap() {
  ...
  if (deskLanguageMapCache && nowMs - deskLanguageMapCache.timestamp < DESK_LANGUAGE_CACHE_TTL_MS) {
  ...
  try {
    const rows = await readSheetRows("officeDeskLanguage", {
      ...
      readCategory: "language",
      selectedMonth: "-",
      selectedOffice: "-",
    });
    const map = buildDeskLanguageMap(rows);
...
async function readOfficeAgentRosterRows(officeScope = "") {
  ...
  try {
    const rows = await readSheetRows("officeAgentRoster", {
      ...
      spreadsheetId: OFFICE_AGENT_ROSTER_SPREADSHEET_ID,
      readCategory: "office_roster",
      selectedMonth: "-",
      selectedOffice: officeScope || "-",
    });
...
async function readMonthData({
  ...
  const scopedOffice = officeScope || monthRecord.office_name || "";
  const selectedMonth = String(monthRecord?.key || "-");
  const rosterRowsPromise = Array.isArray(officeRosterRows)
    ? Promise.resolve(officeRosterRows)
    : readOfficeAgentRosterRows(scopedOffice);
  const legacyInfoRowsPromise = readSheetRows("infoAgents", {
    tabConfig: infoAgentsTabConfig,
    spreadsheetId: monthRecord.sheet_id,
    readCategory: "info_agents",
    selectedMonth,
    selectedOffice: scopedOffice || "-",
  })
    .then((rows) => mapRowsWithScope(rows, scopedOffice))
    .catch(() => []);
  let rawRows = [];
  try {
  ...
  const [rosterRowsRaw, legacyInfoRows] = await Promise.all([rosterRowsPromise, legacyInfoRowsPromise]);
...
function builderExtraMetricValues(rows = [], tabConfig, summary = {}, options = {}) {
  const requestedExtraMetricKeys =
    options.requestedExtraMetricKeys instanceof Set
      ? options.requestedExtraMetricKeys
      : new Set(BUILDER_EXTRA_METRIC_KEYS);
  const needsAgentCount = requestedExtraMetricKeys.has("agentCount");
  const needsAvgLeadByAgent =
    requestedExtraMetricKeys.has("avgLeadByAgent") || requestedExtraMetricKeys.has("avgLeadByAgentDaily");
  const needsLeadDaily = requestedExtraMetricKeys.has("avgLeadByAgentDaily");
  const needsDeskAverage = requestedExtraMetricKeys.has("avgFtdByAgent") || requestedExtraMetricKeys.has("avgFtdByAgentDaily");
  const needsFtdDaily = requestedExtraMetricKeys.has("avgFtdByAgentDaily");
  const needsLongTermAverage =
    requestedExtraMetricKeys.has("avgFtdByDeskLongTerm") || requestedExtraMetricKeys.has("ftdBenchmarkRate");
  const needsAgentWorkedMonth =
    requestedExtraMetricKeys.has("agentAvgFtdPerWorkedMonth") || requestedExtraMetricKeys.has("ftdBenchmarkRate");
  const needsAnyExtraMetric =
    needsAgentCount || needsAvgLeadByAgent || needsDeskAverage || needsLongTermAverage || needsAgentWorkedMonth;
  if (!needsAnyExtraMetric) {
    return {
      agentCount: 0,
      avgLeadByAgent: 0,
      avgLeadByAgentDaily: 0,
      avgFtdByAgent: 0,
      avgFtdByAgentDaily: 0,
      agentAvgFtdPerWorkedMonth: 0,
      avgFtdByDeskLongTerm: 0,
      ftdBenchmarkRate: 0,
    };
  }
  const agentField = getFieldName(tabConfig, "agentNames");
  ...
  for (const row of rows) {
    const normalizedAgent = normalizeAgentName(getRowValue(row, agentField));
    if (normalizedAgent && needsAnyExtraMetric) {
      uniqueAgents.add(normalizedAgent);
      if (needsAgentWorkedMonth) {
    ...
    if (needsLeadDaily || needsFtdDaily || needsAgentWorkedMonth) {
      const parsedDate = parseDateValue(getRowValue(row, leadDateField) || getRowValue(row, createdField));
      if (parsedDate) {
        const year = String(parsedDate.getUTCFullYear());
        const month = String(parsedDate.getUTCMonth() + 1).padStart(2, "0");
        const day = String(parsedDate.getUTCDate()).padStart(2, "0");
        if (needsLeadDaily || needsFtdDaily) {
          uniqueDates.add(`${year}-${month}-${day}`);
        }
        if (needsAgentWorkedMonth && normalizedAgent && monthKeysByAgent.has(normalizedAgent)) {
  ...
  const totalLeads = Number(summary.totalLeads || 0);
  const totalFtd = Number(summary.totalFtd || 0);
  const avgLeadByAgent = needsAvgLeadByAgent && agentCount > 0 ? totalLeads / agentCount : 0;
  ...
  const requiresDeskBenchmarkValues = needsDeskAverage || needsLongTermAverage || needsAgentWorkedMonth;
  const resolvedDesk = requiresDeskBenchmarkValues ? deskFromValues || deskFromRows : "";
  const resolvedCountry = requiresDeskBenchmarkValues ? inferCountryForBuilderScope(rowValues, rows, tabConfig) : "";
  const benchmarkKey = requiresDeskBenchmarkValues
    ? resolveDeskLanguageBenchmarkKey({
        desk: resolvedDesk,
  ...
  const deskAverage =
    needsDeskAverage
      ? deskFtdAverages.get(benchmarkKey)?.avgFtdByAgent ?? (agentCount > 0 ? totalFtd / agentCount : 0)
      : 0;
  const deskLongTermAverage =
    needsLongTermAverage ? deskFtdAverages.get(benchmarkKey)?.avgFtdByLongTerm?.[workProfile.longTermBucket] || 0 : 0;
  ...
  const resolvedAgentFtd =
    uniqueAgent && ftdByAgent.has(uniqueAgent)
      ...
      : uniqueAgents.size === 1
        ? totalFtd
        : 0;
  const agentAvgFtdByMonth =
    needsAgentWorkedMonth && uniqueAgent && agentActiveMonthCount > 0 ? resolvedAgentFtd / agentActiveMonthCount : 0;
  const benchmarkRate =
    requestedExtraMetricKeys.has("ftdBenchmarkRate") && deskLongTermAverage > 0
      ? (agentAvgFtdByMonth / deskLongTermAverage) * 100
      : 0;
  const dayCount = uniqueDates.size;
  return {
    agentCount: needsAgentCount ? agentCount : 0,
    avgLeadByAgent,
    avgLeadByAgentDaily: needsLeadDaily && dayCount > 0 ? avgLeadByAgent / dayCount : 0,
    avgFtdByAgent: needsDeskAverage ? deskAverage : 0,
    avgFtdByAgentDaily: needsFtdDaily && dayCount > 0 ? deskAverage / dayCount : 0,
    agentAvgFtdPerWorkedMonth: needsAgentWorkedMonth ? agentAvgFtdByMonth : 0,
    avgFtdByDeskLongTerm: needsLongTermAverage ? deskLongTermAverage : 0,
...
function specificBuilderTable(rows, tabConfig, infoContext, monthFilter, query = {}, now = new Date(), options = {}) {
  ...
  const selectedExtraMetricKeys = new Set(
    metrics.filter((metric) => BUILDER_EXTRA_METRIC_KEYS.has(metric.key)).map((metric) => metric.key),
  );
  const needsBuilderExtraMetrics = selectedExtraMetricKeys.size > 0;
  const needsDeskFtdAverages = [...selectedExtraMetricKeys].some((key) => DESK_BENCHMARK_EXTRA_METRIC_KEYS.has(key));
  ...
  const deskFtdAverages = needsDeskFtdAverages
    ? buildDeskFtdAverages(rows, tabConfig, infoContext, now, deskLanguageMap)
    : new Map();
          ...
          const metricValues = {
            ...
            ...(needsBuilderExtraMetrics
              ? builderExtraMetricValues(bucketRows, tabConfig, summary, {
                  ...
                  deskLanguageMap,
                  now,
                  requestedExtraMetricKeys: selectedExtraMetricKeys,
                })
              : {}),
          };
  ...
  const detailRows = groupedEntries
    .map((entry) => {
...
function buildLast4AgentMatrix(monthData = [], tabConfig, query = {}, now = new Date(), options = {}) {
  ...
  const filteredRowsByMonthKey =
    options.filteredRowsByMonthKey instanceof Map ? options.filteredRowsByMonthKey : null;
  ...
  for (const item of monthData) {
    ...
    const filteredRows = filteredRowsByMonthKey?.get(monthKey) || applyDashboardFilters(item.rows, tabConfig, query, now);
...
export async function loadDashboardReport(accessContext, query = {}, options = {}) {
  ...
  const assertWithinBudget = (stage) => {
    if (Date.now() - reportStartedAt > REPORT_MAX_SAFE_DURATION_MS) {
      throw reportTooHeavyError(
  ...
  const requestedLast4QuickMode = ["1", "true", "yes", "on"].includes(normalizeText(query.last4QuickMode));
  if (reportMode === "specific" && requestedLast4QuickMode && selectedMonthKeys.length > 4) {
    selectedMonthKeys = selectedMonthKeys.slice(0, 4);
  }
  logDashboardTiming("REPORT_REQUEST", {
    reportMode,
    selectedMonths: selectedMonthKeys,
    selectedOffice: selectedOfficeScopes.join(","),
  });
  ...
  if (reportMode === "last4") {
    ...
    const last4MonthKeys = monthsForMode.map((item) => String(item?.key || "")).filter(Boolean);
    logDashboardTiming("GOOGLE_SHEETS_LOAD_START", {
      reportMode,
      selectedMonths: last4MonthKeys,
      selectedOffice: primaryOfficeScope,
    });
    ...
    const last4RowsLoaded = monthData.reduce((sum, item) => sum + Number(item?.rows?.length || 0), 0);
    logDashboardTiming("GOOGLE_SHEETS_LOAD_END", {
      reportMode,
      selectedMonths: last4MonthKeys,
      selectedOffice: primaryOfficeScope,
      rowsLoaded: last4RowsLoaded,
    });
    const filteredRowsByMonthKey = new Map(
      monthData.map((item) => [String(item?.monthRecord?.key || ""), applyDashboardFilters(item.rows, tabConfig, query, now)]),
    );
    ...
    const rowsByMonth = monthData.map(({ monthRecord: itemMonth, rows, infoContext }) => {
      const filteredMonthRows = filteredRowsByMonthKey.get(String(itemMonth?.key || "")) || applyDashboardFilters(rows, tabConfig, query, now);
    ...
    logDashboardTiming("TOTAL_EXECUTION_TIME", {
      reportMode,
      selectedMonths: last4MonthKeys,
      selectedOffice: primaryOfficeScope,
      elapsedMs: Date.now() - reportStartedAt,
  ...
  const selectedMonthRecordsRaw = selectedOfficeScopes
    .flatMap((office) => {
  ...
  const selectedMonthRecordKeys = new Set();
  for (const record of selectedMonthRecordsRaw) {
    const office = String(record?.office_name || primaryOfficeScope || "").trim();
    const monthKey = String(record?.key || "").trim();
    if (!office || !monthKey) {
      continue;
    }
    const uniqueKey = `${office}::${monthKey}`;
    if (selectedMonthRecordKeys.has(uniqueKey)) {
      continue;
    }
    selectedMonthRecordKeys.add(uniqueKey);
    selectedMonthRecords.push(record);
  }
  if (!selectedMonthRecords.length) {
    throw new Error("No active month mapping found for selected office/month.");
  }
  const isBenchmarkRequest = reportMode === "specific" && ["1", "true", "yes", "on"].includes(normalizeText(query.benchmarkMode));
  if (
    isBenchmarkRequest &&
    selectedMonthRecords.length > BENCHMARK_MAX_UNFILTERED_MONTHS &&
    !hasNarrowBenchmarkFilters(query)
  ) {
    throw reportTooHeavyError(
      `Benchmark report is too large to load without filters. Select up to ${BENCHMARK_MAX_UNFILTERED_MONTHS} months or narrow Desk/Team Leader/Agent/Date/Country/Campaign filters.`,
      {
        stage: "Preparing report",
        selectedMonthCount: selectedMonthRecords.length,
      },
    );
  }
  ...
  logDashboardTiming("GOOGLE_SHEETS_LOAD_START", {
    reportMode,
    selectedMonths: selectedMonthRecords.map((record) => String(record?.key || "")).filter(Boolean),
    selectedOffice: selectedMonthOffices.join(","),
  });
  ...
  const selectedRowsLoaded = selectedMonthData.reduce((sum, item) => sum + Number(item?.rows?.length || 0), 0);
  logDashboardTiming("GOOGLE_SHEETS_LOAD_END", {
    reportMode,
    selectedMonths: selectedMonthRecords.map((record) => String(record?.key || "")).filter(Boolean),
    selectedOffice: selectedMonthOffices.join(","),
    rowsLoaded: selectedRowsLoaded,
  });
  ...
  if (reportMode === "specific") {
    const isLast4QuickMode = ["1", "true", "yes", "on"].includes(normalizeText(query.last4QuickMode));
    const isAgentProductivityPlanMode = ["1", "true", "yes", "on"].includes(normalizeText(query.agentProductivityPlanMode));
    const isBenchmarkMode = ["1", "true", "yes", "on"].includes(normalizeText(query.benchmarkMode));
    const shouldApplyHeavyGuard = !isLast4QuickMode && !isAgentProductivityPlanMode && !isBenchmarkMode;
    const selectedDimensions = selectedSpecificDimensions(query);
    const selectedMetrics = selectedSpecificMetrics(query);
    const complexityScore =
      modeRowsCombined.length * Math.max(1, selectedDimensions.length) * Math.max(1, selectedMetrics.length);
    if (
      shouldApplyHeavyGuard &&
      (modeRowsCombined.length > REPORT_TOO_HEAVY_ROW_LIMIT || complexityScore > REPORT_TOO_HEAVY_COMPLEXITY_LIMIT)
    ) {
      throw reportTooHeavyError(
        "This report is too heavy for one request. Please narrow Date/Country/Campaign filters and retry.",
        {
          stage: "Preparing report",
  ...
  let builder = null;
  let deskLanguageMap = new Map();

  if (reportMode === "specific") {
    const needsDeskLanguageMap = selectedSpecificMetrics(query).some((metricKey) =>
      DESK_BENCHMARK_EXTRA_METRIC_KEYS.has(metricKey),
    );
    if (needsDeskLanguageMap) {
      deskLanguageMap = await readOfficeDeskLanguageMap();
    }
    const filteredRowsByMonthKey = new Map();
    ...
    const summaryTargetFromMonths = selectedMonthData.reduce((sum, item) => {
      ...
      const monthTargetAggregation = targetAggregationForScope({
        rows: monthRows,
        tabConfig,
        infoContext: item?.infoContext || combinedInfoContext,
        filters: monthFilter ? { date: monthFilter } : {},
        scope: {
      ...
      return sum + Number(monthTargetAggregation?.includedTarget || 0);
    }, 0);
    summary = {
      ...summary,
    ...
    builder = specificBuilderTable(modeRowsCombined, tabConfig, combinedInfoContext, combinedMonthFilter, query, now, {
      ...
      shouldAbort: () => Date.now() - reportStartedAt > REPORT_MAX_SAFE_DURATION_MS,
      onProgress: (info = {}) => {
        emitProgress(info.step || "Building Table", info.progress || 60, {
          rowsProcessed: Number(info.rowsProcessed || 0),
        });
      },
    });
    table = builder.table;
    ...
    assertWithinBudget("specific_builder_built");
  ...
  const useFastOptions =
    reportMode === "specific" || modeRowsCombined.length > FAST_OPTIONS_ROW_THRESHOLD || selectedMonthRecords.length > 2;
