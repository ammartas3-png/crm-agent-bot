...
async function readMonthData({
  ...
  const rosterRemappedAllLeads = remapLeadRowsByRosterProfile(rowsWithScope, tabConfig, rosterProfileMap);
  const permissionRowsAliased = remapLeadRowsByRosterProfile(permissionRowsRaw, tabConfig, rosterProfileMap);
  const leadsProfileMap = buildAgentProfileMapFromLeadsRows(rosterRemappedAllLeads, tabConfig);
  const kycFtdRowsBuilt = includeKycFtd
    ? buildKycFtdRowsFromFtdSheet(ftdRowsRaw, ftdTabConfig, tabConfig, rosterProfileMap, {
        ...
        leadsProfileMap,
        monthKey: monthRecord.key || monthRecord.period || "",
      })
    : [];
...
function unionKycFtdRows(primary = [], secondary = []) {
...
export function filterKycFtdRowsForPermission(kycFtdRows = [], tabConfig, permissionFilters = {}, allowedAgents = new Set()) {
  ...
  const agentMatched = allowedAgents.size
    ? kycFtdRows.filter((row) => {
        const normalized = normalizeAgentName(getRowValue(row, agentField));
        const raw = normalizeAgentName(row?.__rawFtdAgent || "");
        return allowedAgents.has(normalized) || allowedAgents.has(raw);
      })
    : [];
  if (!permissionFiltered.length) {
    return agentMatched;
  }
  if (!agentMatched.length) {
    return permissionFiltered;
  }
  return unionKycFtdRows(permissionFiltered, agentMatched);
}

function buildTeamLeaderDeskMap(leadsProfileMap = new Map()) {
  const map = new Map();
  for (const profile of leadsProfileMap.values()) {
    const teamLeader = normalizeText(profile?.teamLeader || "");
    const desk = cleanSpreadsheetText(profile?.desk || "");
    if (teamLeader && desk && !map.has(teamLeader)) {
      map.set(teamLeader, desk);
    }
  }
  return map;
}

function isTruthyFtdCheckerValue(value = "") {
  const text = String(value ?? "").trim();
  if (!text) {
    return true;
  }
  if (text === "0" || /^no|false$/i.test(text)) {
    return false;
  }
  return true;
}
...
function resolveFtdSheetValue(row = {}, tabConfig = {}, fieldKey = "", fallbacks = []) {
  ...
  for (const key of Object.keys(row || {})) {
    ...
    if (
      ...
      (normalizedKey === "cid" || normalizedKey === "customer id" || normalizedKey === "id" || normalizedKey === "accid")
    ) {
      const value = String(row[key] || "").trim();
      if (value) {
        return value;
    ...
    if (
      ...
      (normalizedKey.includes("country") || normalizedKey === "list of countrys")
    ) {
      const value = String(row[key] || "").trim();
      if (value) {
        return value;
    ...
    if (normalizedFieldKey === "team" && (normalizedKey === "team" || normalizedKey === "team leader")) {
      const value = String(row[key] || "").trim();
      if (value) {
        return value;
      }
    }
    if (normalizedFieldKey === "brand" && normalizedKey === "brand") {
      const value = String(row[key] || "").trim();
      if (value) {
        return value;
      }
    ...
    if (
      normalizedFieldKey === "affiliate" &&
      (normalizedKey === "aff" || normalizedKey === "affiliate" || normalizedKey === "campaign")
    ) {
      const value = String(row[key] || "").trim();
      if (value) {
        return value;
    ...
    if (
      normalizedFieldKey === "checker" &&
      (normalizedKey === "cheker" || normalizedKey === "checker" || normalizedKey === "check")
    ) {
      const value = String(row[key] || "").trim();
      if (value) {
        return value;
...
async function readFtdRowsForKyc(ftdTabConfig, monthScope = {}) {
  try {
    return await readSheetRows("ftd", {
...
export function buildKycFtdRowsFromFtdSheet(ftdRows = [], ftdTabConfig = {}, tabConfig = {}, rosterProfileMap = new Map(), options = {}) {
  ...
  const teamLeaderDeskMap = buildTeamLeaderDeskMap(leadsProfileMap);
  const countryAliases = ["LIST OF COUNTRYS", "LIST OF COUNTRY'S", "LIST OF COUNTRIES", "Country"];
  const output = [];
  let rowSequence = 0;
  for (const row of ftdRows || []) {
    const rowValues = Object.values(row || {});
    const checkerRaw = resolveFtdSheetValue(row, ftdTabConfig, "checker", ["Cheker", "Checker"]);
    if (!isTruthyFtdCheckerValue(checkerRaw)) {
      continue;
    }
    ...
    const teamLeader =
      profile?.teamLeader ||
      cleanSpreadsheetText(resolveFtdSheetValue(row, ftdTabConfig, "team", ["TEAM", "Team Leader", "Team"]) || String(rowValues[6] || ""));
    const desk =
      profile?.desk ||
      teamLeaderDeskMap.get(normalizeText(teamLeader)) ||
      "";
    const customerId = normalizeCustomerIdToken(
      resolveFtdSheetValue(row, ftdTabConfig, "customerId", ["CID", "Customer ID", "ID", "ACC ID"]) ||
        String(rowValues[1] || ""),
    );
    const country = cleanSpreadsheetText(
      resolveFtdSheetValue(row, ftdTabConfig, "country", countryAliases) || String(rowValues[2] || ""),
    );
    const brand = cleanSpreadsheetText(
      resolveFtdSheetValue(row, ftdTabConfig, "brand", ["BRAND", "Brand"]) || String(rowValues[7] || ""),
    );
    const campaign = cleanSpreadsheetText(
      resolveFtdSheetValue(row, ftdTabConfig, "affiliate", ["AFF", "Affiliate", "Campaign"]) ||
        String(rowValues[4] || ""),
    );
    const registrationDateRaw =
      resolveFtdSheetValue(row, ftdTabConfig, "registrationDate", ["RegistrationDate", "Registration Date"]) ||
      String(rowValues[5] || "");
    const ftdDateRaw =
      resolveFtdSheetValue(row, ftdTabConfig, "date", ["FTD Date", "FTD Date Duplicate", "FTD Date 10", "Date"]) ||
      String(rowValues[0] || rowValues[9] || "");
    const parsedFtdDate = parseDateValue(ftdDateRaw);
    const ftdDateValue = parsedFtdDate ? ftdDateRaw : "";
    const monthKey =
      String(options.monthKey || "").trim() ||
      (parsedFtdDate
        ? `${parsedFtdDate.getUTCFullYear()}-${String(parsedFtdDate.getUTCMonth() + 1).padStart(2, "0")}`
        : "");
    rowSequence += 1;
    output.push({
      __kycFtd: 1,
      __kycFtdSource: "ftd-sheet",
      __kycFtdRowSeq: rowSequence,
...
function monthFilterKeyFromDateFilter(dateFilter = {}) {
  if (!dateFilter || typeof dateFilter !== "object" || dateFilter.type !== "month") {
    return "";
  }
...
function kycFtdRowMatchesDateFilter(row = {}, tabConfig, dateFilter = "", now = new Date()) {
  if (!dateFilter || typeof dateFilter !== "object" || !dateFilter.type) {
    return true;
  }
  if (row?.__kycFtdSource === "ftd-sheet") {
    const monthKey = monthFilterKeyFromDateFilter(dateFilter);
  ...
  if (!String(dateValue || "").trim() && row?.__kycFtdSource === "ftd-sheet") {
    return true;
  }
...
function kycFtdRowsForScope(kycFtdRows = [], tabConfig, dateFilter = "", scope = {}, now = new Date()) {
  ...
  if (dateFilter && typeof dateFilter === "object" && dateFilter.type) {
    filtered = filtered.filter((row) => kycFtdRowMatchesDateFilter(row, tabConfig, dateFilter, now));
  } else {
    const dateValue =
  ...
  if (Array.isArray(scope.agent) && scope.agent.length) {
    ...
    filtered = filtered.filter((row) => {
      const desk = normalizeText(getRowValue(row, deskField) || row?.__scopeOfficeName || "");
      return allowed.has(desk);
    });
  }
  return filtered;
}
...
export async function loadDashboardReport(accessContext, query = {}, options = {}) {
  ...
  const combinedKycFtdRows = selectedMonthData.flatMap((item) => item.kycFtdRows || []);
