...
export async function refreshOfficeDeskLanguageBenchmarks(options = {}) {
  // Benchmark source is now the existing Language sheet values.
  // Keep this API as a lightweight "refresh cache from sheet" action only.
  ...
  const benchmarkCache = languageSnapshot?.benchmarkCache instanceof Map ? languageSnapshot.benchmarkCache : new Map();
  return {
    officeCount: 0,
    monthCount: 0,
    rowCount: languageRows.length,
    updatedRows: benchmarkCache.size,
  };
}
...
function isAggregationFirstHierarchyReport(query = {}) {
  const reportMode = String(query.reportMode || "").trim().toLowerCase();
  const specificType = String(query.specificType || "builder").trim().toLowerCase();
  if (reportMode !== "specific" || specificType !== "builder") {
    return false;
  }
  ...
  const allowedDimensionKeys = new Set(["desk", "teamLeader", "agent"]);
  if (!dimensionKeys.every((key) => allowedDimensionKeys.has(key))) {
    return false;
  }
  const columnDimension = String(query.columnDimension || "").trim().toLowerCase();
  if (columnDimension && columnDimension !== "month") {
    return false;
  }
  return true;
}
...
function builderExtraMetricValues(rows = [], tabConfig, summary = {}, options = {}) {
  ...
  const deskLongTermAverage = cachedLongTermBenchmark > 0 ? cachedLongTermBenchmark : 0;
...
export async function loadDashboardReport(accessContext, query = {}, options = {}) {
  ...
  const allowLargeRawRowsAfterAggregation = isAggregationFirstHierarchyReport(query);
  ...
  const assertWithinBudget = (stage, context = {}) => {
    if (
      stage === "table_pagination" &&
      allowLargeRawRowsAfterAggregation &&
      Number(context.finalOutputRows || 0) > 0
    ) {
      return;
    }
  ...
  if (reportMode === "specific" && benchmarkMode && benchmarkHydrateRequested) {
    try {
      deskLanguageSnapshot = await readOfficeDeskLanguageSnapshot({ forceRefresh: true });
      deskBenchmarkCache =
        deskLanguageSnapshot?.benchmarkCache instanceof Map ? deskLanguageSnapshot.benchmarkCache : new Map();
    } catch {
      // Keep existing snapshot cache on read failures.
    }
  }
  const shouldLoadBenchmarkBaselineFallback = false;
  const shouldLoadUnscopedSelectedMonthData =
    shouldLoadBenchmarkBaselineFallback &&
    reportMode === "specific" &&
    benchmarkMode &&
  ...
  if (reportMode === "specific") {
    ...
    if (!allowLargeRawRowsAfterAggregation && complexityScore > 10_000_000) {
      throw reportTooHeavyError(
    ...
    builder = specificBuilderTable(modeRowsCombined, tabConfig, combinedInfoContext, combinedMonthFilter, query, now, {
      ...
      shouldAbort: (stage) =>
        !allowLargeRawRowsAfterAggregation && Date.now() - startedAt > REPORT_MAX_SAFE_DURATION_MS && stage !== "table_pagination",
      onProgress: (info = {}) => {
  ...
  assertWithinBudget("table_pagination", { finalOutputRows: totalRows });
