...
export function buildLeadSplitterReport(rows = [], tabConfig) {
  ...
  for (const row of Array.isArray(rows) ? rows : []) {
    ...
    if (!agentMap.has(agent)) {
      agentMap.set(agent, { leads: 0, ftd: 0 });
    }
  ...
  for (const desk of desks) {
    ...
    const countries = [...countryMap.entries()].map(([country, agentMap]) => {
      const agents = [...agentMap.entries()]
        .map(([agent, value]) => ({ agent, leads: value.leads, ftd: value.ftd, cr: crOf(value.ftd, value.leads) }))
        .sort((left, right) => right.leads - left.leads || String(left.agent).localeCompare(String(right.agent)));
      const leads = agents.reduce((sum, item) => sum + item.leads, 0);
      const ftd = agents.reduce((sum, item) => sum + item.ftd, 0);
      return { country, agents, leads, ftd };
    });
    countries.sort((left, right) => right.leads - left.leads || String(left.country).localeCompare(String(right.country)));
    for (const entry of countries) {
      ...
      outRows.push({
        ...
        leads: entry.leads,
        ftd: entry.ftd,
        cr: crOf(entry.ftd, entry.leads),
      });
      deskLeads += entry.leads;
      deskFtd += entry.ftd;
    }
    outRows.push({
      kind: "deskTotal",
      ...
      leads: deskLeads,
      ftd: deskFtd,
      cr: crOf(deskFtd, deskLeads),
    });
  }

  return {
    rows: outRows,
    summary: { leads: grandLeads, ftd: grandFtd, cr: crOf(grandFtd, grandLeads) },
  };
}
