...
function collectStatusByAgentFromRows(rows = [], tabConfig, allowedAgents = new Set()) {
  ...
  for (const row of rows) {
    ...
    const rawStatus = rawCandidates.find((value) => String(value ?? "").trim()) || "";
    let status = normalizeWorkingStatusValue(rawStatus);
    if (!status) {
      // Latest month leads table is the source of truth for current status.
      // Unknown/blank values are treated as not working.
      status = "not_working";
    }
...
async function readMonthData({ monthRecord, officeScope, tabConfig, infoAgentsTabConfig, permissionFilters }) {
  ...
  const statusByAgent = collectStatusByAgent(infoRows, infoAgentsTabConfig, allowedAgents);
  const leadStatusByAgent = collectStatusByAgentFromRows(permissionRows, tabConfig, allowedAgents);
  for (const [normalizedAgent, status] of leadStatusByAgent.entries()) {
    statusByAgent.set(normalizedAgent, status);
  }
...
function workTimeValuesFromRows(rows = [], tabConfig, infoContext, now = new Date(), options = {}) {
  ...
  if (!statusRaw && rowTeamLeader) {
    statusRaw =
      latestStatusMap?.get(rowTeamLeader) ||
      normalizeWorkingStatusValue(infoContext?.byAgent?.get(rowTeamLeader)?.working_status || "");
  }
  return {
