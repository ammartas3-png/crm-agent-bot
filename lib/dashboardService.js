...
function summaryWithTargets(rows, tabConfig, infoContext, dateFilter, now = new Date(), scope = {}) {
  ...
  return {
    ...summary,
    ...targetMetrics,
    kycFtd: kycFtdCountFromRows(rows, tabConfig, infoContext),
...
async function readMonthData({
  ...
  const legacyInfoRowsPromise = includeLegacyInfoTargets
    ...
    : Promise.resolve([]);
  const kycFtdRowsPromise = includeKycFtd
    ? (() => {
        notifyProgress("FTD");
  ...
  const [rosterRowsRaw, legacyInfoRows, ftdRowsForKyc] = await Promise.all([
    rosterRowsRawPromise,
    legacyInfoRowsPromise,
    kycFtdRowsPromise,
  ]);
  const kycFtdCountByAgent = includeKycFtd ? buildKycFtdCountByAgent(ftdRowsForKyc, ftdTabConfig) : new Map();
  ...
  infoContext.kycFtdCountByAgent = kycFtdCountByAgent;
    ...
    if (needsTargetAggregation) {
      if (isOnlyFtdTargetMetric) {
        ...
        return {
          totalLeads: 0,
          totalFtd: 0,
          kycFtd: kycFtdCountFromRows(subsetRows, tabConfig, activeInfoContext),
          ftdTarget: Number(targetAggregation?.includedTarget || 0),
          ftdTargetReach: 0,
          cr: 0,
          crTarget: 0,
...
export async function loadDashboardReport(accessContext, query = {}, options = {}) {
  ...
  attachKycFtdMapsToInfoContext(combinedInfoContext, selectedMonthData);
  ...
  const comparisonDetailRows = isComparisonReportQuery(query)
    ? buildComparisonDetailRows(modeRowsCombined, tabConfig, combinedMonthFilter, now)
    : null;
  const comparisonTables = comparisonDetailRows
    ? buildComparisonTablesFromDetailRows(comparisonDetailRows, parseComparisonSelections(query.comparisonSelections))
    : isComparisonReportQuery(query)
      ? buildComparisonTablesFromRows(
          modeRowsCombined,
          tabConfig,
          combinedMonthFilter,
          now,
          parseComparisonSelections(query.comparisonSelections),
        )
      : null;
