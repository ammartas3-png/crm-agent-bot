...
const OFFICE_DESK_LANGUAGE_COLUMNS = ["Desk", "Lang", "Benchmark"];
...
function officeDeskLanguageTabConfig() {
  ...
  return {
    ...
    name: safeTabName,
    range: `'${safeTabName.replace(/'/g, "''")}'!A:C`,
...
function selectDeskLongTermBenchmarkValue(entry = {}) {
  const moreThanTwoMonths = Number(entry?.avgFtdByLongTerm?.[LONG_TERM_BUCKET_MORE_THAN_2] || 0);
  const lessThanTwoMonths = Number(entry?.avgFtdByLongTerm?.[LONG_TERM_BUCKET_LESS_THAN_2] || 0);
  const fallbackAverage = Number(entry?.avgFtdByAgent || 0);
  if (moreThanTwoMonths > 0) {
    return moreThanTwoMonths;
  }
  if (lessThanTwoMonths > 0) {
    return lessThanTwoMonths;
  }
  return fallbackAverage > 0 ? fallbackAverage : 0;
}

function buildDeskBenchmarkCache(rows = [], deskLanguageMap = new Map()) {
  const map = new Map();
  for (const row of rows || []) {
    const benchmarkValue = parseMetricNumber(row?.Benchmark);
    ...
    map.set(benchmarkKey, benchmarkValue);
  }
  return map;
}
...
export async function refreshOfficeDeskLanguageBenchmarks(options = {}) {
  ...
  await Promise.all(
    offices.map(async (office) => {
      rosterRowsByOffice.set(office, await readOfficeAgentRosterRows(office));
    }),
  );
  ...
  for (const office of offices) {
    ...
    for (const record of officeRecords) {
      ...
      const uniqueKey = `${normalizeText(office)}::${monthKey}`;
  ...
  const monthRecords = [...uniqueMonthRecords.values()];
  ...
  const monthData = await Promise.all(
    monthRecords.map((record) =>
      readMonthData({
        ...
        skipPermissionFilters: true,
      }),
    ),
  );
  const allRows = monthData.flatMap((item) => item.rows || []);
  const mergedInfoContext = mergeInfoContexts(monthData.map((item) => item.infoContext));
  ...
  const outputRows = languageRows.map((row) => {
    ...
    const benchmarkValue = benchmarkValueForCell(selectDeskLongTermBenchmarkValue(benchmarkEntry || {}));
    return [desk, language, benchmarkValue];
  });
  await updateSheetValues({
    spreadsheetId: OFFICE_AGENT_ROSTER_SPREADSHEET_ID,
    range: `'Language'!A:C`,
    values: [["Desk", "Lang", "Benchmark"], ...outputRows],
...
async function readMonthData({
  ...
  const rosterRowsRaw = Array.isArray(officeRosterRows)
    ? officeRosterRows
    : await readOfficeAgentRosterRows(officeScope || monthRecord.office_name || "");
...
function builderExtraMetricValues(rows = [], tabConfig, summary = {}, options = {}) {
  ...
  const cachedLongTermBenchmark = Number(deskBenchmarkCache.get(benchmarkKey) || 0);
...
export async function loadDashboardReport(accessContext, query = {}, options = {}) {
  ...
  const shouldHydrateBenchmarkCacheNow = reportMode === "specific" && benchmarkMode && deskBenchmarkCache.size === 0;
  ...
  if (reportMode === "specific" && benchmarkMode && hasPermissionRestrictions && deskBenchmarkCache.size === 0) {
    ...
    const benchmarkDataByKey = new Map();
    const buildBenchmarkRecordKey = (office, monthKey) =>
      `${normalizeText(office)}::${String(monthKey || "").trim()}`;
    selectedMonthDataRaw.forEach((item, index) => {
      const sourceRecord = selectedMonthRecords[index] || {};
      const office = sourceRecord.office_name || item?.monthRecord?.office_name || primaryOfficeScope;
      const monthKey = sourceRecord.key || item?.monthRecord?.key || "";
      const recordKey = buildBenchmarkRecordKey(office, monthKey);
      if (recordKey.endsWith("::")) {
        return;
      }
      benchmarkDataByKey.set(recordKey, item);
    });
    const benchmarkMonthRecordMap = new Map();
    benchmarkOfficeScopes
      .flatMap((office) => {
        const officeRecords = collectOfficeScopedMonthRecords(officeMap, office)
          .filter((record) => record.active !== false)
          .sort((left, right) => String(right.key || "").localeCompare(String(left.key || "")));
      ...
      .filter(Boolean)
      .forEach((record) => {
        const recordKey = buildBenchmarkRecordKey(record.office_name || primaryOfficeScope, record.key);
        if (!recordKey.endsWith("::") && !benchmarkMonthRecordMap.has(recordKey)) {
          benchmarkMonthRecordMap.set(recordKey, record);
        }
      });
    const benchmarkMonthRecords = [...benchmarkMonthRecordMap.values()];
    if (benchmarkMonthRecords.length) {
      const missingBenchmarkRecords = benchmarkMonthRecords.filter((record) => {
        const recordKey = buildBenchmarkRecordKey(record.office_name || primaryOfficeScope, record.key);
        return !benchmarkDataByKey.has(recordKey);
      });
      if (missingBenchmarkRecords.length) {
        const missingOffices = [...new Set(missingBenchmarkRecords.map((record) => record.office_name).filter(Boolean))];
        await Promise.all(missingOffices.map((office) => resolveOfficeRosterRows(office)));
        const fetchedMissingBenchmarkData = await Promise.all(
          missingBenchmarkRecords.map((record) =>
            readMonthData({
              monthRecord: record,
              officeScope: record.office_name || primaryOfficeScope,
              tabConfig,
              infoAgentsTabConfig,
              permissionFilters,
              officeRosterRows: rosterRowsByOffice.get(record.office_name || primaryOfficeScope) || [],
              skipPermissionFilters: true,
            }),
          ),
        );
        fetchedMissingBenchmarkData.forEach((item, index) => {
          const sourceRecord = missingBenchmarkRecords[index] || {};
          const office = sourceRecord.office_name || item?.monthRecord?.office_name || primaryOfficeScope;
          const monthKey = sourceRecord.key || item?.monthRecord?.key || "";
          const recordKey = buildBenchmarkRecordKey(office, monthKey);
          if (!recordKey.endsWith("::")) {
      ...
      const benchmarkMonthData = benchmarkMonthRecords
        .map((record) => {
          const recordKey = buildBenchmarkRecordKey(record.office_name || primaryOfficeScope, record.key);
          return benchmarkDataByKey.get(recordKey) || null;
        })
        .filter(Boolean);
      benchmarkRowsOverride = benchmarkMonthData.flatMap((item) => item.rows || []);
      benchmarkInfoContextOverride = mergeInfoContexts(benchmarkMonthData.map((item) => item.infoContext));
      benchmarkInfoContextOverride.latestStatusByAgent = deriveLatestStatusByAgent(benchmarkMonthData);
    }
  }
