...
function oldNameAliasMapFromRoster(rows = []) {
  ...
  for (const row of rows) {
    ...
    if (!canonicalAgent || !oldName) {
      continue;
    }
    const normalizedCanonical = normalizeAgentName(canonicalAgent);
    const normalizedOld = normalizeAgentName(oldName);
    if (!normalizedCanonical || !normalizedOld) {
      continue;
    }
    if (isExcludedNormalizedAgent(normalizedCanonical) || isExcludedNormalizedAgent(normalizedOld)) {
      continue;
    }
    aliasMap.set(normalizedOld, canonicalAgent);
  }
  return aliasMap;
}

function remapLeadAgentNamesByAlias(rows = [], tabConfig, aliasMap = new Map()) {
  if (!aliasMap || typeof aliasMap.get !== "function" || aliasMap.size === 0) {
    return rows;
  }
  const agentField = getFieldName(tabConfig, "agentNames");
  return rows.map((row) => {
    ...
    const mappedAgent = aliasMap.get(normalizeAgentName(rawAgent));
    if (!mappedAgent || mappedAgent === rawAgent) {
      return row;
    }
    return {
      ...row,
      [agentField]: mappedAgent,
    };
  });
}
...
async function readMonthData({ monthRecord, officeScope, tabConfig, infoAgentsTabConfig, permissionFilters }) {
  ...
  const oldNameAliasMap = oldNameAliasMapFromRoster(rosterRowsRaw);
  const permissionRowsRaw = filterRowsByPermission(rowsWithScope, tabConfig, permissionFilters || {});
  const permissionRowsAliased = remapLeadAgentNamesByAlias(permissionRowsRaw, tabConfig, oldNameAliasMap);
