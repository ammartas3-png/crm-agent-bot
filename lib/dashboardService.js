...
function longTermBucketFromText(value = "") {
  const normalized = normalizeText(value);
  if (normalized.includes("less than 2")) {
    return LONG_TERM_BUCKET_LESS_THAN_2;
  }
  if (normalized.includes("more than 2")) {
    return LONG_TERM_BUCKET_MORE_THAN_2;
  }
  return LONG_TERM_BUCKET_UNKNOWN;
}
...
function builderExtraMetricValues(rows = [], tabConfig, summary = {}, options = {}) {
  ...
  const workProfile = workProfileFromStartDateRaw(startDateRaw, now, endDateRaw);
  const deskStats = deskFtdAverages.get(normalizedDesk) || null;
  const deskAverage = deskStats?.avgFtdByAgent ?? (agentCount > 0 ? totalFtd / agentCount : 0);
  const resolvedLongTermBucket =
    workProfile.longTermBucket !== LONG_TERM_BUCKET_UNKNOWN
      ? workProfile.longTermBucket
      : longTermBucketFromText(rowValues.workLongTerm || "");
  const deskLongTermAverage = deskStats?.avgFtdByLongTerm?.[resolvedLongTermBucket] || 0;
...
function specificBuilderTable(rows, tabConfig, infoContext, monthFilter, query = {}, now = new Date(), options = {}) {
  ...
  const deskFtdAverages = needsBuilderExtraMetrics ? buildDeskFtdAverages(rows, tabConfig, infoContext, now) : new Map();
