...
const OFFICE_AGENT_ROSTER_COLUMNS = ["Agent", "Working Status", "Desk", "Team Leader", "Starting Date", "Old Name"];
...
function officeAgentRosterTabConfig(tabName = "") {
  ...
  return {
    ...
    name: safeTabName,
    range: `'${safeTabName.replace(/'/g, "''")}'!A:F`,
...
function specificBuilderTable(rows, tabConfig, infoContext, monthFilter, query = {}, now = new Date(), options = {}) {
  ...
  const rowOnlyTargetForScope = (subsetRows, scopeValues = {}) => {
    const aggregation = targetAggregationForScope({
      rows: subsetRows,
      tabConfig,
      infoContext: null,
    ...
    return Number(aggregation?.includedTarget || 0);
  };
  ...
  if (columnDimension) {
    ...
    const tableRows = [...grouped.values()]
      .map((entry) => {
        ...
        let monthTargetByColumn = null;
        if (columnDimension === "month" && needsTargetAggregation) {
          monthTargetByColumn = new Map();
          for (const monthKey of orderedColumnValues) {
            const monthRows = entry.byColumn.get(monthKey) || [];
            const monthTarget = rowOnlyTargetForScope(monthRows, entry.values);
            monthTargetByColumn.set(monthKey, monthTarget);
          }
        }
        for (const columnValue of columnValuesWithGrandTotal) {
          const bucketRows =
            columnValue === COLUMN_GRAND_TOTAL_KEY
              ? [...entry.byColumn.values()].flat()
          ...
          const bucketInfoContext =
            columnDimension === "month" && columnValue !== COLUMN_GRAND_TOTAL_KEY
              ...
              : infoContext;
          const summary = scopedSummary(bucketRows, entry.values, {}, bucketInfoContext);
          const metricValues = {
          ...
          if (columnDimension === "month" && monthTargetByColumn) {
            if (columnValue === COLUMN_GRAND_TOTAL_KEY) {
              const grandTarget = [...monthTargetByColumn.values()].reduce(
                (sum, targetValue) => sum + Number(targetValue || 0),
                0,
              );
              metricValues.ftdTarget = grandTarget;
              metricValues.ftdTargetReach = targetReachPercent(Number(metricValues.ftd || 0), grandTarget);
            } else if (monthTargetByColumn.has(columnValue)) {
              const monthTarget = Number(monthTargetByColumn.get(columnValue) || 0);
              metricValues.ftdTarget = monthTarget;
              metricValues.ftdTargetReach = targetReachPercent(Number(metricValues.ftd || 0), monthTarget);
            }
          }
...
export async function loadDashboardReport(accessContext, query = {}, options = {}) {
  ...
  if (reportMode === "specific") {
    const rowOnlyTargetAggregation = targetAggregationForScope({
      rows: modeRowsCombined,
      tabConfig,
      infoContext: null,
      filters: {},
      scope: targetScopeFromRows(modeRowsCombined, tabConfig),
      now,
    });
    const rowOnlyTarget = Number(rowOnlyTargetAggregation?.includedTarget || 0);
    summary = {
      ...summary,
      ftdTarget: rowOnlyTarget,
      ftdTargetReach: targetReachPercent(summary.totalFtd, rowOnlyTarget),
    };
