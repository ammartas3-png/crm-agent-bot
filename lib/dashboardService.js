...
const DEFAULT_SPECIFIC_METRICS = ["leads", "ftd", "ftdTarget", "ftdTargetReach", "cr", "crTarget", "crTargetReach"];
...
function selectedSpecificDimensions(query = {}) {
  ...
  const groupByKey = GROUP_BY_TO_DIMENSION[String(query.groupBy || "").trim()];
  if (groupByKey && SPECIFIC_DIMENSION_BY_KEY.has(groupByKey)) {
    return uniqueSelection([groupByKey, ...selected]);
  }
...
function specificBuilderTable(rows, tabConfig, infoContext, monthFilter, query = {}, now = new Date()) {
  ...
  for (const row of rows) {
    ...
    const key = dimensions.map((dimension) => normalizeText(values[dimension.key])).join("::");
    if (!grouped.has(key)) {
  ...
  const tableRows = [...grouped.values()]
    ...
    .sort((left, right) => {
      for (const dimension of dimensions) {
        const leftValue = String(left[dimension.key] || "");
        const rightValue = String(right[dimension.key] || "");
