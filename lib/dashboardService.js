...
async function loadDashboardReportUncached(accessContext, query = {}, options = {}) {
  ...
  if (isTrafficPriorityMode) {
    ...
    builder = specificBuilderTable(modeRowsCombined, tabConfig, combinedInfoContext, combinedMonthFilter, query, now, {
      ...
      deskBenchmarkCache,
      useOfficeSheetBenchmarksOnly,
      // Collapse an agent who worked under two teams in the same month into
      // their most-recent team. Otherwise the agent shows on two rows and their
      // agent-scoped metrics (KYC FTD, FTD Target) are repeated in full on each,
      // so the column/desk sums no longer match the deduped Grand Total. Applies
      // to Target Result, multi-month, and the standard agent report.
      useLatestAgentHierarchy: isTargetResultMode || selectedMonthKeys.length > 1 || isStandardAgentReport,
