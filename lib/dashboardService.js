...
function specificBuilderTable(rows, tabConfig, infoContext, monthFilter, query = {}, now = new Date()) {
  ...
  const orderedDimensions =
    groupByDimension && selectedDimensionByKey.has(groupByDimension)
  ...
  const mappedRows = [...grouped.values()]
    .map((entry) => {
  ...
  const totalLeadBase = mappedRows.reduce((sum, entry) => sum + Number(entry.leadBase || 0), 0);

  const tableRows = mappedRows
    .map((entry) => {
      if (includeLeadShare) {
        entry.payload.leadShare = totalLeadBase > 0 ? (Number(entry.leadBase || 0) / totalLeadBase) * 100 : 0;
      }
