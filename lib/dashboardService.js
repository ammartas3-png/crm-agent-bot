...
const OFFICE_DESK_LANGUAGE_COLUMNS = ["Desk", "Lang", "LESS THAN 2 MONTHS", "MORE THAN MONTHS"];
...
function officeDeskLanguageTabConfig() {
  ...
  return {
    ...
    fields: {
      ...
      moreThanMonths: "MORE THAN MONTHS",
    },
  };
}
...
function buildDeskBenchmarkCache(rows = [], deskLanguageMap = new Map()) {
  ...
  for (const row of rows || []) {
    const lessThanTwoMonths = parseMetricNumber(
      row?.["LESS THAN 2 MONTHS"] ?? row?.lessThanTwoMonths ?? row?.["Less Than 2 Months"] ?? "",
    );
    const moreThanTwoMonths = parseMetricNumber(
      row?.["MORE THAN MONTHS"] ?? row?.moreThanMonths ?? row?.["More Than Months"] ?? "",
    );
...
export async function refreshOfficeDeskLanguageBenchmarks(options = {}) {
  ...
  await updateSheetValues({
    spreadsheetId: OFFICE_AGENT_ROSTER_SPREADSHEET_ID,
    range: `'Language'!A:D`,
    values: [["Desk", "Lang", "LESS THAN 2 MONTHS", "MORE THAN MONTHS"], ...outputRows],
