...
function specificBuilderTable(rows, tabConfig, infoContext, monthFilter, query = {}, now = new Date(), options = {}) {
  ...
  if (columnDimension) {
    ...
    const columnValuesWithGrandTotal = orderedColumnValues.length
...
export async function loadDashboardReport(accessContext, query = {}, options = {}) {
  ...
  if (reportMode === "last4") {
    ...
    const useFastOptionsForLast4 = modeRows.length > FAST_OPTIONS_ROW_THRESHOLD;
    const reportPayload = {
      reportMode: "last4",
      specificType: "",
  ...
  const useFastOptions =
    modeRowsCombined.length > FAST_OPTIONS_ROW_THRESHOLD || (reportMode === "specific" && selectedMonthRecords.length > 2);
