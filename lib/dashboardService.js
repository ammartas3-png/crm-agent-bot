...
import { currentMonthKey, getMonthFile, listMonthFiles, monthFilterFromKey, filterReportMonthRecords } from "./monthlyReports.js";
...
function allowByRole(telegramUser, authorityScope) {
  ...
  return Boolean(authorityScope?.canUseDashboard);
}
...
function collectOfficeScopedMonthRecords(officeMap, officeScope = "") {
  ...
  const matchedOffice = Object.keys(officeMap?.byOffice || {}).find((name) => {
    ...
    return normalizedName === normalizedScope || officeNamesEquivalent(name, officeScope);
  });
...
function resolveMonthRecord(monthKey, officeScope, officeMap, now = new Date()) {
  ...
  if (scopedRecords.length) {
    const explicitKeys = new Set(scopedRecords.map((record) => String(record?.key || "").trim()).filter(Boolean));
    const filtered = filterReportMonthRecords(scopedRecords, now, { explicitOfficeMonthKeys: explicitKeys });
    if (filtered.length) {
      return filtered[0];
    }
    return [...scopedRecords].sort((left, right) => String(right.key || "").localeCompare(String(left.key || "")))[0] || null;
  }
  const fallbackRecords = filterReportMonthRecords(listMonthFiles({ includeInactive: false }), now);
  return fallbackRecords[0] || getMonthFile(currentMonthKey(now), { includeInactive: false }) || listMonthFiles({ includeInactive: false })[0] || null;
}
...
function normalizeOfficeScopeOptions(officeMap = {}, permissionFilters = {}) {
  ...
  const allowedOffices = normalizeStringList(permissionFilters.office);
  if (!allowedOffices.length) {
    return allOffices.sort((left, right) => left.localeCompare(right));
  }
  return allOffices
    .filter((office) => allowedOffices.some((allowed) => officeNamesEquivalent(allowed, office)))
...
function filterOfficeAgentRosterRowsByPermission(rows = [], permissionFilters = {}) {
  const allowedDesks = [
    ...normalizeStringList(permissionFilters.desk),
    ...normalizeStringList(permissionFilters.officeOrDepartment),
  ...
  return rows.filter((row) => {
    ...
    if (allowedTeamLeaders.length) {
      const matchesTeamScope =
        allowedTeamLeaders.includes(normalizedTeamLeader) || allowedTeamLeaders.includes(normalizedAgent);
      if (!matchesTeamScope) {
        return false;
      }
    }
...
async function readMonthData({
  ...
  const rowsRemappedForPermission = remapLeadRowsByRosterProfile(rowsWithScope, tabConfig, rosterProfileMap);
  const permissionRowsAliased = skipPermissionFilters
    ? rowsRemappedForPermission
    : filterRowsByPermission(rowsRemappedForPermission, tabConfig, permissionFilters || {});
...
function builderExtraMetricValues(rows = [], tabConfig, summary = {}, options = {}) {
  ...
  const deskLongTermAverage =
    cachedLongTermBenchmark > 0
      ...
      : deskFtdAverages.get(benchmarkKey)?.avgFtdByLongTerm?.[workProfile.longTermBucket] ||
        deskFtdAverages.get(benchmarkKey)?.avgFtdByAgent ||
        0;
...
function specificBuilderTable(rows, tabConfig, infoContext, monthFilter, query = {}, now = new Date(), options = {}) {
  ...
  const needsDeskFtdAverages =
    needsDeskAverageBaseline || (needsLongTermBaseline && deskBenchmarkCache.size === 0);
...
export async function loadDashboardReport(accessContext, query = {}, options = {}) {
  ...
  const scopedMonthRecords = filterReportMonthRecords(
    selectedOfficeScopes
      .flatMap((office) =>
    ...
    now,
    {
      explicitOfficeMonthKeys: new Set(
        selectedOfficeScopes
          .flatMap((office) => collectOfficeScopedMonthRecords(officeMap, office))
          .map((record) => String(record?.key || "").trim())
          .filter(Boolean),
      ),
    },
  );
  const monthOptionsSource = scopedMonthRecords.length ? scopedMonthRecords : filterReportMonthRecords(rawMonthRecords, now);
