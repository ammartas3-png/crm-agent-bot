...
export async function loadDashboardReport(accessContext, query = {}, options = {}) {
  ...
  const scopedMonthRecords = selectedOfficeScopes
    .flatMap((office) =>
      officeMonthRecordsWithLegacy(
        office,
        collectOfficeScopedMonthRecords(officeMap, office).filter((record) => record.active !== false),
      ).map((record) => ({
        ...record,
        office_name: office,
      })),
  ...
  const officeLiveMonthKeys = collectOfficeScopedMonthRecords(officeMap, primaryOfficeScope)
    .filter((record) => record.active !== false)
    .sort((left, right) => String(right.key || "").localeCompare(String(left.key || "")))
  ...
  const firstLoadableMonthKey =
    selectedMonthKeys.find((monthKey) => {
      const scopedMatch = collectOfficeScopedMonthRecords(officeMap, primaryOfficeScope).find(
        (record) => String(record.key || "") === String(monthKey || "") && String(record.sheet_id || "").trim(),
      );
      return Boolean(scopedMatch?.sheet_id);
    }) ||
    selectedMonthKeys.find((monthKey) => String(getMonthFile(monthKey, { includeInactive: false })?.sheet_id || "").trim()) ||
    selectedMonthKeys[0] ||
    fallbackMonthKey;
  const monthRecord = resolveMonthRecord(firstLoadableMonthKey, primaryOfficeScope, officeMap, now);
  ...
  const selectedMonthRecords = selectedOfficeScopes
    .flatMap((office) => {
      const officeRecords = collectOfficeScopedMonthRecords(officeMap, office)
        .filter((record) => record.active !== false && String(record.sheet_id || "").trim())
        .sort((left, right) => String(right.key || "").localeCompare(String(left.key || "")));
