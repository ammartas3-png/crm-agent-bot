...
function summaryWithTargets(rows, tabConfig, infoContext, dateFilter, now = new Date()) {
  const summary = calculateSummary(rows, tabConfig, dateFilter ? { date: dateFilter } : {}, now);
  const targetMetrics = buildTargetMetrics(rows, tabConfig, infoContext, dateFilter, now);
  return {
    ...summary,
    ...targetMetrics,
...
export async function loadDashboardReport(accessContext, query = {}, options = {}) {
  ...
  if (reportMode === "last4") {
    ...
    return {
      ...
      summary,
      table: rowsByMonth,
      tableTitle: "Last 4 Months Breakdown",
      filters: {
        officeScope,
  ...
  let table = groupTable(modeRows, tabConfig, singleMonth.infoContext, monthFilter, requestedGroupField, now);
  let tableTitle = `Grouped by ${String(query.groupBy || "Agent")}`;
