...
export function specificBuilderTable(rows, tabConfig, infoContext, monthFilter, query = {}, now = new Date(), options = {}) {
  ...
  if (columnDimension) {
    ...
    // column — the reason an August report showed July date columns.
    const orderedColumnValues =
      columnDimension === "date" && monthFilter && monthFilter.type === "month"
        ? orderedColumnValuesRaw.filter((columnValue) => dateMatches(columnValue, monthFilter, now))
