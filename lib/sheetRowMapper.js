...
// FTD / Info Agents tabs do not use the leads ID column. Keep every row so the
// dashboard can hydrate KYC FTD and legacy targets from Redis.
export function prepareAuxiliaryRowsForStore(rows = [], tabConfig, meta = {}) {
  const prepared = [];
  for (const row of rows) {
