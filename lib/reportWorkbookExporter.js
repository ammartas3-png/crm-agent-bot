...
import { parseDateValue } from "./calculations.js";
...
function modeColumns(mode = "full") {
  if (mode === "last4") {
    return [
      ...
      { header: "Desk", key: "office", width: 20 },
...
const LAST4_MONTH_HEADER_COLORS = [
  "FFB7DEE8",
  "FFD8E4BC",
  "FFFCD5B4",
  "FFE4DFEC",
...
function monthHeaderColor(index = 0) {
  return LAST4_MONTH_HEADER_COLORS[index % LAST4_MONTH_HEADER_COLORS.length];
}

function applyCellBorder(cell, overrides = {}) {
  const current = cell.border || {};
  cell.border = {
    top: overrides.top || current.top || { style: "thin", color: { argb: "FFEEEEEE" } },
    left: overrides.left || current.left || { style: "thin", color: { argb: "FFEEEEEE" } },
    bottom: overrides.bottom || current.bottom || { style: "thin", color: { argb: "FFEEEEEE" } },
    right: overrides.right || current.right || { style: "thin", color: { argb: "FFEEEEEE" } },
  };
}
...
function monthKeyRank(monthKey = "") {
...
function startingDateMonthRank(value = "") {
...
function writeLast4AllSheet(
  worksheet,
  {
    ...
    rows = [],
    startRow = 1,
    metricColumns = DEFAULT_LAST4_ALL_METRIC_COLUMNS,
    tailColumns = [],
  },
) {
  ...
  const tailCount = tailColumns.length;
  const totalColumns = infoCount + totalMonthColumns + tailCount;
  ...
  months.forEach((month, index) => {
    ...
    monthCell.fill = {
      type: "pattern",
      pattern: "solid",
      fgColor: { argb: monthHeaderColor(index) },
    };
  });

  for (let index = 0; index < tailColumns.length; index += 1) {
    const colIndex = infoCount + totalMonthColumns + index + 1;
    worksheet.getColumn(colIndex).width = tailColumns[index].width || 16;
    worksheet.mergeCells(startRow, colIndex, startRow + 1, colIndex);
    const tailCell = worksheet.getCell(startRow, colIndex);
    tailCell.value = tailColumns[index].label;
    styleMatrixHeaderCell(tailCell);
  }
  ...
  // Draw clear month frames (left/right borders) like matrix blocks.
  months.forEach((_month, index) => {
    const start = infoCount + index * metricCount + 1;
    const end = start + metricCount - 1;
    for (let col = start; col <= end; col += 1) {
      const topCell = worksheet.getCell(startRow, col);
      const headerCell = worksheet.getCell(startRow + 1, col);
      applyCellBorder(topCell, {
        top: { style: "medium", color: { argb: "FF9E9E9E" } },
        ...(col === start ? { left: { style: "medium", color: { argb: "FF9E9E9E" } } } : {}),
        ...(col === end ? { right: { style: "medium", color: { argb: "FF9E9E9E" } } } : {}),
      });
      applyCellBorder(headerCell, {
  ...
  rows.forEach((rowData) => {
    ...
    tailColumns.forEach((column) => {
      values.push(rowData[column.key] || "");
    });
    const row = worksheet.addRow(values);
    const startMonthRank = startingDateMonthRank(rowData.jobEntry);
    row.eachCell((cell, colIndex) => {
      const metricStart = infoCount + 1;
      const metricEnd = infoCount + totalMonthColumns;
      const isMetric = colIndex >= metricStart && colIndex <= metricEnd;
      const isInfo = !isMetric;
      const metricOffset = colIndex - infoCount - 1;
      const metric = isMetric ? metricColumns[metricOffset % metricCount] : null;
      const isPercent = metric?.type === "percent" || metric?.type === "reach_percent";
      styleMatrixDataCell(cell, { italic: !isInfo, percent: isPercent });
      if (isInfo) {
        cell.font = { ...(cell.font || {}), bold: colIndex <= infoCount, italic: false };
        cell.alignment = { vertical: "middle", horizontal: "left" };
        if (colIndex <= infoCount && rowData.level === "Team Leader") {
          cell.fill = {
            type: "pattern",
            pattern: "solid",
      ...
      if (isMetric && Number.isFinite(startMonthRank)) {
        const monthIndex = Math.floor(metricOffset / metricCount);
        const metricMonthRank = monthKeyRank(months[monthIndex]?.key || "");
        if (Number.isFinite(metricMonthRank) && metricMonthRank < startMonthRank) {
          cell.fill = {
            type: "pattern",
            pattern: "solid",
            fgColor: { argb: "FFE0E0E0" },
...
export async function buildLast4AllWorkbookBuffer({
  ...
  for (const sheet of sheets) {
    ...
    worksheet.mergeCells(
      ...
      Math.max(
        1,
        (sheet.infoColumns?.length || 1) +
          months.length * metricColumns.length +
          (sheet.tailColumns?.length || 0),
      ),
    );
