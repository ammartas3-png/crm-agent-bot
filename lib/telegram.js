...
export function sanitizeTelegramCaption(caption, maxLength = TELEGRAM_CAPTION_LIMIT) {
  ...
  const safeMax = Math.max(4, Number(maxLength) || TELEGRAM_CAPTION_LIMIT);
...
export async function sendTelegramDocument(chatId, fileBuffer, fileName, options = {}) {
  ...
  if (options.caption) {
    const caption = sanitizeTelegramCaption(options.caption);
    if (caption) {
