...
// Telegram echoes the secret configured at setWebhook time in this header. When
// TELEGRAM_WEBHOOK_SECRET is not set we accept every request (backward
// compatible); when it is set we require an exact match.
export function isValidWebhookSecret(providedToken, env = process.env) {
...
export function isValidWebhookRequest(request, env = process.env) {
  const provided =
    typeof request?.headers?.get === "function"
      ? request.headers.get(WEBHOOK_SECRET_HEADER)
      : undefined;
  return isValidWebhookSecret(provided, env);
}
