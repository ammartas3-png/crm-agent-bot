...
const ROW_TARGET_BASE_KEYS = ["TARGET'S", "Agent Target", "Target", "TARGET", "D"];

function targetCandidateKeysFromRows(rows = []) {
  const keySet = new Set(ROW_TARGET_BASE_KEYS);
...
function positiveTargetFromRow(row = {}, candidateKeys = []) {
...
function rowTargetForSingleAgent(rows = [], tabConfig, normalizedAgent = "") {
  if (!normalizedAgent) {
    return 0;
  }
  ...
  const candidateKeys = targetCandidateKeysFromRows(rows);
  const targetByMonth = new Map();
  for (const row of rows || []) {
    const currentAgent = normalizeAgentName(getRowValue(row, agentField));
    if (currentAgent !== normalizedAgent) {
      continue;
    }
    const parsedTarget = positiveTargetFromRow(row, candidateKeys);
    if (!parsedTarget) {
      continue;
    }
    ...
    const currentMonthTarget = Number(targetByMonth.get(monthKey) || 0);
    if (parsedTarget > currentMonthTarget) {
      targetByMonth.set(monthKey, parsedTarget);
    }
  }
  let totalTarget = 0;
  for (const targetValue of targetByMonth.values()) {
...
function buildRowTargetByAgent(rows, tabConfig) {
  ...
  const leadDateField = getFieldName(tabConfig, "leadDate");
  const createdField = getFieldName(tabConfig, "created");
  const targetCandidateKeys = targetCandidateKeysFromRows(rows);
  ...
  for (const row of rows || []) {
    ...
    const parsedTarget = positiveTargetFromRow(row, targetCandidateKeys);
    if (!parsedTarget) {
      continue;
    }
    const sourceMonthKey = String(row?.__sourceMonthKey || "").trim();
...
export function targetAggregationForScope({
  ...
  const normalizedOffices = normalizedFilterValues(scope.office || filters.office);
  const normalizedTeamLeaders = normalizedFilterValues(scope.teamLeader || filters.teamLeader);
  const singleScopedAgent =
    scope.groupField === "agentNames" &&
    normalizedAgents.length === 1 &&
    !normalizedOffices.length &&
    !normalizedTeamLeaders.length;
  ...
  if (singleScopedAgent) {
    const normalizedName = normalizedAgents[0];
    const record = infoContext?.byAgent?.get(normalizedName) || null;
    const rowTarget = rowTargetForSingleAgent(rows, tabConfig, normalizedName);
    ...
    const ftd = Number(ftdByAgent.get(normalizedName) || 0);
