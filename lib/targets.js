...
function buildRowTargetByAgent(rows, tabConfig) {
  ...
  for (const row of rows || []) {
    ...
    const parsedTarget = rawTargetCandidates
      .map((value) => parseTargetNumber(value))
      .find((value) => Number.isFinite(value) && value > 0);
    if (!parsedTarget) {
      continue;
    }
    const current = Number(targetByAgent.get(normalizedAgent) || 0);
    if (parsedTarget > current) {
      targetByAgent.set(normalizedAgent, parsedTarget);
    }
  }
...
export function targetAggregationForScope({
  ...
  for (const normalizedName of candidateAgents) {
    ...
    const target = Number(record?.target || 0) || Number(rowTargetByAgent.get(normalizedName) || 0) || 0;
