...
export function targetAggregationForScope({
  ...
  for (const normalizedName of candidateAgents) {
    ...
    rawTarget += Number.isFinite(target) && target > 0 ? target : 0;
    totalFtd += ftd;
  ...
  return {
    includedTarget,
    // Sum of assigned targets regardless of working status. Used for the
    // displayed "FTD Target" metric so an agent's target shows even when they
    // are not working (the achievement rate still uses includedTarget logic).
    rawTarget,
    totalFtd,
    details,
  };
