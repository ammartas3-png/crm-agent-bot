...
function detectInfoAgentFields(row = {}) {
  ...
  const dedupedStartDateKeyCandidates = [...new Set(startDateKeyCandidates)];
  const startDateKey =
    dedupedStartDateKeyCandidates.find((key) => sanitizeStartDate(row?.[key])) ||
    dedupedStartDateKeyCandidates[0] ||
    keys[11];
...
export function buildInfoAgentsContext(infoAgentRows = []) {
  ...
  for (const row of infoAgentRows) {
    ...
    const startDate = sanitizeStartDate(row?.[startDateKey]);
    ...
    latestByAgent.set(normalizedAgent, {
      ...
      start_date: startDate || previous?.start_date || "",
