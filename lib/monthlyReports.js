...
const BUILTIN_MONTH_MAPPINGS_2026 = [
  ...
  { month: "June 2026", sheet_id: "1sry1psAKWFWGWnE47aQYR2uC0k0Wj7-0RUD8sn5XOAM" },
];
...
// Report month lists should not include future months or the current calendar
// month unless that month is explicitly mapped in the Offices tab (avoids empty
// July rows on July 1 when only June data exists yet).
export function filterReportMonthRecords(records = [], now = new Date(), options = {}) {
  const currentKey = currentMonthKey(now);
  const explicitKeys = options.explicitOfficeMonthKeys;
  const explicitSet =
    explicitKeys instanceof Set
      ? explicitKeys
      : new Set(
          (Array.isArray(explicitKeys) ? explicitKeys : [])
  ...
  return [...records]
    .filter((record) => {
      ...
      if (!key || record.active === false || !String(record.sheet_id || "").trim()) {
        return false;
      }
      if (key > currentKey) {
        return false;
      }
      if (key === currentKey && explicitSet.size > 0 && !explicitSet.has(key)) {
        return false;
      }
      return true;
    })
...
export function mergeMonthRecordsByKey(...lists) {
  ...
  for (const list of lists) {
    ...
    for (const record of list) {
      ...
      if (!key || !sheetId || record.active === false) {
        continue;
      }
      if (!merged.has(key)) {
  ...
  return [...merged.values()].sort((left, right) => String(right.key || "").localeCompare(String(left.key || "")));
}

// Pick the most recent month records for lookups such as job-entry / roster
// registration. When minimum is set, always include at least that many recent
// months from the provided pool (e.g. last two months for onboarding data).
export function getRecentMonthRecords(records = [], now = new Date(), options = {}) {
  ...
  const eligible = [...records]
    .filter((record) => {
      const key = String(record?.key || "").trim();
      const sheetId = String(record?.sheet_id || "").trim();
      return key && sheetId && record.active !== false && key <= currentKey;
    })
    .sort((left, right) => String(right.key || "").localeCompare(String(left.key || "")));
  const selected = eligible.slice(0, limit);
  if (minimum > selected.length) {
    ...
    selected.sort((left, right) => String(right.key || "").localeCompare(String(left.key || "")));
  }
  const cap = Math.max(limit, minimum);
