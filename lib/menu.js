...
function hierarchyFromScope({ groupField, label = "", filters = {}, infoContext } = {}) {
  let office = String(filters.office || "").trim();
  let teamLeader = String(filters.teamLeader || "").trim();
  let agent = String(filters.agent || "").trim();
  let country = String(filters.country || "").trim();
  let campaign = String(filters.campaign || "").trim();
  let placement = String(filters.placement || "").trim();
  let subCampaign = String(filters.subCampaign || "").trim();
