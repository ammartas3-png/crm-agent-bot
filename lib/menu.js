...
function buildLast4AllSheetRows({
  ...
  return items.map((item) => {
    ...
    if (groupField === "teamLeader") {
      ...
      return {
        level: "Team Leader",
        office: scopeOfficeName || defaultScopeOffice || officeForTeamLeaderLabel(infoContext, item.label),
    ...
    return {
      level: "Agent",
      office: rowDesk || scopeOfficeName || defaultScopeOffice || assignment.office,
      teamLeader: assignment.teamLeader,
      agent: item.label,
      jobEntry,
      monthMetrics: monthMetricsByKey(item.monthlyMetrics || []),
