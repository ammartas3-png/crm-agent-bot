...
function buildGroupItems(
  ...
  const scopedOfficeNames = new Set(
    rowsWithoutDate.map((row) => String(row.__scopeOfficeName || "").trim()).filter(Boolean),
  );
  const preferDeskLabelForOfficeGroup = groupField === "office" && scopedOfficeNames.size === 1;
  const groups = new Map();
  ...
  for (const row of rowsWithoutDate) {
    const label =
      groupField === "office"
        ? String(
            preferDeskLabelForOfficeGroup
              ? getRowValue(row, fieldName)
              : row.__scopeOfficeName || getRowValue(row, fieldName) || "",
          ).trim()
