...
function simplePresetQuery({ presetKey = "", office = "", monthKey = "", officeMonthKeys = [] } = {}) {
  ...
  if (presetKey === "monthly") {
    ...
    query.metricFields = "leads,kycFtd,ftd,ftdTarget,ftdTargetReach,cr,crTarget,crTargetReach";
    return query;
  }
  if (presetKey === "last4") {
...
async function buildSimpleQuickExportPayload({
  ...
  if (!accessContext?.authorized || !accessContext?.authorityScope?.unrestricted) {
    throw new Error("Only users with ALL authority can request bot exports.");
  }
  // Full-authority exports are identical for the same office/month/report, so
  // cache the built workbook to avoid re-reading the sheet and rebuilding the
  // Excel for every concurrent request.
  const cacheKey = `simple|${presetKey}|${office}|${monthKey}|${officeMonthKeys.join(",")}`;
  return getOrBuildExport(cacheKey, async () => {
    ...
    const fileName = `bot-${safeExportFileName(reportName)}-${officePart}-${monthPart}.xlsx`;
    const captionMonth = report?.month?.label || report?.month?.key || query.monthKey || "auto";
    return {
      workbookBuffer,
      fileName,
      caption: `${reportName} | Office: ${office || "-"} | Month: ${captionMonth}`,
    };
  });
}
...
export async function handleMenuCallback(userId, callbackData, options = {}) {
  ...
  if (!canUseSimpleBotReports(options)) {
    return {
      text: "Only users with ALL authority can request bot reports.",
      replyMarkup: inlineKeyboard([[{ text: "Back", callbackData: "root:start" }]]),
    };
  }
  ...
  if (!String(callbackData || "").startsWith("simple:")) {
    return {
      text: "Bot report flow updated. Please select office and quick report again.",
