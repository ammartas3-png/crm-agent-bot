...
function scopeFiltersWithOfficeSelection(baseFilters = {}, session = {}) {
  const country = String(session.selectedOfficeCountry || "").trim();
  if (!country) {
    return baseFilters;
  }
...
export async function startMenu(userId, options = {}) {
  ...
  const selectedOfficePatterns = selectedOfficeCountry ? officeCountryPatterns(selectedOfficeCountry) : [];
  setSession(userId, {
    step: "select_month",
    selectedOfficeCountry,
    selectedOfficePatterns,
...
export async function handleMenuCallback(userId, callbackData, options = {}) {
  ...
  if (action === "officeScope") {
    ...
    const selectedOfficeFilters = [];
    const selectedOfficePatterns = officeCountryPatterns(country);
    setSession(userId, {
      step: "select_month",
