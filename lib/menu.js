...
export const MAIN_MENU_TEXT = "Select report type:";
...
const LAST4_STARTING_DATE_AGENT_COLUMN = "Agent";
const LAST4_STARTING_DATE_VALUE_COLUMN = "Starting Date";
const LAST4_STARTING_DATE_COLUMNS = [
  "Working Status",
  "LANG",
  LAST4_STARTING_DATE_AGENT_COLUMN,
  "TARGET'S",
  "FTD'S",
  "Office",
  "Team Leader",
  ...
  "CR TARGET",
  "Late FTD",
  LAST4_STARTING_DATE_VALUE_COLUMN,
];
...
const JOB_ENTRY_SOURCE_MONTH_LIMIT = 4;
const JOB_ENTRY_SOURCE_MONTH_MINIMUM = 2;
const LAST4_INFO_AGENTS_TAB_NAME_CANDIDATES = ["Info Agents", "Info Agnets", "Info Agent", "InfoAgents"];
const LAST4_INFO_AGENTS_RAW_COLUMNS = Array.from({ length: 12 }, (_, index) =>
  String.fromCharCode("A".charCodeAt(0) + index),
);

const REPORT_TYPES = {
  office: { label: "Desk", fieldKey: "office" },
...
const REPORT_DATA_CACHE_TTL_MS = 10 * 60 * 1000;
const REPORT_DATA_SHARED_CACHE_MAX = 80;
const sharedReportDataCache = new Map();
const reportDataInflightCache = new Map();
const readRowsCacheIdMap = new WeakMap();
let nextReadRowsCacheId = 1;
...
function resolveOfficeMonthRecords(officeName = "", officeMap = {}) {
  if (Array.isArray(officeMap?.byOffice?.[officeName])) {
    return officeMap.byOffice[officeName];
  }
  const matchedOffice = Object.keys(officeMap?.byOffice || {}).find((name) => {
    const normalizedName = normalizeText(name);
    return normalizedName === normalizeText(officeName) || officeNamesEquivalent(name, officeName);
  });
  return matchedOffice ? officeMap.byOffice[matchedOffice] : [];
}

function officeScopedMonthFiles(officeName = "", officeMap = {}, now = new Date()) {
  const scoped = resolveOfficeMonthRecords(officeName, officeMap);
  const explicitKeys = new Set(scoped.map((record) => String(record?.key || "").trim()).filter(Boolean));
  ...
  // If Offices tab misses a month (e.g. June), fall back to global month mappings for that office scope.
  ...
  return filterReportMonthRecords([...mergedByKey.values()], now, { explicitOfficeMonthKeys: explicitKeys });
}
...
function scopeFiltersWithOfficeSelection(baseFilters = {}, session = {}) {
  const officeScope = String(session.selectedOfficeCountry || "").trim();
  if (!officeScope) {
    return baseFilters;
  }
  const officeContains = Array.isArray(session.selectedOfficePatterns) ? session.selectedOfficePatterns : [];
  const officeValues = Array.isArray(session.selectedOfficeFilters) ? session.selectedOfficeFilters : [];
  const hasCountryScopedMonths = Array.isArray(session.officeMonthFiles) && session.officeMonthFiles.length > 0;
  return {
    ...baseFilters,
    ...(!hasCountryScopedMonths && officeContains.length ? { officeContains } : {}),
    ...(officeValues.length ? { office: officeValues } : {}),
  };
}
...
export function mainMenuKeyboard(telegramUser, options = {}) {
  ...
  const rows = [
    [{ text: "By Desk Results", callbackData: "report:office" }],
    [{ text: "By Team Leader Results", callbackData: "report:teamLeader" }],
    [{ text: "By Agent Results", callbackData: "report:agent" }],
  ];
  if (!onlyCore) {
    rows.push([{ text: "By Country Results", callbackData: "report:country" }]);
...
function dateFilterKeyboard(options = {}) {
...
function specificReportsKeyboard() {
  return inlineKeyboard([
    [{ text: "By Hourly", callbackData: "special:hourly" }],
    [{ text: "Country Comparison", callbackData: "special:compareCountry" }],
    [{ text: "Best Performance", callbackData: "special:bestOpen" }],
...
function bestPerformersKeyboard() {
  return inlineKeyboard([
    ...
    [{ text: "Best Country", callbackData: "special:bestCountries" }],
    [{ text: "Best Campaign", callbackData: "special:bestCampaigns" }],
...
function reportFilterHeaderLines(month, dateConfig, session = {}) {
...
function reportFilterTitle(month, dateConfig, session = {}) {
  const lines = reportFilterHeaderLines(month, dateConfig, session);
  lines.push(MAIN_MENU_TEXT);
  return lines.join("\n");
}

function compactSummaryLine(summary = {}) {
  const lead = Number(summary.totalLeads || 0).toLocaleString("en-US");
  const ftd = Number(summary.totalFtd || 0).toLocaleString("en-US");
  const cr = Number(summary.cr || 0).toFixed(2);
  const selfs = Number(summary.selfs || 0).toLocaleString("en-US");
  const lateFtd = Number(summary.lateFtd || 0).toLocaleString("en-US");
  return `Summary (selected date): Lead ${lead} | FTD ${ftd} | CR ${cr}% | Selfs ${selfs} | Late FTD ${lateFtd}`;
}
...
function permissionScopeFiltersForUser(authorityScope = {}, telegramUser) {
  if (isAdminTelegramUser(telegramUser)) {
    return {};
  }
  return authorityScopeFilters(authorityScope);
}
...
function filterInfoAgentRowsByScope(infoRows = [], infoAgentsTabConfig, scopeFilters = {}, options = {}) {
  ...
  return infoRows.filter((row) => {
    const scopeOffice = normalizeText(row.__scopeOfficeName || "");
    ...
    if (normalizedOfficeSet.size > 0 && !normalizedOfficeSet.has(scopeOffice || office)) {
      return false;
    }
...
function hourlyReportKeyboard(options = {}) {
  ...
  rows.push([{ text: "By Desk", callbackData: "special:hourlyScope:office" }]);
...
function bestContextNavigationRows() {
  return [
    [{ text: "Back to Best Performance", callbackData: "special:bestOpen" }],
    [{ text: "Back to Specific Reports", callbackData: "special:open" }],
    [{ text: "Back to Report Filters", callbackData: "menu:filters" }],
    [{ text: "Change Month", callbackData: "menu:main" }],
  ];
...
function getLastFourMonthRecords(now = new Date(), session = {}) {
  const sessionMonths = sessionMonthRecords(session).filter((month) => month.active !== false);
  const globalMonths = filterReportMonthRecords(listMonthFiles({ includeInactive: false }), now);
  const merged = mergeMonthRecordsByKey(sessionMonths, globalMonths);
  return getRecentMonthRecords(merged, now, { limit: 4 });
}
...
function jobEntryLookupKey(agentName, office = "") {
  const normalizedName = normalizeJobEntryName(agentName);
  if (!normalizedName) {
    return "";
  }
  const normalizedOffice = normalizeText(office);
  return normalizedOffice ? `${normalizedOffice}::${normalizedName}` : normalizedName;
}

const LANGUAGE_TOKEN_SET = new Set([
  "tl",
  "tr",
  "eng",
  "english",
...
function looksLikeLanguageToken(value = "") {
  const normalized = normalizeText(value).replace(/[^a-z]/g, "");
  if (!normalized) {
    return false;
  }
  return LANGUAGE_TOKEN_SET.has(normalized);
}

function pickJobEntryAgentName(row = {}) {
  const explicitAgent =
    getRowValue(row, LAST4_STARTING_DATE_AGENT_COLUMN) ||
    getRowValue(row, "Agent Name") ||
    getRowValue(row, "AGENT NAMES") ||
    getRowValue(row, LAST4_JOB_ENTRY_NAME_COLUMN);
  const langValue = getRowValue(row, "LANG");
  const colB = getRowValue(row, "B");
  const colC = getRowValue(row, "C");
  const hasRawColumns = Object.keys(row).some((key) => /^[A-L]$/i.test(String(key || "").trim()));

  if (explicitAgent && !looksLikeLanguageToken(explicitAgent)) {
    return explicitAgent;
  }
  if (explicitAgent && langValue && !looksLikeLanguageToken(langValue)) {
    return langValue;
  }
  if (hasRawColumns) {
    if (colB && !looksLikeLanguageToken(colB)) {
      return colB;
    }
    if (colC && !looksLikeLanguageToken(colC)) {
      return colC;
    }
  }
  if (colC && !looksLikeLanguageToken(colC)) {
  ...
  if (langValue && !looksLikeLanguageToken(langValue)) {
    return langValue;
  }
  return explicitAgent || colC || colB || langValue || "";
}

function pickJobEntryOfficeName(row = {}) {
  const explicitOffice = getRowValue(row, "Office") || getRowValue(row, "Desk") || getRowValue(row, "Department");
  if (explicitOffice) {
    return explicitOffice;
  }
  const colF = String(getRowValue(row, "F") || "").trim();
  const colG = String(getRowValue(row, "G") || "").trim();
  if (colF && !/%$/.test(colF) && !/^\d+(?:[.,]\d+)?$/.test(colF)) {
    return colF;
  }
  if (colG) {
    return colG;
...
function formatJobEntryValue(value) {
  ...
  if (!normalizedRaw || normalizedRaw === "starting date" || normalizedRaw === "job entry") {
    return "";
  }
...
function buildLast4JobEntryMap(rows = []) {
  ...
  for (const row of rows) {
    const rawName = pickJobEntryAgentName(row);
    const genericKey = jobEntryLookupKey(rawName);
    if (!genericKey) {
      continue;
    }
    const rawEntry =
      getRowValue(row, LAST4_STARTING_DATE_VALUE_COLUMN) ||
      getRowValue(row, LAST4_JOB_ENTRY_VALUE_COLUMN) ||
      getRowValue(row, "L");
    const formattedEntry = formatJobEntryValue(rawEntry);
    if (!formattedEntry) {
      continue;
    }
    const rawOffice = pickJobEntryOfficeName(row);
    const scopedKey = jobEntryLookupKey(rawName, rawOffice);
    if (scopedKey && !map.has(scopedKey)) {
      map.set(scopedKey, formattedEntry);
    }
    if (!map.has(genericKey)) {
      map.set(genericKey, formattedEntry);
    }
  }
  return map;
}

function infoAgentsTabNamesForJobEntry(infoAgentsTabConfig = {}) {
  const configuredName = String(infoAgentsTabConfig?.name || "").trim();
  return [
    ...new Set(
      [configuredName, ...LAST4_INFO_AGENTS_TAB_NAME_CANDIDATES]
...
function jobEntrySourceMonths(session = {}, now = new Date()) {
  const sessionMonths = sessionMonthRecords(session).filter((month) => month.active !== false);
  const globalMonths = filterReportMonthRecords(listMonthFiles({ includeInactive: false }), now);
  const recentMonths = getRecentMonthRecords(mergeMonthRecordsByKey(sessionMonths, globalMonths), now, {
    limit: JOB_ENTRY_SOURCE_MONTH_LIMIT,
    minimum: JOB_ENTRY_SOURCE_MONTH_MINIMUM,
  })
    .filter((month) => month?.sheet_id)
    .sort((left, right) => String(right.key || "").localeCompare(String(left.key || "")));
  const selectedSpreadsheetId = normalizeSpreadsheetId(session?.spreadsheetId || "");
  const sourceMonths = [
    ...recentMonths,
    ...(selectedSpreadsheetId
      ? [
          {
            key: String(session?.monthKey || "selected"),
            sheet_id: selectedSpreadsheetId,
          },
        ]
      : []),
  ];
  ...
  for (const month of sourceMonths) {
    const id = normalizeSpreadsheetId(month.sheet_id);
    if (!id || seen.has(id)) {
      continue;
    }
    seen.add(id);
    uniqueRecent.push({ ...month, sheet_id: id });
  }
  return uniqueRecent;
}

async function readLast4JobEntryMap(readRows, options = {}) {
  const session = options.session || {};
  const now = options.now || new Date();
  const infoAgentsTabConfig = options.infoAgentsTabConfig || getTabConfig("infoAgents");
  const months = jobEntrySourceMonths(session, now);
  if (months.length) {
    const tabNames = infoAgentsTabNamesForJobEntry(infoAgentsTabConfig);
    const map = new Map();
    for (const month of months) {
      for (const tabName of tabNames) {
        const tabConfig = {
          ...infoAgentsTabConfig,
          name: tabName,
          range: `'${String(tabName).replace(/'/g, "''")}'!A:L`,
          columns: LAST4_STARTING_DATE_COLUMNS,
        };
        try {
          const rows = await readRows("infoAgents", { tabConfig, spreadsheetId: month.sheet_id });
          for (const [agentName, entryDate] of buildLast4JobEntryMap(rows)) {
            if (!map.has(agentName)) {
              map.set(agentName, entryDate);
            }
          }
          if (map.size > 0) {
            break;
      ...
      for (const tabName of tabNames) {
        const rawTabConfig = {
          ...
          name: tabName,
          range: `'${String(tabName).replace(/'/g, "''")}'!A:L`,
          columns: LAST4_INFO_AGENTS_RAW_COLUMNS,
        };
        try {
          const rows = await readRows("infoAgents", { tabConfig: rawTabConfig, spreadsheetId: month.sheet_id });
          for (const [agentName, entryDate] of buildLast4JobEntryMap(rows)) {
            if (!map.has(agentName)) {
              map.set(agentName, entryDate);
            }
...
function buildLatestAssignmentsByAgent(rows = [], tabConfig, latestMonthKey = "") {
  const agentField = getFieldName(tabConfig, "agentNames");
  const teamLeaderField = getFieldName(tabConfig, "teamLeader");
  const deskField = getFieldName(tabConfig, "desk");
  const officeField = getFieldName(tabConfig, "office");
  const departmentField = getFieldName(tabConfig, "department");
  const rowsForLatestMonth = latestMonthKey
    ? rows.filter((row) => String(row.__reportMonthKey || "").trim() === String(latestMonthKey).trim())
    : rows;
  const grouped = new Map();
  for (const row of rowsForLatestMonth) {
    const normalizedAgent = normalizeAgentName(canonicalAgentName(getRowValue(row, agentField)));
    if (!normalizedAgent) {
      continue;
    }
    const desk = String(
      getRowValue(row, "Desk") ||
        getRowValue(row, deskField) ||
        getRowValue(row, officeField) ||
        getRowValue(row, "Office") ||
        getRowValue(row, departmentField) ||
        getRowValue(row, "Department") ||
        "",
    ).trim();
    const teamLeader = String(
      getRowValue(row, "Team Leader") || getRowValue(row, teamLeaderField) || "",
    ).trim();
    if (!grouped.has(normalizedAgent)) {
      grouped.set(normalizedAgent, { desks: [], teamLeaders: [] });
    }
    if (desk) {
      grouped.get(normalizedAgent).desks.push(desk);
    }
    if (teamLeader) {
      grouped.get(normalizedAgent).teamLeaders.push(teamLeader);
    }
  }
  const assignments = new Map();
  for (const [agentKey, values] of grouped.entries()) {
    assignments.set(agentKey, {
      desk: pickMostFrequentLabel(values.desks || []),
      teamLeader: pickMostFrequentLabel(values.teamLeaders || []),
    });
  }
  return assignments;
}
...
function remapRowsToCurrentInfo(rows = [], tabConfig, infoContext, options = {}) {
  ...
  const latestAssignmentsByAgent = options.latestAssignmentsByAgent || new Map();
  return rows.map((row) => {
    ...
    const latestAssignment = normalizedAgent ? latestAssignmentsByAgent.get(normalizedAgent) : null;
    const normalizedOfficeField = normalizeText(officeField);
    const existingOfficeValue = getRowValue(row, officeField) || getRowValue(row, "Desk");
    const resolvedOfficeValue =
      normalizedOfficeField === "desk"
        ? latestAssignment?.desk || currentRecord.office || existingOfficeValue
        : currentRecord.office || existingOfficeValue;
    const resolvedTeamLeader =
      currentRecord.team_leader || latestAssignment?.teamLeader || getRowValue(row, teamLeaderField);
    return {
      ...row,
      [agentField]: currentRecord.agent_name || resolvedAgentName || rawAgentName,
      [officeField]: resolvedOfficeValue,
      [teamLeaderField]: resolvedTeamLeader,
    };
  });
}
...
async function readReportData(readRows, tabConfig, infoAgentsTabConfig, spreadsheetId, options = {}) {
  ...
  const permissionFilters = options.permissionFilters || {};
  ...
  if (options.last3Mode) {
    ...
    for (const month of months) {
      const monthScopeOffice = String(month.office_name || options.session?.selectedOfficeCountry || "").trim();
      const rawMonthRows = await readRows("leads", { tabConfig, spreadsheetId: month.sheet_id });
      const scopedMonthRows = rawMonthRows.map((row) => ({
        ...row,
        __scopeOfficeName: monthScopeOffice || String(row.__scopeOfficeName || "").trim(),
      }));
      const permissionRows = filterRowsByPermission(scopedMonthRows, tabConfig, permissionFilters);
      if (scopedMonthRows.length > 0 && permissionRows.length === 0 && Object.keys(permissionFilters).length > 0) {
        const debug = permissionFilterDebug(scopedMonthRows, tabConfig, permissionFilters);
        console.warn("Permission filter removed all rows (last4)", {
          monthKey: month.key,
          culpritField: debug.culpritField,
          normalizedFilters: debug.normalizedFilters,
          availableByField: debug.availableByField,
          unmatchedByField: debug.unmatchedByField,
          steps: debug.steps,
        });
      }
      const monthRows = scopedRowsByFilters(permissionRows, tabConfig, scopeFilters, scopeNow);
      let monthInfoRows = [];
      try {
        ...
        monthInfoRows = monthInfoRows.map((row) => ({
          ...row,
          __scopeOfficeName: monthScopeOffice || String(row.__scopeOfficeName || "").trim(),
        }));
      } catch {
    ...
    const latestAssignmentsByAgent = buildLatestAssignmentsByAgent(aliasedRows, tabConfig, infoMonth?.key || "");
    const remappedRows = remapRowsToCurrentInfo(aliasedRows, tabConfig, infoContext, {
      ...
      monthAgentDirectoryByKey,
      latestAssignmentsByAgent,
    });
    const agentField = getFieldName(tabConfig, "agentNames");
    const workingAgents = new Set(infoContext.agents || []);
    const hasCurrentWorkingAgents = workingAgents.size > 0;
    const workingAgentsByMonth = new Map(
      Object.entries(monthInfoContextByKey).map(([monthKey, context]) => [monthKey, new Set(context?.agents || [])]),
    );
    const filteredRows = remappedRows.filter((row) => {
      ...
      if (hasCurrentWorkingAgents) {
        // Last 4 Months uses latest month's Working/Active roster as the source of truth.
        return workingAgents.has(normalizedAgent);
      }
      const monthKey = String(row.__reportMonthKey || "");
      const monthWorkingSet = workingAgentsByMonth.get(monthKey);
      if (monthWorkingSet && monthWorkingSet.size > 0) {
        return monthWorkingSet.has(normalizedAgent);
      }
      return false;
    });
    return {
  ...
  const scopeOfficeName = String(options.session?.selectedOfficeCountry || "").trim();
  const rawRows = await readRows("leads", { tabConfig, spreadsheetId });
  const scopedRows = rawRows.map((row) => ({
    ...row,
    __scopeOfficeName: scopeOfficeName || String(row.__scopeOfficeName || "").trim(),
  }));
  const permissionRows = filterRowsByPermission(scopedRows, tabConfig, permissionFilters);
  if (scopedRows.length > 0 && permissionRows.length === 0 && Object.keys(permissionFilters).length > 0) {
    const debug = permissionFilterDebug(scopedRows, tabConfig, permissionFilters);
    console.warn("Permission filter removed all rows", {
      spreadsheetId,
      culpritField: debug.culpritField,
      normalizedFilters: debug.normalizedFilters,
      availableByField: debug.availableByField,
  ...
  const rows = applyAgentNameAliases(
    scopedRowsByFilters(permissionRows, tabConfig, scopeFilters, scopeNow),
    tabConfig,
  ).map((row) => ({
    ...row,
    __scopeOfficeName: scopeOfficeName || String(row.__scopeOfficeName || "").trim(),
  ...
  try {
    ...
    infoAgentRows = infoAgentRows.map((row) => ({
      ...row,
      __scopeOfficeName: scopeOfficeName || String(row.__scopeOfficeName || "").trim(),
    }));
  } catch {
  ...
  infoAgentRows =
    scopeFilters.department && scopedAgents.size === 0
      ...
      : filterInfoAgentRowsByScope(infoAgentRows, infoAgentsTabConfig, scopeFilters, {
          ...
          enforceAllowedAgentSet:
            Boolean(scopeFilters.department) ||
            Object.values(permissionFilters).some((value) => Array.isArray(value) && value.length > 0),
        });
...
function reportDataCacheKey(spreadsheetId, options = {}) {
  return JSON.stringify({
    spreadsheetId: String(spreadsheetId || ""),
    last3Mode: Boolean(options.last3Mode),
    last3MonthKeys: Array.isArray(options.last3MonthKeys) ? options.last3MonthKeys : [],
    scopeFilters: options.scopeFilters || {},
    permissionFilters: options.permissionFilters || {},
    selectedOfficeCountry: String(options.session?.selectedOfficeCountry || ""),
  });
}

function pruneSharedReportDataCache() {
  if (sharedReportDataCache.size <= REPORT_DATA_SHARED_CACHE_MAX) {
    return;
  }
  const oldest = [...sharedReportDataCache.entries()].sort(
    (left, right) => Number(left[1]?.ts || 0) - Number(right[1]?.ts || 0),
  );
  while (sharedReportDataCache.size > REPORT_DATA_SHARED_CACHE_MAX && oldest.length) {
    const [key] = oldest.shift();
    sharedReportDataCache.delete(key);
  }
}

function readRowsCacheId(readRows) {
  if (typeof readRows !== "function") {
    return "default";
  }
  if (!readRowsCacheIdMap.has(readRows)) {
    readRowsCacheIdMap.set(readRows, nextReadRowsCacheId++);
  }
  return String(readRowsCacheIdMap.get(readRows));
}

async function readReportDataWithCache(userId, readRows, tabConfig, infoAgentsTabConfig, spreadsheetId, options = {}) {
  const cacheKey = `${readRowsCacheId(readRows)}::${reportDataCacheKey(spreadsheetId, options)}`;
  const session = getSession(userId);
  const cache = session.reportDataCache || null;
  if (cache && cache.key === cacheKey && Date.now() - Number(cache.ts || 0) < REPORT_DATA_CACHE_TTL_MS) {
    return cache.data;
  }
  const sharedCache = sharedReportDataCache.get(cacheKey);
  if (sharedCache && Date.now() - Number(sharedCache.ts || 0) < REPORT_DATA_CACHE_TTL_MS) {
    setSession(userId, {
      reportDataCache: {
        key: cacheKey,
        ts: Date.now(),
  ...
  if (sharedCache) {
    sharedReportDataCache.delete(cacheKey);
  }
  if (reportDataInflightCache.has(cacheKey)) {
    const data = await reportDataInflightCache.get(cacheKey);
    setSession(userId, {
      reportDataCache: {
        key: cacheKey,
        ts: Date.now(),
  ...
  const pending = readReportData(readRows, tabConfig, infoAgentsTabConfig, spreadsheetId, options);
  reportDataInflightCache.set(cacheKey, pending);
  const data = await pending.finally(() => {
    if (reportDataInflightCache.get(cacheKey) === pending) {
      reportDataInflightCache.delete(cacheKey);
    }
  });
  const nowTs = Date.now();
  sharedReportDataCache.set(cacheKey, { ts: nowTs, data });
  pruneSharedReportDataCache();
  setSession(userId, {
    reportDataCache: {
      key: cacheKey,
      ts: nowTs,
...
function metricPayload(
  ...
  const targetAggregation = targetAggregationForScope({
    ...
    scope: {
      ...
      onlyWorkingAgents: Boolean(options.onlyWorkingAgents),
    },
    now,
  });
...
function hasScopedFilterValue(value) {
  if (Array.isArray(value)) {
...
function scopeRowsToSelectedDimensions(rows = [], tabConfig, filters = {}, now = new Date()) {
  const scopeFilters = {};
  const scopedKeys = [
    "office",
    "officeOrDepartment",
  ...
  for (const key of scopedKeys) {
    if (hasScopedFilterValue(filters[key])) {
      scopeFilters[key] = filters[key];
    }
  }
  if (scopeFilters.agent && hasScopedFilterValue(filters.agentField)) {
    scopeFilters.agentField = filters.agentField;
  }
  if (!Object.keys(scopeFilters).length) {
    return rows;
  }
  return filteredRows(rows, tabConfig, scopeFilters, now);
}
...
function buildGroupItems(
  ...
  for (const row of rowsWithoutDate) {
    const label =
      groupField === "office"
        ? String(getRowValue(row, fieldName) || row.__scopeOfficeName || "").trim()
        : String(getRowValue(row, fieldName) || "").trim();
    if (!label) {
      continue;
    }
  ...
  const rosterLabels = groupField === "office" ? [] : infoAgentsLabelsForGroup(infoContext, groupField, baseFilters);
            ...
            return {
              ...
              metrics: metricPayload(
                ...
                now,
                {
                  ...
                  onlyWorkingAgents: options.metricsMode === "last4",
                },
              ),
            };
          })
...
function buildDetailKeyboard(view, rootType) {
  ...
  if (rootType === "agent") {
    rows.push([
      ...
      { text: "Back to Desk filter", callbackData: "report:office" },
    ]);
  }
  rows.push([{ text: "Export Excel", callbackData: "export:current" }]);
  rows.push([{ text: "Back to Report Filters", callbackData: "menu:filters" }]);
...
function listComputationCacheKey(view = {}) {
  const monthKeys = (view.monthBreakdownMonths || [])
    .map((month) => String(month?.key || ""))
    .filter(Boolean);
  return JSON.stringify({
    rootType: view.rootType || "",
    groupField: view.groupField || "",
    baseFilters: view.baseFilters || {},
    metricsMode: view.metricsMode || "full",
    monthKeys,
  });
}

function slimListItems(items = []) {
...
function renderListView(view, rows, tabConfig, targetsMap, infoContext, now = new Date()) {
  const computationKey = listComputationCacheKey(view);
  const cachedComputation =
    view?.listComputationCache && view.listComputationCache.key === computationKey ? view.listComputationCache : null;
  const items =
    cachedComputation?.items ||
    slimListItems(
      buildGroupItems(
        rows,
        tabConfig,
  ...
  const totalMetrics =
    cachedComputation?.totalMetrics || totalMetricsForView(view, rows, tabConfig, targetsMap, infoContext, now);
  const { page, totalPages, start, chunk } = paginateItems(items, view.page || 0, 8);
  const listComputationCache = {
    key: computationKey,
    items,
    totalMetrics,
  };
  ...
  if (!items.length) {
    return {
      ...
      nextView: { ...view, page: 0, listComputationCache },
    };
  }
  ...
  return {
    ...
    nextView: { ...view, page, listComputationCache },
    items,
  };
}
...
function pickMostFrequentLabel(values = []) {
  const byKey = new Map();
...
function deskLabelFromSummary(item = {}, tabConfig) {
  const summaryRows = Array.isArray(item.summary?.contextRows)
    ? item.summary.contextRows
    : Array.isArray(item.summary?.rows)
      ? item.summary.rows
      : [];
  if (!summaryRows.length) {
    return "";
  }
  const latestMonthKey = summaryRows.reduce((latest, row) => {
    const monthKey = String(row.__reportMonthKey || "").trim();
    return monthKey > latest ? monthKey : latest;
  }, "");
  const latestRows = latestMonthKey
    ? summaryRows.filter((row) => String(row.__reportMonthKey || "").trim() === latestMonthKey)
    : summaryRows;
  const rowsForDesk = latestRows.length ? latestRows : summaryRows;
  const deskField = getFieldName(tabConfig, "desk");
  const departmentField = getFieldName(tabConfig, "department");
  const officeField = getFieldName(tabConfig, "office");
  const deskValues = rowsForDesk
    .map((row) =>
      String(
        getRowValue(row, "Desk") ||
          getRowValue(row, deskField) ||
    ...
    .filter(Boolean);
  return pickMostFrequentLabel(deskValues);
}
...
function buildLast4AllSheetRows({
  ...
  return items.map((item) => {
    ...
    if (groupField === "teamLeader") {
      const scopeOfficeName =
        String(item.summary?.contextRows?.[0]?.__scopeOfficeName || item.summary?.rows?.[0]?.__scopeOfficeName || "").trim();
      const mappedDesk = officeForTeamLeaderLabel(infoContext, item.label);
      return {
        level: "Team Leader",
        office: scopeOfficeName || mappedDesk || defaultScopeOffice,
    ...
    const scopeOfficeName =
      String(item.summary?.contextRows?.[0]?.__scopeOfficeName || item.summary?.rows?.[0]?.__scopeOfficeName || "").trim();
    const rowDesk = deskLabelFromSummary(item, tabConfig);
    const scopedJobEntryKey = jobEntryLookupKey(item.label, assignment.office);
    const fallbackJobEntryKey = jobEntryLookupKey(item.label);
    const jobEntry =
      (scopedJobEntryKey && jobEntryByAgent.get(scopedJobEntryKey)) ||
      (fallbackJobEntryKey && jobEntryByAgent.get(fallbackJobEntryKey)) ||
      "";
    return {
      level: "Agent",
      office: rowDesk || assignment.office || scopeOfficeName || defaultScopeOffice,
      teamLeader: assignment.teamLeader,
      agent: item.label,
      jobEntry,
      monthMetrics: monthMetricsByKey(item.monthlyMetrics || []),
...
function last4ExportActionsKeyboard(hasOfficeValues = true) {
  const rows = [];
  if (hasOfficeValues) {
    rows.push([{ text: "Specific Desk Excel", callbackData: "last4:officeList:0" }]);
  }
...
async function buildLast4AllExportPayload({
  ...
  const scopedExportFilters = officeFilter
    ? {
        ...(scopeFilters || {}),
        office: officeFilter,
      }
  ...
  const { rows, targetsMap, infoContext, monthInfoContextByKey = {}, months = [] } = await readReportData(
    ...
    month.sheet_id,
    {
      ...
      session,
      now,
      scopeFilters: scopedExportFilters,
      permissionFilters,
    },
  );
  ...
  const agentRows = buildLast4AllSheetRows({
    ...
    jobEntryByAgent,
    defaultScopeOffice: String(session.selectedOfficeCountry || "").trim(),
    now,
  });
...
async function resolveOfficeScopeForStart(options = {}) {
  ...
  const authorityOffices = Array.isArray(scopeFilters.office)
    ...
    : [];
  const isAllScope = isAdminTelegramUser(options.telegramUser) || !authorityOffices.length;
...
export function simpleExportPresetFromCallback(callbackData = "") {
  const data = String(callbackData || "");
  if (data.startsWith("simple:report:")) {
    const presetKey = data.slice("simple:report:".length);
    const preset = SIMPLE_REPORT_BY_KEY.get(presetKey);
    if (preset && preset.monthMode !== "single") {
      return preset;
    }
    return null;
  }
  if (data.startsWith("simple:month:")) {
    const presetKey = data.split(":")[2] || "";
    const preset = SIMPLE_REPORT_BY_KEY.get(presetKey);
    return preset?.monthMode === "single" ? preset : null;
  }
  return null;
}

export function simpleExportWaitMessage(preset = {}) {
  const label = String(preset?.label || "report").trim() || "report";
  return [`Generating ${label}...`, "Please wait for the Excel file.", "Do not tap other buttons while the report is building."].join(
    "\n",
  );
}

function isExportGenerationLocked(session = {}) {
  return Boolean(session.exportInProgress) && Date.now() - Number(session.exportStartedAt || 0) < EXPORT_LOCK_TTL_MS;
}

export function beginSimpleExportGeneration(userId, preset = {}) {
  setSession(userId, {
    exportInProgress: true,
    exportStartedAt: Date.now(),
    exportPresetKey: String(preset?.key || "").trim(),
  });
}

export function finishSimpleExportGeneration(userId) {
  setSession(userId, {
...
export function isSimpleExportGenerationLocked(session = {}) {
  return isExportGenerationLocked(session);
}

function canUseSimpleBotReports(options = {}) {
  const scope = options.authorityScope;
  // No scope supplied (tests / internal calls): allow — the webhook route is
  // responsible for resolving and passing an authority scope in production.
  if (!scope || isAdminTelegramUser(options.telegramUser)) {
    return true;
  }
  return Boolean(scope.canUseBot);
}

function simpleOfficeKeyboard(offices = []) {
  const rows = chunkButtons(
    offices.map((office) => ({
      text: office,
...
async function buildSimpleQuickExportPayload({
  ...
  // Cache the built workbook so repeated/concurrent identical exports skip the
  // sheet read and Excel rebuild. The key includes a scope signature so
  // restricted users (filtered data) never receive another scope's file.
  const scopeSignature = isAdminTelegramUser(telegramUser)
    ? "all"
...
export async function startMenu(userId, options = {}) {
  ...
  if (isExportGenerationLocked(existing)) {
    return {
      text: simpleExportWaitMessage(SIMPLE_REPORT_BY_KEY.get(existing.exportPresetKey) || { label: "report" }),
  ...
  return {
    ...
    replyMarkup: simpleOfficeKeyboard(availableOffices),
  };
}
...
async function openReportFiltersResponseWithSummary({
  userId,
  month,
  dateConfig,
  telegramUser,
  ...
  infoAgentsTabConfig = getTabConfig("infoAgents"),
  now = new Date(),
  scopeFilters = {},
  permissionFilters = {},
}) {
  const base = openReportFiltersResponse(month, dateConfig, telegramUser, session);
  if (session.last3Mode) {
    return base;
  }
  try {
    const reportData = await readReportDataWithCache(
      userId,
      readRows,
      tabConfig,
      infoAgentsTabConfig,
    ...
    const summary = calculateSummary(reportData.rows || [], tabConfig, { date: dateConfig.filter }, now);
    const text = [
      ...reportFilterHeaderLines(month, dateConfig, session),
      "",
      compactSummaryLine(summary),
      "",
      MAIN_MENU_TEXT,
    ].join("\n");
    return {
...
export async function handleMenuCallback(userId, callbackData, options = {}) {
  ...
  const permissionScopeFilters = permissionScopeFiltersForUser(options.authorityScope, telegramUser);
  const sessionScope = getSession(userId);
  const effectiveScopeFilters = scopeFiltersWithOfficeSelection({}, sessionScope);
  if (!canUseSimpleBotReports(options)) {
    return {
  ...
  const sessionAtStart = getSession(userId);
  const exportPreset = simpleExportPresetFromCallback(callbackData);
  if (isExportGenerationLocked(sessionAtStart) && !exportPreset) {
    return {
      text: simpleExportWaitMessage(SIMPLE_REPORT_BY_KEY.get(sessionAtStart.exportPresetKey) || { label: "report" }),
      replyMarkup: inlineKeyboard([]),
      editCurrentMessage: true,
    };
  }
  ...
  if (callbackData === "simple:office:list") {
    ...
    return {
      text: OFFICE_SCOPE_TEXT,
      replyMarkup: simpleOfficeKeyboard(available),
      editCurrentMessage: true,
    };
  }
  ...
  if (callbackData === "menu:filters" || callbackData === "simple:report:list") {
    ...
    if (!selectedOffice) {
      return {
        text: OFFICE_SCOPE_TEXT,
        replyMarkup: simpleOfficeKeyboard(Array.isArray(session.availableOfficeCountries) ? session.availableOfficeCountries : []),
        editCurrentMessage: true,
      };
    }
  ...
  if (!String(callbackData || "").startsWith("simple:")) {
    return {
      text: "Report flow updated. Please select office and quick report again.",
  ...
  if (callbackData === "menu:filters") {
    ...
    if (session.last3Mode) {
      ...
      return {
        ...openReportFiltersResponse(month, { label: modeLabel, filter: null }, telegramUser, nextSession),
        editCurrentMessage: true,
      };
    }
    ...
    return {
      ...openReportFiltersResponse(month, dateConfig, telegramUser),
      editCurrentMessage: true,
    };
  }
  ...
  if (action === "officeScope") {
    const session = getSession(userId);
    const officeName = decodeURIComponent(String(value || ""));
    const available = Array.isArray(session.availableOfficeCountries) ? session.availableOfficeCountries : [];
    if (!officeName || !available.includes(officeName)) {
      return startMenu(userId, { telegramUser, authorityScope: options.authorityScope });
    }
    const officeMap = await getOfficeMonthMap();
    const officeMonthFiles = officeScopedMonthFiles(officeName, officeMap);
    const selectedOfficeFilters = [];
    const selectedOfficePatterns = officeCountryPatterns(officeCountryFromName(officeName));
    setSession(userId, {
      step: "select_month",
      selectedOfficeCountry: officeName,
      selectedOfficePatterns,
  ...
  if (action === "month") {
    ...
    if (value === "last4") {
      ...
      const exportPayload = await buildLast4AllExportPayload({
        ...
        permissionFilters: permissionScopeFilters,
      });
      if (!exportPayload) {
        return startMenu(userId, { telegramUser, authorityScope: options.authorityScope });
      }
    ...
    return {
      ...
      replyMarkup: dateFilterKeyboard({ isHistorical: month.key < currentMonthKey(now) }),
    };
  }
  ...
  if (action === "date") {
    ...
    return openReportFiltersResponseWithSummary({
      userId,
      month,
      dateConfig,
      telegramUser,
  ...
  if (action === "last4") {
    ...
    if (value === "officeList") {
      ...
      if (!officeValues.length) {
        return {
          text: "No desk data found for Last 4 Months export.",
          replyMarkup: last4ExportActionsKeyboard(false),
        };
      }
      setSession(userId, { last4OfficePage: page });
      return {
        text: "Select desk for Last 4 Months Excel:",
    ...
    if (value === "officePick") {
      ...
      if (!Number.isFinite(pickedIndex) || !officeValues[pickedIndex]) {
        return {
          text: "Desk selection expired. Please pick again.",
      ...
      return {
        ...
        documentCaption: `Last 4 Months Desk Export | Desk: ${office} | ${exportPayload.monthLabel}`,
      };
    }
  }
  ...
  if (action === "report") {
    ...
    if (session.last3Mode && !LAST4_REPORT_TYPES.has(value)) {
      ...
      return {
        text: `${session.monthLabel || "Selected Months"} mode supports only Desk, Team Leader and Agent reports.`,
    ...
    const { rows, targetsMap, infoContext, monthInfoContextByKey = {}, months = [] } = await readReportDataWithCache(
      userId,
      readRows,
      tabConfig,
      infoAgentsTabConfig,
  ...
  if (action === "special") {
    ...
    if (session.last3Mode) {
      ...
      return {
        text: `Specific Reports are disabled in ${session.monthLabel || "Selected Months"} mode. Use Desk, Team Leader or Agent.`,
        replyMarkup: mainMenuKeyboard(telegramUser, { onlyCore: true }),
      };
    }
    const month = selectedMonthRecord(session, now);
    ...
    if (value === "bestOpen") {
      ...
      return {
        text: `Best Performance — ${month.month_label} (${dateConfig.label})\nChoose drill entry point:`,
        replyMarkup: bestPerformersKeyboard(),
      };
    }

    const { rows } = await readReportDataWithCache(userId, readRows, tabConfig, infoAgentsTabConfig, month.sheet_id, {
      scopeFilters: effectiveScopeFilters,
      permissionFilters: permissionScopeFilters,
      now,
    });
    const dateFilters = withDateFilter(effectiveScopeFilters, dateConfig);
    ...
    if (value === "bestCampaigns") {
      const values = bestSelectionValues(rows, tabConfig, dateFilters, "campaign", now);
      setSession(userId, {
        ...commonSession,
        step: "special_reports_menu",
        specialSelection: {
      ...
      return {
        text: "Select campaign to start best-performer drill:",
        replyMarkup: listSelectionKeyboard({
          values,
          page: 0,
          pickPrefix: "specialPick",
    ...
    if (value === "bestCampaignPlacementList") {
      const selectedCampaign = session.specialBestCampaign;
      if (!selectedCampaign) {
        return {
          text: "Select a campaign first from Best Campaign.",
          replyMarkup: bestPerformersKeyboard(),
        };
      }
      const values = bestSelectionValues(
        rows,
        tabConfig,
        applyFieldFilter(dateFilters, "campaign", selectedCampaign),
        "placement",
        now,
      );
      setSession(userId, {
        ...commonSession,
        specialSelection: {
          mode: "bestCampaignPlacement",
          fieldKey: "placement",
          values,
          page: 0,
          backCallback: "special:bestCampaigns",
        },
      });
      return {
        text: `Select placement in ${selectedCampaign}:`,
        replyMarkup: listSelectionKeyboard({
          values,
          page: 0,
          pickPrefix: "specialPick",
    ...
    if (value === "hourlyDate") {
      if (extra === "custom") {
        ...
        return {
          ...
          replyMarkup: inlineKeyboard([
            [{ text: "Back to By Hourly", callbackData: "special:hourly" }],
            [{ text: "Back to Specific Reports", callbackData: "special:open" }],
          ]),
        };
      }
    ...
    if (value === "hourly") {
      ...
      return {
        text: formatHourlyReport(
          `By Hourly (${scopeLabel}) — ${month.month_label} (${dateConfig.label})`,
          distribution,
        ),
  ...
  if (action === "specialPick") {
    ...
    if (selection.mode === "hourly") {
      ...
      return {
        text: formatHourlyReport(
          `By Hourly — ${metricLabelForField(selection.fieldKey)}: ${pickedValue} (${dateConfig.label})`,
          distribution,
        ),
        replyMarkup: hourlyReportKeyboard({
    ...
    if (selection.mode === "bestCampaign") {
      const filters = applyFieldFilter(dateFilters, "campaign", pickedValue);
      const summary = calculateSummary(rows, tabConfig, filters, now);
      const topCountries = rankedPerformanceItems(rows, tabConfig, filters, "country", now);
      ...
      return {
        text: [
          `Best Campaign Explorer — ${pickedValue} (${dateConfig.label})`,
          bestScopeSummaryLine(summary),
          "",
          ...bestRankingLines("Top Countries", topCountries),
          "",
          ...bestRankingLines("Top AFF (Placement)", topPlacements),
        ].join("\n"),
        replyMarkup: bestDetailKeyboard([
          [{ text: `Select Placement in ${pickedValue}`, callbackData: "special:bestCampaignPlacementList" }],
          [{ text: "Choose Another Campaign", callbackData: "special:bestCampaigns" }],
        ]),
      };
    }
    ...
    if (selection.mode === "bestCampaignPlacement") {
      let filters = applyFieldFilter(dateFilters, "campaign", session.specialBestCampaign || "");
      filters = applyFieldFilter(filters, "placement", pickedValue);
      const summary = calculateSummary(rows, tabConfig, filters, now);
      const topSubCampaigns = rankedPerformanceItems(rows, tabConfig, filters, "subCampaign", now);
      setSession(userId, {
      ...
      return {
        text: [
          `Campaign > Placement Drill — ${session.specialBestCampaign} / ${pickedValue} (${dateConfig.label})`,
          bestScopeSummaryLine(summary),
          "",
          ...bestRankingLines("Top Sub-Campaigns", topSubCampaigns),
        ].join("\n"),
        replyMarkup: bestDetailKeyboard([
          [{ text: "Choose Another Placement", callbackData: "special:bestCampaignPlacementList" }],
          [{ text: "Back to Campaign List", callbackData: "special:bestCampaigns" }],
        ]),
      };
    }
  ...
  if (action === "export" && value === "current") {
    ...
    const { rows, targetsMap, infoContext } = await readReportDataWithCache(
      userId,
      readRows,
      tabConfig,
      infoAgentsTabConfig,
  ...
  if (action === "drill") {
    ...
    const rowsForViewCache = new Map();
    const scopedListRenderCache = new Map();
    const scopedDetailRenderCache = new Map();
    const rowsForView = (candidateView = {}) => {
      const filterKey = JSON.stringify(currentFilterSetForView(candidateView) || {});
      if (!rowsForViewCache.has(filterKey)) {
        rowsForViewCache.set(
          filterKey,
          scopeRowsToSelectedDimensions(rows, tabConfig, currentFilterSetForView(candidateView), now),
        );
      }
      return rowsForViewCache.get(filterKey);
    };
    const renderScopedList = (candidateView) => {
      const cacheKey = JSON.stringify({
        title: candidateView.title || "",
        rootType: candidateView.rootType || "",
        groupField: candidateView.groupField,
        page: candidateView.page || 0,
        baseFilters: candidateView.baseFilters || {},
        metricsMode: candidateView.metricsMode || "full",
        monthKeys: (candidateView.monthBreakdownMonths || []).map((item) => item.key),
        hasDimensions: Boolean(candidateView.dimensionOptions?.length),
        backDepth: Array.isArray(candidateView.backStack) ? candidateView.backStack.length : 0,
      });
      if (!scopedListRenderCache.has(cacheKey)) {
        scopedListRenderCache.set(
          cacheKey,
          renderListView(candidateView, rowsForView(candidateView), tabConfig, targetsMap, infoContext, now),
        );
      }
      return scopedListRenderCache.get(cacheKey);
    };
    const renderScopedDetail = (candidateView) => {
      const cacheKey = JSON.stringify({
        title: candidateView.title || "",
        rootType: candidateView.rootType || "",
        groupField: candidateView.groupField,
        selectedLabel: candidateView.selectedLabel || "",
        filters: candidateView.filters || {},
        metricsMode: candidateView.metricsMode || "full",
        monthKeys: (candidateView.monthlyMetrics || []).map((item) => item.monthKey || item.monthLabel),
        backDepth: Array.isArray(candidateView.backStack) ? candidateView.backStack.length : 0,
      });
      if (!scopedDetailRenderCache.has(cacheKey)) {
        scopedDetailRenderCache.set(
          cacheKey,
          renderDetailView(candidateView, rowsForView(candidateView), tabConfig, targetsMap, infoContext, now),
        );
      }
      return scopedDetailRenderCache.get(cacheKey);
    };

    if (value === "multiStart" && view.mode === "list" && MULTI_SELECT_GROUP_FIELDS.has(view.groupField)) {
      const rendered = renderScopedList(view);
      const nextView = enableMultiSelectView(rendered.nextView, rendered.items || []);
      if (!(nextView.multiSelect?.values || []).length) {
        setSession(userId, { view: rendered.nextView });
    ...
    if (value === "multiCancel" && view.multiSelect) {
      ...
      const rendered = renderScopedList(nextView);
      setSession(userId, { view: rendered.nextView });
    ...
    if (value === "multiDone" && view.multiSelect) {
      ...
      const rendered = renderScopedList(filteredView);
      setSession(userId, { view: rendered.nextView });
      return { text: rendered.text, replyMarkup: rendered.replyMarkup, editCurrentMessage: true };
    }
    ...
    if (value === "next" && view.mode === "detail") {
      const requestedFieldKey = extra;
      const allowedNext = DETAIL_NEXT_FIELDS[view.groupField] || [];
      const fieldKey = requestedFieldKey && allowedNext.includes(requestedFieldKey) ? requestedFieldKey : allowedNext[0];
      if (!fieldKey) {
        return {
          text: "No deeper breakdown available.",
          replyMarkup: buildDetailKeyboard(view, view.rootType),
          editCurrentMessage: true,
        };
      }
    ...
    if (value === "listNext" && view.mode === "list") {
      ...
      if (!fieldKey) {
        return {
          text: "No deeper breakdown available.",
          replyMarkup: buildListKeyboard(view, [], 0, view.page || 0, 1),
          editCurrentMessage: true,
        };
      }
      const renderedCurrent = renderScopedList(view);
      const selectedCurrentLevel = selectedFilterValues(
        currentFilterSetForView(view),
        fieldToFilterKey(view.groupField),
      );
    ...
    if (value === "back") {
      ...
      const rendered = previous.mode === "detail"
        ? renderScopedDetail(previous)
        : renderScopedList(previous);
      setSession(userId, { view: rendered.nextView });
      return { text: rendered.text, replyMarkup: rendered.replyMarkup, editCurrentMessage: true };
    }
    ...
    if (value === "pick" && view.mode === "list") {
      ...
      if (nextMode.mode === "detail") {
        ...
        const detail = renderScopedDetail(detailView);
        setSession(userId, { view: detail.nextView });
        return { text: detail.text, replyMarkup: detail.replyMarkup, editCurrentMessage: true };
      }
      ...
      if (nextMode.mode === "dimension") {
        ...
        const breakdownView = createListView({
          rootType: view.rootType,
          title: `${metricLabelForField(view.groupField)}: ${picked.label} — By Desk`,
          groupField: "office",
          baseFilters: parentFilters,
          backStack,
          nextMode: { mode: "detail", fieldKey: "office", label: "Desk" },
        ...
        const list = renderScopedList(breakdownView);
        ...
        return { text: list.text, replyMarkup: list.replyMarkup, editCurrentMessage: true };
      }
      ...
      if (nextMode.mode === "list") {
        ...
        const list = renderScopedList(nextView);
        const autoMulti = renderedListResponseWithAutoMulti(list);
        setSession(userId, { view: autoMulti?.nextView || list.nextView });
        if (autoMulti) {
          return {
...
export async function handleMenuText(userId, text, options = {}) {
  const session = getSession(userId);
  if (isExportGenerationLocked(session)) {
    return {
      text: simpleExportWaitMessage(SIMPLE_REPORT_BY_KEY.get(session.exportPresetKey) || { label: "report" }),
      replyMarkup: inlineKeyboard([]),
    };
  }
  const telegramUser = options.telegramUser;
  ...
  const now = options.now || new Date();
  const permissionScopeFilters = permissionScopeFiltersForUser(options.authorityScope, telegramUser);
  const effectiveScopeFilters = scopeFiltersWithOfficeSelection({}, getSession(userId));
  if (!canUseSimpleBotReports(options)) {
    return {
      text: "Report request is available only for users with ALL authority.",
      replyMarkup: inlineKeyboard([[{ text: "Back", callbackData: "root:start" }]]),
  ...
  return openReportFiltersResponseWithSummary({
    userId,
    month,
    dateConfig: customDate,
    telegramUser,
    session: nextSession,
    readRows,
    tabConfig: options.tabConfig || getTabConfig("leads"),
    infoAgentsTabConfig: options.infoAgentsTabConfig || getTabConfig("infoAgents"),
    now,
    scopeFilters: effectiveScopeFilters,
    permissionFilters: permissionScopeFilters,
  });
