...
const SIMPLE_QUICK_REPORTS = [
  ...
  { key: "target-result", label: "Target Result", monthMode: "single" },
  { key: "team-roster", label: "Team Roster", monthMode: "single" },
...
function simplePresetQuery({ presetKey = "", office = "", monthKey = "", officeMonthKeys = [] } = {}) {
  ...
  if (presetKey === "traffic-priority") {
    ...
    // Load the 2 most recent months so a big office stays under the timeout.
    query.monthKey = selectedAllMonths.slice(0, 2).join(",");
    query.trafficPriority = "1";
    query.rowDimensions = "country,campaign,agent";
    query.metricFields = "leads,ftd,cr";
    return query;
  }
  if (presetKey === "target-result") {
    query.includeWorkTime = "1";
    query.hideNotWorking = "0";
    query.targetResult = "1";
    query.rowDimensions = "desk,teamLeader,agent";
    query.metricFields = "ftdTarget,ftd,ftdTargetReach";
    return query;
  }
  if (presetKey === "team-roster") {
    query.teamRoster = "1";
    query.rowDimensions = "";
    query.metricFields = "";
    return query;
