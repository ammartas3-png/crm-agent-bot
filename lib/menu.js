...
export async function startMenu(userId, options = {}) {
  ...
  if (officeScope.countries.length >= 1) {
    setSession(userId, {
  ...
  const officeMonthFiles = selectedOfficeCountry ? officeScopedMonthFiles(selectedOfficeCountry, officeScope) : [];
  const selectedOfficeFilters = [];
  const selectedOfficePatterns = selectedOfficeCountry
    ? officeCountryPatterns(officeCountryFromName(selectedOfficeCountry))
    : [];
  setSession(userId, {
    step: "select_month",
    selectedOfficeCountry,
  ...
  return {
    text: selectedOfficeCountry ? `Office: ${selectedOfficeCountry}\n${MONTH_MENU_TEXT}` : MONTH_MENU_TEXT,
