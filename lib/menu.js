...
function buildAgentDirectoryMap(agentDirectoryRows = [], agentDirectoryTabConfig) {
  ...
  for (const row of agentDirectoryRows) {
    ...
    const agentName = String(getRowValue(row, agentNameField) || getRowValue(row, "Agent Name") || "").trim();
...
function remapRowsToCurrentInfo(rows = [], tabConfig, infoContext, options = {}) {
  ...
  return rows.map((row) => {
    ...
    const nameFromCurrentById = normalizedAgentId ? currentAgentNameById.get(normalizedAgentId) : "";
    const nameFromPreviousMonths = normalizedAgentId
      ? agentNameFromPreviousMonthsById(
          normalizedAgentId,
