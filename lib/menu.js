...
const LAST3_REPORT_TYPES = new Set(["office", "teamLeader", "agent"]);
...
function monthKeyboard(telegramUser) {
  ...
  rows.push([{ text: "Last 3 Months", callbackData: "month:last3" }]);
...
function reportFilterTitle(month, dateConfig, session = {}) {
  const monthLabel = session.last3Mode ? session.monthLabel || "Last 3 Months" : month.month_label;
...
function getLastThreeMonthRecords(now = new Date()) {
...
function currentInfoMonthRecord(last3Records = [], now = new Date()) {
  const current = getMonthFile(currentMonthKey(now), { includeInactive: false });
  if (current) {
    return current;
  }
  return last3Records[0] || null;
}
    ...
    if (infoMonth) {
      try {
        infoAgentRows = await readRows("infoAgents", {
          ...
          spreadsheetId: infoMonth.sheet_id,
        });
      } catch {
        infoAgentRows = [];
      }
...
function formatMetricLine(label, metrics, mode = "full") {
  if (mode === "last3") {
    return [
      ...
      `CR ${metrics.lead > 0 ? formatPercent(metrics.cr) : "-"}`,
      `CR Target Reach ${
...
function renderDetailView(view, rows, tabConfig, targetsMap, infoContext, now = new Date()) {
  ...
  return {
    text: formatMetricBlock(view.title, metrics, view.metricsMode),
...
function openReportFiltersResponse(month, dateConfig, telegramUser, session = {}) {
  return {
    ...
    replyMarkup: mainMenuKeyboard(telegramUser, { onlyCore: Boolean(session.last3Mode) }),
  };
}
...
export async function handleMenuCallback(userId, callbackData, options = {}) {
  ...
  if (callbackData === "menu:filters") {
    ...
    if (session.last3Mode) {
      const nextSession = {
        ...
        last3Mode: true,
        monthLabel: session.monthLabel || "Last 3 Months",
        last3MonthKeys: session.last3MonthKeys || getLastThreeMonthRecords(now).map((item) => item.key),
        dateFilter: null,
        dateFilterLabel: "Last 3 Months",
        dateFilterKey: "last3",
        step: "select_report_type",
        view: null,
      };
      setSession(userId, nextSession);
      return openReportFiltersResponse(month, { label: "Last 3 Months", filter: null }, telegramUser, nextSession);
    }
  ...
  if (action === "month") {
    if (value === "last3") {
      const months = getLastThreeMonthRecords(now);
      if (months.length === 0) {
        return {
      ...
      const label = `Last 3 Months (${oldest.month_label} - ${latest.month_label})`;
      const nextSession = {
      ...
      setSession(userId, nextSession);
      return openReportFiltersResponse(latest, { label: "Last 3 Months", filter: null }, telegramUser, nextSession);
    }
  ...
  if (action === "report") {
    ...
    const session = getSession(userId);
    if (session.last3Mode && !LAST3_REPORT_TYPES.has(value)) {
      const month = selectedMonthRecord(session, now);
      if (!month) {
        return startMenu(userId, { telegramUser });
      }
      return {
        text: "Last 3 Months mode supports only Office, Team Leader and Agent reports.",
    ...
    const effectiveDateConfig = session.last3Mode
      ? { label: "Last 3 Months", filter: null, key: "last3" }
      : dateConfig;
    ...
    const view = createRootView(value, displayMonthLabel, effectiveDateConfig, {
      metricsMode: session.last3Mode ? "last3" : "full",
    });
  ...
  if (action === "special") {
    ...
    if (session.last3Mode) {
      ...
      return {
        text: "Specific Reports are disabled in Last 3 Months mode. Use Office, Team Leader or Agent.",
        replyMarkup: mainMenuKeyboard(telegramUser, { onlyCore: true }),
      };
    }
    const month = selectedMonthRecord(session, now);
