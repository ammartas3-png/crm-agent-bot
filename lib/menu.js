...
function juneMonthRecordForJobEntry(session = {}) {
  const months = sessionMonthRecords(session)
    .filter((item) => item.active !== false)
    .sort((left, right) => String(right.key || "").localeCompare(String(left.key || "")));
  return months.find((month) => String(month.key || "").endsWith("-06")) || null;
}
...
async function readLast4JobEntryMap(readRows, options = {}) {
  ...
  const juneMonth = juneMonthRecordForJobEntry(session);
  if (juneMonth?.sheet_id) {
    ...
    try {
      const rows = await readRows("infoAgents", { tabConfig, spreadsheetId: juneMonth.sheet_id });
      const map = buildLast4JobEntryMap(rows);
      if (map.size > 0) {
        return map;
      }
    } catch {
      // Fall back to previous Turkey No1 onboarding table source.
    }
  }
...
export async function startMenu(userId, options = {}) {
  ...
  const selectedOfficeFilters = selectedOfficeCountry ? officeScope.officesByCountry[selectedOfficeCountry] || [] : [];
...
export async function handleMenuCallback(userId, callbackData, options = {}) {
  ...
  if (action === "officeScope") {
    ...
    const selectedOfficeFilters = officeMap.officesByCountry?.[country] || [];
