...
async function readReportData(readRows, tabConfig, infoAgentsTabConfig, spreadsheetId, options = {}) {
  ...
  const rows = applyAgentNameAliases(
    scopedRowsByFilters(scopedRows, tabConfig, scopeFilters, scopeNow),
    tabConfig,
  ).map((row) => ({
    ...row,
    __scopeOfficeName: scopeOfficeName || String(row.__scopeOfficeName || "").trim(),
...
export async function handleMenuCallback(userId, callbackData, options = {}) {
  ...
  const effectiveScopeFilters = scopeFiltersWithOfficeSelection(scopedAuthorityFilters, sessionScope);
