...
import { isLegacyOffice, legacyMonthRecordsForOffice, resolveLast4MonthKeysForOffice } from "./legacyOfficeData.js";
...
function officeScopedMonthFiles(officeName = "", officeMap = {}) {
  const resolvedOfficeName = resolveOfficeNameForMonthMap(officeName, officeMap);
  const scoped = Array.isArray(officeMap?.byOffice?.[resolvedOfficeName]) ? officeMap.byOffice[resolvedOfficeName] : [];
  ...
  for (const record of scoped) {
    ...
    mergedByKey.set(key, {
      ...record,
      office_name: String(record?.office_name || resolvedOfficeName || "").trim(),
    });
  }
  ...
  for (const record of listMonthFiles({ includeInactive: false })) {
    ...
    mergedByKey.set(key, {
      ...record,
      office_name: String(resolvedOfficeName || record?.office_name || "").trim(),
    });
  }
  if (isLegacyOffice(resolvedOfficeName)) {
    for (const record of legacyMonthRecordsForOffice(resolvedOfficeName)) {
      if (!mergedByKey.has(record.key)) {
        mergedByKey.set(record.key, record);
      }
    }
  }
...
function resolveOfficeNameForMonthMap(officeName = "", officeMap = {}) {
  const trimmed = String(officeName || "").trim();
  if (!trimmed) {
    return "";
  }
  if (Array.isArray(officeMap?.byOffice?.[trimmed])) {
    return trimmed;
  }
  const normalized = normalizeText(trimmed);
  const directOffice = Object.keys(officeMap?.byOffice || {}).find((name) => normalizeText(name) === normalized);
  if (directOffice) {
    return directOffice;
  }
  const countryOffices = officeMap?.officesByCountry?.[trimmed];
  if (Array.isArray(countryOffices) && countryOffices.length === 1) {
    return countryOffices[0];
  }
  const matchedCountry = Object.entries(officeMap?.officesByCountry || {}).find(
    ([country]) => normalizeText(country) === normalized,
  );
  if (matchedCountry?.[1]?.length === 1) {
    return matchedCountry[1][0];
  }
  return trimmed;
}
...
function canUseSimpleBotReports(options = {}) {
  ...
  // A resolved scope may still be allowed but restricted (filtered) access.
...
function simplePresetQuery({ presetKey = "", office = "", monthKey = "", officeMonthKeys = [], officeMonthFiles = [] } = {}) {
  ...
  if (presetKey === "last4") {
    const officeLiveMonthKeys = (officeMonthFiles || [])
      .filter((item) => String(item?.sheet_id || "").trim())
      .map((item) => String(item?.key || "").trim())
      .filter(Boolean);
    query.monthKey = resolveLast4MonthKeysForOffice(office, officeLiveMonthKeys).join(",");
