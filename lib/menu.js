...
import { logBotActivity } from "./activityLogService.js";
...
export async function startMenu(userId, options = {}) {
  ...
  logBotActivity(options.telegramUser, "open_quick_reports", {
    offices: availableOffices.join(", "),
  });
  return {
...
export async function handleMenuCallback(userId, callbackData, options = {}) {
  ...
  if (String(callbackData || "").startsWith("simple:report:")) {
    ...
    logBotActivity(telegramUser, "quick_report", {
      presetKey,
      office: resolvedOffice,
      month: payload.caption || "auto",
      metrics: simplePresetQuery({
        presetKey,
        office: resolvedOffice,
        monthKey: "",
        officeMonthKeys: officeMonthFiles.map((item) => String(item?.key || "").trim()).filter(Boolean),
      }).metricFields,
    });
    setSession(userId, {
      step: "simple_select_report",
