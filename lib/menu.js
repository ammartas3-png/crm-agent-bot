...
function withHourFilter(baseFilters = {}, hourRange = null) {
  if (!hourRange) {
    return { ...baseFilters };
  }
  return { ...baseFilters, hourRange };
}
...
function parseHourRangeInput(text) {
  ...
  const start = parseHourPart(parts[0]);
  const end = parseHourPart(parts[1]);
  ...
  return {
    ...
    label: `${String(start).padStart(2, "0")}:00-${String(end).padStart(2, "0")}:59`,
  };
}
...
function hourlyReportKeyboard(options = {}) {
  ...
  rows.push([
    { text: "All Hours", callbackData: "special:hours:all" },
    { text: "Set Hour Range", callbackData: "special:hours:custom" },
  ]);
...
export async function handleMenuCallback(userId, callbackData, options = {}) {
  ...
  if (action === "special") {
    ...
    const commonSession = {
      ...
      dateFilterLabel: dateConfig.label,
      dateFilterKey: dateConfig.key,
      hourRange: session.hourRange || null,
      hourRangeLabel: session.hourRangeLabel || null,
    };
    ...
    if (value === "hours") {
      if (extra === "all") {
        setSession(userId, { hourRange: null, hourRangeLabel: null });
        return handleMenuCallback(userId, "special:hourly", options);
      }
      if (extra === "custom") {
        setSession(userId, {
          ...commonSession,
          step: "select_hour_custom",
        });
        return {
          text: "Send hour range as:\nHH:MM - HH:MM (example: 09:00 - 13:00)\nYou can also send: 9 - 13",
          replyMarkup: inlineKeyboard([
    ...
    const temporalFilters = withHourFilter(dateFilters, session.hourRange);

    if (value === "hourly") {
      const currentScope = session.hourlyScope || null;
      const scopeFilters =
        currentScope?.fieldKey && currentScope?.value
          ? applyFieldFilter(temporalFilters, currentScope.fieldKey, currentScope.value)
          : temporalFilters;
      ...
      const scopeLabel = currentScope?.fieldKey
        ...
        : "All";
      const hourLabel = session.hourRangeLabel || "All Hours";
      setSession(userId, {
        ...commonSession,
        step: "special_reports_menu",
        specialSelection: null,
      ...
      return {
        text: formatHourlyReport(
          `Hourly Leads (${scopeLabel}) — ${month.month_label} (${dateConfig.label}) [${hourLabel}]`,
          distribution,
        ),
    ...
    if (value === "hourlyScope") {
      ...
      const values = specialScopedValues(rows, tabConfig, fieldKey, temporalFilters, now);
      setSession(userId, {
        ...commonSession,
        step: "special_reports_menu",
        specialSelection: {
      ...
      return {
        text: `Select ${metricLabelForField(fieldKey)} for hourly lead report (${session.hourRangeLabel || "All Hours"}):`,
    ...
    if (value === "compareCountry") {
      const values = specialScopedValues(rows, tabConfig, "country", temporalFilters, now);
      setSession(userId, {
        ...commonSession,
        step: "special_reports_menu",
        specialSelection: {
    ...
    if (value === "compareCampaignList") {
      ...
      const values = specialScopedValues(
        ...
        tabConfig,
        "campaign",
        { ...temporalFilters, country: selectedCountry },
        now,
      );
      setSession(userId, {
        ...commonSession,
  ...
  if (action === "specialPick") {
    ...
    if (selection.mode === "hourly") {
      const scopeFilters = applyFieldFilter(temporalFilters, selection.fieldKey, pickedValue);
      const distribution = hourlyDistribution(rows, tabConfig, scopeFilters, "created", "totalLeads", now);
      setSession(userId, {
      ...
      return {
        text: formatHourlyReport(
          `Hourly Leads — ${metricLabelForField(selection.fieldKey)}: ${pickedValue} (${dateConfig.label}) [${
            session.hourRangeLabel || "All Hours"
          }]`,
          distribution,
        ),
        replyMarkup: hourlyReportKeyboard({
    ...
    if (selection.mode === "compareCountry") {
      const filters = { ...temporalFilters, country: pickedValue };
      const lines = [
...
export async function handleMenuText(userId, text, options = {}) {
  ...
  if (session.step === "select_hour_custom") {
    const parsed = parseHourRangeInput(text);
    if (!parsed) {
      return {
        text: "Invalid hour range. Send as:\nHH:MM - HH:MM (example: 09:00 - 13:00)\nOr: 9 - 13",
        replyMarkup: inlineKeyboard([
          [{ text: "Back to Hourly Leads", callbackData: "special:hourly" }],
          [{ text: "Back to Specific Reports", callbackData: "special:open" }],
        ]),
    ...
    setSession(userId, {
      hourRange: { start: parsed.start, end: parsed.end },
      hourRangeLabel: parsed.label,
      step: "special_reports_menu",
    });
    return handleMenuCallback(userId, "special:hourly", options);
  }
