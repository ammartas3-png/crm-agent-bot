...
import { dateFilterToRange, loadLeadRows } from "./dataProvider.js";
import { clearSession, getSession, setSession } from "./session.js";

function scopeForSession(session, now) {
  return dateFilterToRange(session?.dateRange?.filter, now);
}
...
export function formatReport(session, rows, tabConfig, now = new Date(), options = {}) {
  ...
  const report = generateReport({
    ...
    extraFilters: reportType?.filterExtra || {},
    now,
  });
  return formatSummary(reportTitle(session), report.dateRangeLabel, report.summary, options);
}
...
export async function handleMenuCallback(userId, callbackData, options = {}) {
  ...
  const readRows = options.readRows || loadLeadRows;
  ...
  if (action === "date") {
    ...
    const rows = await readRows("leads", { tabConfig, scope: scopeForSession(nextSession, now) });
  ...
  if (action === "breakdown") {
    const session = getSession(userId);
    const rows = await readRows("leads", { tabConfig, scope: scopeForSession(session, now) });
    return {
