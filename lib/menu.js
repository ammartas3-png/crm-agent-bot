...
const MULTI_SELECT_GROUP_FIELDS = new Set(["office", "teamLeader"]);
...
function monthMultiKeyboard(selectedKeys = []) {
  const selected = new Set(selectedKeys);
  const rows = listMonthFiles().map((month) => [
    {
...
async function readReportData(readRows, tabConfig, infoAgentsTabConfig, spreadsheetId, options = {}) {
  ...
  const rows = applyAgentNameAliases(await readRows("leads", { tabConfig, spreadsheetId }), tabConfig);
  let infoAgentRows = [];
  try {
...
function hierarchyFromScope({ groupField, label = "", filters = {}, infoContext } = {}) {
  ...
  let office = firstFilterValue(filters.office);
  let teamLeader = firstFilterValue(filters.teamLeader);
  let agent = firstFilterValue(filters.agent);
  let country = firstFilterValue(filters.country);
  let campaign = firstFilterValue(filters.campaign);
  let placement = firstFilterValue(filters.placement);
  let subCampaign = firstFilterValue(filters.subCampaign);
...
function buildListKeyboard(view, pageChunk, start, page, totalPages) {
  ...
  if (view.mode === "list" && MULTI_SELECT_GROUP_FIELDS.has(view.groupField) && pageChunk.length > 0) {
    rows.push([{ text: "Select Multiple", callbackData: "drill:multiStart" }]);
  }
...
function workbookRowsForListView(view, rows, tabConfig, targetsMap, infoContext, now = new Date()) {
  ...
  const outputRows = [
    {
      kind: "summary",
      level: "Summary",
      office: summaryHierarchy.office,
      teamLeader: summaryHierarchy.teamLeader,
      agent: summaryHierarchy.agent,
      country: summaryHierarchy.country,
      campaign: summaryHierarchy.campaign,
      placement: summaryHierarchy.placement,
      subCampaign: summaryHierarchy.subCampaign,
      item: "Summary (all records)",
  ...
  for (const item of items) {
    ...
    outputRows.push({
      ...
      office: itemHierarchy.office,
      teamLeader: itemHierarchy.teamLeader,
      agent: itemHierarchy.agent,
      country: itemHierarchy.country,
      campaign: itemHierarchy.campaign,
      placement: itemHierarchy.placement,
      subCampaign: itemHierarchy.subCampaign,
      item: item.label,
      name: item.label,
      month: "",
  ...
  return [
    {
      kind: "group",
      level: metricLabelForField(view.groupField),
      office: detailHierarchy.office,
      teamLeader: detailHierarchy.teamLeader,
      agent: detailHierarchy.agent,
      country: detailHierarchy.country,
      campaign: detailHierarchy.campaign,
      placement: detailHierarchy.placement,
      subCampaign: detailHierarchy.subCampaign,
...
export async function handleMenuCallback(userId, callbackData, options = {}) {
  ...
  if (action === "month") {
    if (value === "multi") {
      ...
      return {
        text: "Select one or more months:",
        replyMarkup: monthMultiKeyboard([]),
      };
    }
    if (value === "last4") {
  ...
  if (action === "monthMulti") {
    const currentSelection = Array.isArray(getSession(userId).monthMultiSelected)
      ? [...new Set(getSession(userId).monthMultiSelected)]
      : [];
    if (value === "toggle") {
      const key = extra;
      const month = getMonthFile(key, { includeInactive: false });
      if (!month) {
        return {
          text: "Month not found. Please select again.",
          replyMarkup: monthMultiKeyboard(currentSelection),
        };
      }
      ...
      return {
        text: `Selected ${nextSelection.length} month(s).`,
        replyMarkup: monthMultiKeyboard(nextSelection),
      };
    }
    if (value === "all") {
      const all = listMonthFiles()
        .map((item) => item.key)
        .sort((left, right) => right.localeCompare(left));
      setSession(userId, { monthMultiSelected: all, step: "select_month_multi" });
      return {
        text: `Selected ${all.length} month(s).`,
        replyMarkup: monthMultiKeyboard(all),
      };
    }
    if (value === "clear") {
    ...
    if (value === "done") {
      ...
      setSession(userId, nextSession);
      return openReportFiltersResponse(latest, { label, filter: null }, telegramUser, nextSession);
    }
  }
  ...
  if (action === "export" && value === "current") {
    ...
    const workbookBuffer = await buildReportWorkbookBuffer({
      ...
      context: {
        ...
        filters: session.view.filters || session.view.baseFilters || {},
      },
    });
  ...
  if (action === "drill") {
    ...
    if (value === "multiStart" && view.mode === "list" && MULTI_SELECT_GROUP_FIELDS.has(view.groupField)) {
      const rendered = renderListView(view, rows, tabConfig, targetsMap, infoContext, now);
      const values = (rendered.items || []).map((item) => item.label).filter(Boolean);
      if (!values.length) {
        setSession(userId, { view: rendered.nextView });
      ...
      return { text: multiSelectPrompt(nextView), replyMarkup: buildMultiSelectKeyboard(nextView) };
    }
    ...
    if (value === "multiDone" && view.multiSelect) {
      ...
      if (view.groupField === "office" && view.nextMode?.mode === "list" && view.nextMode.fieldKey === "teamLeader") {
        ...
        const nextView = createListView({
          ...
          nextMode:
            view.nextMode.fieldKey === "agentNames"
              ? { mode: "detail", fieldKey: "agentNames", label: "Agent" }
              : { mode: "list", fieldKey: "agentNames", label: "Agents" },
          metricsMode: view.metricsMode,
          monthBreakdownMonths: view.monthBreakdownMonths || [],
          monthContextByKey: view.monthContextByKey || {},
        });
