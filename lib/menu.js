...
function targetForSummary(summary, groupField, selectedLabel, tabConfig, targetsMap, infoContext) {
  if (groupField === "agentNames") {
    return infoContext ? agentTarget(infoContext.targetsByAgent, selectedLabel) : agentTarget(targetsMap, selectedLabel);
  }
  if (groupField === "teamLeader") {
    const target = targetForTeamLeader(infoContext, selectedLabel);
    if (target > 0) {
      return target;
    }
  }
  if (groupField === "office") {
    const target = targetForOffice(infoContext, selectedLabel);
    if (target > 0) {
      return target;
    }
  }
...
function metricPayload(summary, groupField, selectedLabel, tabConfig, targetsMap, infoContext) {
  const ftdTarget = targetForSummary(
    summary,
    groupField,
    selectedLabel,
    tabConfig,
  ...
  return [...groups.entries()]
    .map(([key, groupRows]) => {
      ...
      return {
        label,
        summary,
        metrics: metricPayload(summary, groupField, label, tabConfig, targetsMap, infoContext),
      };
    })
    .sort(
      (left, right) =>
