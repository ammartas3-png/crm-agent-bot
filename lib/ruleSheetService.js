...
export const STATUS_HINT_SHEET_NAME = "Sheet1";
...
function detectStatusHintColumns(headers = []) {
  const normalizedHeaders = headers.map((header) => normalizeKey(header));
  const indexByPattern = (patterns, fallback) => {
  ...
  return {
    status: indexByPattern(["status"], 0),
    description: indexByPattern(["description", "rule", "aciklama", "açıklama"], 1),
  };
}
...
function statusHintFromRow(row = [], columns, rowNumber) {
  const status = canonicalStatusName(row[columns.status]);
...
export async function listStatusHintsFromSheet(options = {}) {
  const sheets = options.sheetsClient || createWriteClient();
  const spreadsheetId = options.spreadsheetId || RULE_SPREADSHEET_ID;
  const sheetName = options.sheetName || STATUS_HINT_SHEET_NAME;
  const range = `'${sheetName}'!A:Z`;
  const response = await sheets.spreadsheets.values.get({
    spreadsheetId,
    range,
  ...
  const firstRowColumns = { status: 0, description: 1 };
  const firstRowHint = statusHintFromRow(values[0], firstRowColumns, 1);
  if (firstRowHint) {
    return values
      .map((row, index) => statusHintFromRow(row, firstRowColumns, index + 1))
      .filter(Boolean);
  }

  const headers = values[0] || [];
  const columns = detectStatusHintColumns(headers);
  return values
    .slice(1)
    .map((row, index) => statusHintFromRow(row, columns, index + 2))
    .filter(Boolean);
}
