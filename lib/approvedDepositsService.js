...
async function loadAllKycLanguageMaps({ kycSources = [], readValues, getTitles } = {}) {
  ...
  for (const source of kycSources) {
    ...
    kycOfficeSources.push({
      ...
      matchedCount: officeSource.languageByAccId.size,
    });
    ...
    for (const [accId, record] of officeSource.languageByAccId.entries()) {
      if (!languageByAccId.has(accId)) {
        languageByAccId.set(accId, record);
      }
    }
  }
