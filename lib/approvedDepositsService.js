...
import { categorizeKycLanguage, parseKycLanguage } from "./kycTextParser.js";
...
const MONTH_TAB_TITLE_PATTERN = /^(january|february|march|april|may|june|july)(?:\d{2,4})?$/;
...
export function normalizeApprovedDepositAccId(value = "") {
  ...
  const compact = raw.replace(/\s+/g, "").toLocaleUpperCase("en-US");
  if (/^ACC\d+$/.test(compact)) {
    return compact;
  }
  const digits = compact.match(/\d+/)?.[0] || "";
  return digits ? `ACC${digits}` : compact;
}
...
function kycLanguageRecord(row = [], headerMap) {
  const country = rowCountry(row, headerMap);
  const kycText = String(valueAt(row, headerMap, ["kyc"]) || "");
  const language = parseKycLanguage(kycText);
  return {
    cid: normalizeApprovedDepositAccId(valueAt(row, headerMap, ["cid", "customerid", "id"])),
    country,
    language,
  };
}

function amountRecord(row = [], headerMap, languageByAccId = new Map()) {
  ...
  const country = titleCase(valueAt(row, headerMap, AMOUNT_COLUMN_ALIASES.country)) || "Unknown";
  const language = languageByAccId.get(accId)?.language || "";
  return {
    cid: accId,
    accId,
    ...
    languageCategory: categorizeKycLanguage({ country, language }),
    month,
    monthKey: month.key,
  };
}
...
async function loadKycLanguageMap({ spreadsheetId, readValues, sheetTitles } = {}) {
  const validTabs = validApprovedDepositTabTitles(sheetTitles || []);
  const languageByAccId = new Map();
  ...
  for (const title of validTabs) {
    ...
    for (const row of values.slice(headerRowIndex + 1)) {
      const record = kycLanguageRecord(row, headerMap);
      if (record.cid && record.language && !languageByAccId.has(record.cid)) {
        languageByAccId.set(record.cid, record);
      }
    }
  }
...
async function loadAmountRows({ spreadsheetId, readValues, sheetTitles, languageByAccId } = {}) {
  ...
  for (const title of sourceTabs) {
    ...
    const headerMap = headerMapFromRow(values[headerRowIndex]);
    for (const row of values.slice(headerRowIndex + 1)) {
      const record = amountRecord(row, headerMap, languageByAccId);
...
export async function loadApprovedDepositsReport(query = {}, options = {}) {
  const kycSpreadsheetId = options.kycSpreadsheetId || options.spreadsheetId || APPROVED_DEPOSITS_KYC_SPREADSHEET_ID;
  ...
  const kycTitles = Array.isArray(options.kycSheetTitles)
    ? options.kycSheetTitles
    : Array.isArray(options.sheetTitles)
      ? options.sheetTitles
      : await getSheetTitles(kycSpreadsheetId);
  ...
  const kycSource = await loadKycLanguageMap({
    spreadsheetId: kycSpreadsheetId,
    readValues,
    sheetTitles: kycTitles,
  });
  const amountSource = await loadAmountRows({
    spreadsheetId: amountSpreadsheetId,
    readValues,
    sheetTitles: amountTitles,
    languageByAccId: kycSource.languageByAccId,
  });
