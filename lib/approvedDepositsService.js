...
const MONTHS = [
  ...
  { key: "august", title: "AUGUST", label: "Aug" },
  { key: "september", title: "SEPTEMBER", label: "Sep" },
  { key: "october", title: "OCTOBER", label: "Oct" },
  { key: "november", title: "NOVEMBER", label: "Nov" },
  { key: "december", title: "DECEMBER", label: "Dec" },
];
const MONTH_BY_NORMALIZED_TITLE = new Map(MONTHS.map((month, index) => [normalizeText(month.title), { ...month, index }]));
const MONTH_TAB_TITLE_PATTERN = new RegExp(
  `^(${MONTHS.map((month) => month.key).join("|")})(?:\\d{2,4})?$`,
);
...
export function parseApprovedDepositMonthTab(title = "") {
  const normalized = normalizeText(title).replace(/\s+/g, "");
  if (!normalized || normalized.includes("self")) {
    return null;
  }
  const match = normalized.match(MONTH_TAB_TITLE_PATTERN);
  if (!match) {
    return null;
  }
  return MONTH_BY_NORMALIZED_TITLE.get(match[1]) || null;
}

function isMonthTab(title = "") {
  return Boolean(parseApprovedDepositMonthTab(title));
}

function tabMeta(title = "") {
  const month = parseApprovedDepositMonthTab(title);
...
function findHeaderRow(values = []) {
  for (let index = 0; index < Math.min(values.length, 12); index += 1) {
    ...
    if ((hasKyc && (hasCid || hasCountry)) || (hasCid && hasCountry)) {
      return index;
    }
  }
  return -1;
...
export function normalizeApprovedDepositCountry(value = "") {
  return normalizeKycCountry(value);
}
...
function countriesMatch(left = "", right = "") {
  return normalizeText(normalizeApprovedDepositCountry(left)) === normalizeText(normalizeApprovedDepositCountry(right));
}

function kycRecordIdentity(record = {}) {
  return [
    normalizeApprovedDepositAccId(record.cid),
    normalizeText(normalizeApprovedDepositCountry(record.country)),
    normalizeText(record.language || ""),
  ].join("|");
}

function dedupeKycMatches(matches = []) {
  const seen = new Set();
  const output = [];
  for (const match of matches) {
    const key = kycRecordIdentity(match);
    if (!seen.has(key)) {
      seen.add(key);
      output.push(match);
    }
...
function pickBestKycMatch(matches = [], { country = "" } = {}) {
  const unique = dedupeKycMatches(matches);
  if (unique.length === 1) {
    return unique[0];
  }
  const countryMatches = unique.filter((match) => countriesMatch(match.country, country));
  if (countryMatches.length === 1) {
    return countryMatches[0];
  }
  const dedupedCountryMatches = dedupeKycMatches(countryMatches);
  if (dedupedCountryMatches.length === 1) {
    return dedupedCountryMatches[0];
  }
  return null;
}
...
function lookupInOfficeLanguageIndex(officeIndex, { accId = "", country = "" } = {}) {
  ...
  return pickBestKycMatch(candidates, { country: normalizedCountry });
}
...
export function lookupKycLanguageRecord(index, { accId = "", country = "", office = "", offices = [] } = {}) {
  ...
  return pickBestKycMatch(matches, { country: normalizedCountry });
}
...
function kycLanguageRecord(row = [], headerMap, office = "") {
  ...
  let kycText = String(valueAt(row, headerMap, ["kyc"]) || "");
  let languages = parseKycLanguageParts(kycText);
  if (!languages.length) {
    const rowText = (Array.isArray(row) ? row : [])
      .map((cell) => String(cell ?? "").trim())
      .filter(Boolean)
      .join("\n");
    const scanned = parseKycLanguageParts(rowText);
    if (scanned.length) {
      languages = scanned;
      if (!kycText.trim()) {
        kycText = rowText;
  ...
  const columnCid = normalizeApprovedDepositAccId(valueAt(row, headerMap, ["cid", "customerid", "id"]));
  const cid = columnCid || normalizeApprovedDepositAccId(kycText);
  return {
    cid,
    country,
    language,
...
export function otherLanguageReason(row = {}) {
  const language = String(row.language || "").trim();
  const kycOffice = String(row.kycOffice || "").trim();
  const country = normalizeApprovedDepositCountry(row.country);
  if (!language || language === "Unknown") {
    return kycOffice ? "KYC matched but language missing" : "No KYC match";
  }
  if (normalizeText(language) === normalizeText(NATIVE_LANGUAGE_PLACEHOLDER)) {
    return country === "Unknown" ? "Native language but country unknown" : "Native language not resolved";
  }
  return `Unmapped language: ${language}`;
}

export function buildOtherLanguageAudit(rows = [], months = []) {
  const monthOrder = (Array.isArray(months) ? months : []).map((month) => ({
    key: String(month?.key || "").trim(),
    label: String(month?.label || month?.key || "").trim(),
  }));
  const columns = monthOrder.map((month) => {
    const grouped = new Map();
    for (const row of rows) {
      if (row?.languageCategory !== "Other") {
        continue;
      }
      if (String(row?.monthKey || "") !== month.key) {
        continue;
      }
      const cid = String(row?.accId || row?.cid || "").trim();
      if (!cid) {
        continue;
      }
      ...
      if (existing) {
        ...
        if (!existing.brand && row.brand) {
          existing.brand = String(row.brand || "").trim();
        }
      ...
      grouped.set(cid, {
        ...
        country: String(row.country || "").trim() || "Unknown",
        language: String(row.language || "").trim() || "Unknown",
        kycOffice: String(row.kycOffice || "").trim(),
        amount: Number(row.amount || 0),
        count: 1,
        reason: otherLanguageReason(row),
      });
    }
...
export async function loadApprovedDepositsReport(query = {}, options = {}) {
  ...
  return {
    ...
    otherLanguageAudit: buildOtherLanguageAudit(filteredRows, aggregate.months),
    ...aggregate,
  };
}

function bumpCount(map, key) {
  const label = String(key || "").trim() || "(blank)";
...
// Reads every KYC office sheet, parses the language of each row and explains
// why rows would be classified as "Other". Intended to be run as a diagnostic
// (needs Google Sheets credentials) to discover which raw language strings are
// still unmapped so we can extend the alias tables.
export async function diagnoseKycLanguages(options = {}) {
  const kycSources = resolveApprovedDepositsKycSources(options);
  const readValues = options.readValues || readSheetValues;
  const getTitles = options.getSheetTitles || getSheetTitles;
  const sampleLimit = Number.isFinite(Number(options.sampleLimit)) ? Number(options.sampleLimit) : 5;

  const totals = { rows: 0, withCid: 0 };
  const byCategory = { Native: 0, English: 0, "English & Native": 0, Other: 0 };
  const otherReasons = {
    noLanguageLabel: 0,
    placeholderOnly: 0,
    unmappedLanguage: 0,
  ...
  const unmappedLanguageCounts = new Map();
  const unmappedLanguageSamples = new Map();
  const noLabelCountries = new Map();
  ...
  for (const source of kycSources) {
    ...
    let titles = [];
    try {
      titles = await getTitles(source.spreadsheetId);
    } catch (error) {
      offices.push({ office, error: String(error?.message || error), tabs: [] });
      continue;
    }
    const validTabs = validApprovedDepositTabTitles(titles || []);
    const officeSummary = { office, tabs: [], rows: 0, other: 0 };
    for (const title of validTabs) {
      let values = [];
      try {
        values = await readValues(source.spreadsheetId, `${quotedSheetName(title)}!A:Z`, { bypassCache: true });
      } catch (error) {
        officeSummary.tabs.push({ title, error: String(error?.message || error) });
        continue;
      }
      const headerRowIndex = findHeaderRow(values);
      if (headerRowIndex < 0) {
        officeSummary.tabs.push({ title, skipped: "no header row detected" });
        continue;
      }
      const headerMap = headerMapFromRow(values[headerRowIndex]);
      let tabRows = 0;
      for (const row of values.slice(headerRowIndex + 1)) {
        ...
        totals.withCid += 1;
        tabRows += 1;
        const category = categorizeKycLanguages({ country: record.country, languages: record.languages });
        byCategory[category] = (byCategory[category] || 0) + 1;
        if (category !== "Other") {
          continue;
        }
        officeSummary.other += 1;
        bumpCount(otherCountries, record.country);
        if (!record.languages.length) {
          otherReasons.noLanguageLabel += 1;
          bumpCount(noLabelCountries, record.country);
        } else if (record.country === "Unknown") {
          otherReasons.unknownCountry += 1;
        } else {
          otherReasons.unmappedLanguage += 1;
          const token = record.languages.join(" & ");
          bumpCount(unmappedLanguageCounts, token);
          const samples = unmappedLanguageSamples.get(token) || [];
          if (samples.length < sampleLimit) {
            samples.push({ cid: record.cid, country: record.country, office });
            unmappedLanguageSamples.set(token, samples);
          }
        }
      }
      officeSummary.tabs.push({ title, rows: tabRows });
      officeSummary.rows += tabRows;
    }
    offices.push(officeSummary);
  }
  ...
  return {
    ...
    byCategory,
    otherReasons,
    topUnmappedLanguages: topEntries(unmappedLanguageCounts).map((entry) => ({
      ...entry,
      samples: unmappedLanguageSamples.get(entry.value) || [],
    })),
    topNoLabelCountries: topEntries(noLabelCountries),
    topOtherCountries: topEntries(otherCountries),
    offices,
  };
}
