...
export function addKycLanguageRecord(index, record = {}) {
  ...
  if (!index.byAccIdAndCountry.has(key)) {
    index.byAccIdAndCountry.set(key, normalizedRecord);
  }
  const accId = normalizedRecord.cid;
  const candidates = index.byAccId.get(accId) || [];
  if (!candidates.some((item) => kycLanguageLookupKey(item.cid, item.country) === key)) {
    candidates.push(normalizedRecord);
    index.byAccId.set(accId, candidates);
  }
}

export function lookupKycLanguageRecord(index, { accId = "", country = "" } = {}) {
  const normalizedAccId = normalizeApprovedDepositAccId(accId);
  if (!normalizedAccId) {
    return null;
  }
  const normalizedCountry = normalizeApprovedDepositCountry(country);
  const exact = index.byAccIdAndCountry.get(kycLanguageLookupKey(normalizedAccId, normalizedCountry));
  if (exact) {
    return exact;
  }
  const candidates = index.byAccId.get(normalizedAccId) || [];
...
function amountRecord(row = [], headerMap, languageIndex = createKycLanguageIndex()) {
  ...
  const kycMatch = lookupKycLanguageRecord(languageIndex, { accId, country });
  const language = kycMatch?.language || "";
  return {
    cid: accId,
    accId,
...
async function loadAllKycLanguageMaps({ kycSources = [], readValues, getTitles } = {}) {
  ...
  for (const source of kycSources) {
    ...
    for (const record of officeSource.records) {
      addKycLanguageRecord(languageIndex, record);
    }
  }
...
async function loadAmountRows({ spreadsheetId, readValues, sheetTitles, languageIndex } = {}) {
  ...
  for (const title of sourceTabs) {
    ...
    const headerMap = headerMapFromRow(values[headerRowIndex]);
    for (const row of values.slice(headerRowIndex + 1)) {
      const record = amountRecord(row, headerMap, languageIndex);
