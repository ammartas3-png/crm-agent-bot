...
const LANGUAGE_LABEL_WORDS =
  "languages|language|langauge|langague|lang|preferred\\s+language|spoken\\s+language|customer\\s+language|native\\s+language|mother\\s+tongue|langue|langues|sprache|idioma|idiomas|lingua|lingue|dil|langue\\s+parlee";
const LANGUAGE_LABEL_PATTERN = new RegExp(
  `\\b(?:${LANGUAGE_LABEL_WORDS})\\b\\s*(?:\\([^)]*\\))?\\s*[:=\\-]\\s*([^\\n\\r;|]+)`,
  "i",
);
const LANGUAGE_MULTILINE_PATTERN = new RegExp(
  `\\b(?:${LANGUAGE_LABEL_WORDS})\\b\\s*(?:\\([^)]*\\))?\\s*[:=\\-]\\s*(?:\\n|\\r\\n?)\\s*([^\\n\\r;|]+)`,
  "i",
);
...
const LANGUAGE_LABEL_WORD_PATTERN = new RegExp(`\\b(?:${LANGUAGE_LABEL_WORDS})\\b`, "i");

function hasLanguageLabel(text = "") {
  return LANGUAGE_LABEL_WORD_PATTERN.test(text);
}
...
function languageMatchesCountryName(language = "", countries = []) {
  const normalizedLanguage = normalizeText(stripDiacritics(language));
  if (!normalizedLanguage) {
    return false;
  }
  return countries.some((country) => {
    const normalizedCountry = normalizeText(stripDiacritics(country));
    return normalizedCountry && normalizedCountry !== "unknown" && normalizedCountry === normalizedLanguage;
  });
}
...
function languageIsNativeForCountries(language = "", ...countries) {
  ...
  if (uniqueCountries.some((country) => isNativeLanguageForCountry(language, country))) {
    return true;
  }
  return languageMatchesCountryName(language, uniqueCountries);
}
