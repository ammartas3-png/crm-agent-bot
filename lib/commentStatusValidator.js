...
function inferStatusFromHints(lastComment, statusHints = []) {
  if (!lastComment || !statusHints.length) {
    return null;
  }
  const commentTokens = tokenizeNormalizedText(lastComment.normalized);
  if (!commentTokens.length) {
    return null;
  }
  const commentTokenSet = new Set(commentTokens);
  const scored = statusHints
    .map((hint) => {
      const hintTokens = tokenizeNormalizedText(hint.description);
      if (!hintTokens.length) {
        return null;
      }
      const matchedTokens = hintTokens.filter((hintToken) =>
        [...commentTokenSet].some(
          (commentToken) =>
            commentToken === hintToken ||
            commentToken.startsWith(hintToken) ||
            hintToken.startsWith(commentToken),
        ),
      );
...
function validateRow(row, activeRules, options = {}) {
  ...
  const hintInference = statusResult.expectedStatus
    ? null
    : inferStatusFromHints(latestComment, options.statusHints || []);
  const appointment = detectAppointment(comments);
  const callResult = appointment.detected ? callCheckResult(row) : "-";

  let suggestedStatus = statusResult.expectedStatus || hintInference?.status || "";
  ...
  const reviewType = normalizeReviewType(
    statusResult.expectedStatus || hintInference ? "Status Change Suggested" : statusResult.reviewType,
    suggestedStatus,
    currentStatus,
  ...
  return {
    ...
    reason:
      reviewType === "Appointment Check"
        ...
        : hintInference
          ? `Matched Sheet1 status description for ${hintInference.status}.`
          : statusResult.reason,
    matchedPositiveKeywords: hintInference
      ? hintInference.matchedTokens
      : statusResult.matchedPositiveKeywords,
...
export function validateCommentStatusRows(rows = [], activeRules = [], options = {}) {
  const flaggedRows = [];
  ...
  for (const row of rows) {
    const result = validateRow(row, activeRules, options);
    if (!result.include) {
      skippedCorrect += 1;
      continue;
    }
