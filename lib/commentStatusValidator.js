...
const SYSTEM_COMMENT_PATTERNS = [/^email/i, /\bwas sent by\b/i];
...
function parseComments(rawComments) {
  ...
  for (const line of lines) {
    ...
    const content = parts.length >= 3 ? parts.slice(2).join("|").trim() : line;
    if (!content) {
      continue;
    }
...
function findKeywordMatches(comments, keywords = []) {
  ...
  for (const keyword of keywords) {
    ...
    if (comments.some((comment) => comment.normalized.includes(normalizedKeyword))) {
...
function resolveExpectedStatus(comments, rules = []) {
  ...
  for (const rule of rules) {
    const positiveMatches = findKeywordMatches(comments, rule.positiveKeywords);
    if (!positiveMatches.length) {
      continue;
    }
    const negativeMatches = findKeywordMatches(comments, rule.negativeKeywords);
...
function detectAppointment(comments = []) {
  const joinedText = comments.map((comment) => comment.text).join(" | ");
  const normalizedJoined = comments.map((comment) => comment.normalized).join(" ");
