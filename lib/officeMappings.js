...
async function loadOfficeMonthMap() {
  ...
  dashboardPerfLog("GOOGLE_SHEETS_READ_START", {
    sheet: officesSheetName(),
    tab: officesSheetName(),
  ...
  dashboardPerfLog("GOOGLE_SHEETS_READ_DONE", {
    ...
    rows: Array.isArray(values) ? values.length : 0,
  });
  dashboardPerfLog("SHEET_NAME", { sheet: officesSheetName() });
  dashboardPerfLog("ROWS_LOADED", { sheet: officesSheetName(), rows: Array.isArray(values) ? values.length : 0 });
...
export async function getOfficeMonthMap(options = {}) {
  ...
  const redisKey = `sheets:offices:${safeCacheKeyPart(spreadsheetId())}:${safeCacheKeyPart(officesSheetName())}`;
  try {
    let value =
      bypassCache
        ? null
        : await readCachedJson(redisKey, {
            cacheScope: "sheet-static",
            cacheLabel: "offices",
          });
    if (!value) {
      value = await loadOfficeMonthMap();
      if (!bypassCache) {
        await writeCachedJson(redisKey, value, STATIC_CACHE_TTL_SECONDS, {
          cacheScope: "sheet-static",
          cacheLabel: "offices",
        });
      }
    ...
    if (isGoogle429Error(error)) {
      dashboardPerfLog("GOOGLE_SHEETS_429", { sheet: officesSheetName() });
    }
