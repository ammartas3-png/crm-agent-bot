...
function logOfficesRead(event = "START", payload = {}) {
  const entry = {
    event,
  ...
  try {
    ...
    console[level]("[GOOGLE_SHEETS_READ]", `${event} offices ${entry.range}`);
  }
}

function withOfficesTimeout(promise, timeoutMs = OFFICES_READ_TIMEOUT_MS, timeoutFactory = () => new Error("Google Sheets read timed out.")) {
  let timerId = null;
  const timeoutPromise = new Promise((_, reject) => {
    timerId = setTimeout(() => reject(timeoutFactory()), Math.max(1, Number(timeoutMs) || OFFICES_READ_TIMEOUT_MS));
...
async function loadOfficeMonthMap() {
  ...
  const tabName = officesSheetName();
  const range = `${quoteSheetName(tabName)}!A:Z`;
  const startedAt = Date.now();
  logOfficesRead("START", {
    spreadsheetId: spreadsheet,
    tabName,
    range,
  });
  try {
    const response = await withOfficesTimeout(
      ...
      OFFICES_READ_TIMEOUT_MS,
      () => {
        const error = new Error(
          `Google Sheets read timed out after ${OFFICES_READ_TIMEOUT_MS}ms. category=offices sheet=${spreadsheet} tab="${tabName}" range=${range} month=- office=-.`,
        );
        error.code = "google_sheets_timeout";
        error.stage = "Loading Google Sheets";
        error.sheet = spreadsheet;
        error.tab = tabName;
        error.range = range;
        return error;
      },
    ...
    return buildOfficeMonthMap(values);
  } catch (error) {
    logOfficesRead("FAIL", {
      spreadsheetId: spreadsheet,
      tabName,
