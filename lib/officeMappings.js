...
export async function getOfficeMonthMap(options = {}) {
  ...
  try {
    ...
    // Do NOT cache the synthetic fallback: caching it would keep serving the
    // degraded office list for the whole TTL even after Sheets recovers. Return
    // it uncached so the next request retries the live load.
    const fallback = {
