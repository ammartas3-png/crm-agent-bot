...
const PREFERRED_COUNTRIES = ["Turkey", "Pakistan", "Argentina", "United Arab Emirates", "Tunisia"];
...
function buildOfficeMonthMap(values = []) {
  ...
  if (!rows.length) {
    return { countries: [], byCountry: {}, offices: [], byOffice: {} };
  }
  const header = rows[0] || [];
  ...
  for (let rowIndex = 1; rowIndex < rows.length; rowIndex += 1) {
    ...
    if (!byOffice[officeName]) {
      byOffice[officeName] = new Map();
    }
    ...
    for (const { parsed, index } of monthColumns) {
      ...
      if (!byOffice[officeName].has(parsed.key)) {
        byOffice[officeName].set(parsed.key, {
          ...parsed,
          sheet_id: id,
          active: true,
          office_name: officeName,
  ...
  for (const officeName of Object.keys(byOffice)) {
    byOfficeArrays[officeName] = [...byOffice[officeName].values()].sort((left, right) =>
      right.key.localeCompare(left.key),
    );
  }
  const offices = Object.keys(byOfficeArrays).sort((left, right) => left.localeCompare(right));
  return {
    countries,
    byCountry: byCountryArrays,
    officesByCountry: officesByCountryArrays,
    offices,
    byOffice: byOfficeArrays,
  };
}
...
export async function getOfficeMonthMap(options = {}) {
  ...
  try {
    ...
    const fallbackOffices = PREFERRED_COUNTRIES.map((country) =>
      country.toLocaleLowerCase("en-US").includes("office") ? country : `${country} Office`,
    );
    const officesByCountry = {};
    for (const country of PREFERRED_COUNTRIES) {
      const officeName = country.toLocaleLowerCase("en-US").includes("office") ? country : `${country} Office`;
      officesByCountry[country] = [officeName];
    }
    const fallback = {
...
// Resolves dashboard/bot office labels (country or office name) to the canonical
// Offices-tab key, e.g. "Argentina" -> "Argentina Office".
export function resolveOfficeNameForMonthMap(officeName = "", officeMap = {}) {
  ...
  const offices = Array.isArray(officeMap?.offices) ? officeMap.offices : [];
  const officeByCountryLabel = offices.find((name) => normalizeText(officeCountryFromName(name)) === normalized);
  if (officeByCountryLabel) {
    return officeByCountryLabel;
  }
  const officeBySubstring = offices.find(
    (name) => normalizeText(name).includes(normalized) || normalized.includes(normalizeText(name)),
  );
  return officeBySubstring || trimmed;
}
