...
import { preparedDataCacheRequired, readPreparedOfficeMonthMap } from "./preparedDataCache.js";
...
export async function getOfficeMonthMap(options = {}) {
  ...
  const preparedMap = await readPreparedOfficeMonthMap();
  if (preparedMap && typeof preparedMap === "object") {
    cache = {
      timestamp: Date.now(),
      value: preparedMap,
    };
