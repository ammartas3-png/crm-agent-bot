...
import { isAdminTelegramUser, parseAdminUsers } from "./permissions.js";
import {
  addNegativeKeyword,
  addPositiveKeyword,
  formatRulesSummary,
  listRulesFromSheet,
  listStatusHintsFromSheet,
...
export function rootStartKeyboard(telegramUser, authorityScope = {}) {
  const rows = [];
  if (isAdminTelegramUser(telegramUser) || authorityScope?.canUseBot) {
    rows.push([{ text: "Results from Months Table", callback_data: "root:results" }]);
  }
  if (isAdminTelegramUser(telegramUser)) {
    rows.push([{ text: "🤖 AI Assistant", callback_data: "root:ai" }]);
    rows.push([{ text: "Access Requests", callback_data: "root:access_requests" }]);
  }
  if (canAccessAction(telegramUser, ACTIONS.DATABASE_CHECK)) {
    rows.push([{ text: "Database Check", callback_data: "dbcheck:open" }]);
  }
  if (!rows.length) {
    rows.push([{ text: "Dashboard Info", callback_data: "root:dashboard" }]);
  }
...
export async function processDatabaseCheckWorkbook(fileBuffer) {
  const rules = await listRulesFromSheet();
  const statusHints = await listStatusHintsFromSheet().catch(() => []);
  const activeRules = rules.filter((rule) => rule.active);
  const rows = readInputWorkbookRows(fileBuffer);
  const validation = validateCommentStatusRows(rows, activeRules, { statusHints });
