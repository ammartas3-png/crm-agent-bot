...
const READ_ROWS_REQUEST_TIMEOUT_MS = (() => {
  const parsed = Number(process.env.GOOGLE_SHEETS_READ_TIMEOUT_MS || 45_000);
...
function readRowsTimeoutError({ timeoutMs, tabName, spreadsheetId, range }) {
  const error = new Error(
    `Google Sheets read timed out for tab "${tabName}" after ${timeoutMs}ms (sheet=${spreadsheetId}, range=${range}).`,
  );
