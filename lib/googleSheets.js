...
export function rowsToObjects(values = [], expectedColumns = []) {
  ...
  const usableHeaders = headerMatchesConfig
    ? headers.map((header, index) => {
        ...
        const configuredHeader = String(expectedColumns[index] || "").trim();
        const preferredHeader = normalizedHeader || configuredHeader;
        if (!preferredHeader) {
          return "";
        }
        if (!usedHeaders.has(preferredHeader)) {
          usedHeaders.add(preferredHeader);
          return preferredHeader;
        }
        if (configuredHeader && configuredHeader !== preferredHeader && !usedHeaders.has(configuredHeader)) {
          usedHeaders.add(configuredHeader);
          return configuredHeader;
        }
        const uniqueHeader = `${preferredHeader} ${index + 1}`;
