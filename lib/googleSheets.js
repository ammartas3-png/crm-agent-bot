...
import { readPreparedSheetRows, preparedDataCacheRequired } from "./preparedDataCache.js";
...
export async function readSheetRows(tabKey, options = {}) {
  ...
  const preparedRows = await readPreparedSheetRows({
    spreadsheetId,
    tabKey,
    tabConfig,
    range,
  ...
  if (preparedDataCacheRequired()) {
    throw new Error(
      `Prepared cache miss for ${String(tabKey || "")} (${String(spreadsheetId || "")}). ` +
