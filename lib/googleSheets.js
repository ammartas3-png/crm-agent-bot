...
const READ_ROWS_REQUEST_TIMEOUT_MS = (() => {
  const parsed = Number(process.env.GOOGLE_SHEETS_READ_TIMEOUT_MS || 30_000);
...
function withPromiseTimeout(promise, timeoutMs, timeoutErrorFactory) {
  ...
  const timeoutPromise = new Promise((_, reject) => {
    timerId = setTimeout(() => {
      reject(typeof timeoutErrorFactory === "function" ? timeoutErrorFactory() : new Error("Google Sheets read timed out."));
    }, Math.max(1, Number(timeoutMs) || READ_ROWS_REQUEST_TIMEOUT_MS));
...
function logGoogleSheetsRead(event = "START", payload = {}) {
  const entry = {
    event,
    ...
    selectedOffice: String(payload.selectedOffice || "-"),
  ...
  try {
    console[level]("[GOOGLE_SHEETS_READ]", JSON.stringify(entry));
  } catch {
    console[level]("[GOOGLE_SHEETS_READ]", `${event} ${entry.tabName} ${entry.range}`);
  }
}

function readRowsTimeoutError({ timeoutMs, tabName, spreadsheetId, range, selectedMonth = "-", selectedOffice = "-", category = "" }) {
  const error = new Error(
    `Google Sheets read timed out after ${timeoutMs}ms. category=${category || "sheet"} sheet=${spreadsheetId} tab="${tabName}" range=${range} month=${selectedMonth} office=${selectedOffice}.`,
  );
  error.code = "google_sheets_timeout";
  error.stage = "Loading Google Sheets";
  error.tab = tabName;
  error.sheet = spreadsheetId;
  error.range = range;
  error.selectedMonth = selectedMonth;
  error.selectedOffice = selectedOffice;
  error.category = category;
  return error;
}

function canUseStaleRowsOnError(error) {
  const code = String(error?.code || "").toLowerCase();
  if (code === "google_sheets_timeout") {
    return false;
  }
...
export async function readSheetRows(tabKey, options = {}) {
  ...
  const selectedMonth = String(options.selectedMonth || options.monthKey || "-");
  const selectedOffice = String(options.selectedOffice || options.officeScope || "-");
  const category = String(options.readCategory || tabKey || "");
  const requestTimeoutMs = Number.isFinite(Number(options.requestTimeoutMs))
    ? Math.max(1, Number(options.requestTimeoutMs))
    : READ_ROWS_REQUEST_TIMEOUT_MS;
  ...
  const staleCacheEntry = !bypassCache ? readRowsCache.get(cacheKey) : null;
  if (!bypassCache) {
  ...
  const pending = withPromiseTimeout(
    ...
    requestTimeoutMs,
    () =>
      readRowsTimeoutError({
        timeoutMs: requestTimeoutMs,
        tabName: String(tabConfig?.name || tabKey || ""),
        spreadsheetId,
        range,
        selectedMonth,
        selectedOffice,
  ...
  try {
    ...
    if (!bypassCache && staleCacheEntry && Array.isArray(staleCacheEntry.rows) && staleCacheEntry.rows.length && canUseStaleRowsOnError(error)) {
      console.warn(
        `[GOOGLE_SHEETS_STALE_FALLBACK] tab=${String(tabConfig?.name || tabKey || "")} sheet=${spreadsheetId} range=${range} reason=${String(
          error?.message || error || "",
        )}`,
      );
      rows = staleCacheEntry.rows;
    } else {
      logGoogleSheetsRead("FAIL", {
        spreadsheetId,
        tabName: String(tabConfig?.name || tabKey || ""),
        range,
        selectedMonth,
