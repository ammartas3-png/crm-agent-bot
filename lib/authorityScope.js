...
function parseAuthorityColumn(authorityText = "") {
  ...
  for (const token of parseList(authorityText)) {
    ...
    if (ALL_TOKENS.has(normalizedToken)) {
      botReportAccess = true;
      role = normalizedToken;
      continue;
    }
    if (ADMIN_TOKENS.has(normalizedToken)) {
      unrestricted = true;
      botReportAccess = true;
      role = normalizedToken;
      continue;
...
function mergeScopeSets(base, nextValues = []) {
  for (const value of nextValues) {
    ...
    if (trimmed && !isAllValue(trimmed) && !isSheetErrorValue(trimmed)) {
      base.add(trimmed);
    }
  }
}
...
export function computeAuthorityScopeFromRows(rows = [], telegramUser) {
  ...
  if (teamLeaderSet.size) {
    ...
    const shouldApplyTeam =
      !role ||
      ALL_TOKENS.has(normalizedRole) ||
      ["manager", "managers", "desk manager", "deskmanager", "desk_manager", "team leader", "teamleader", "team_leader"].includes(
        normalizedRole,
      );
    if (shouldApplyTeam) {
...
export async function resolveAuthorityScopeForUser(telegramUser, options = {}) {
  ...
  if (!principals.length) {
    return { allowed: false, unrestricted: false, botReportAccess: false, filters: {}, matchedRows: 0 };
  }
