...
const AUTHORITY_HEADERS = ["User Name", "User Telegram", "User Telegram ID", "Office", "Desk", "Authority"];
...
function authorityRange(range = "A:F") {
...
function composeAuthorityCell(teamLeaders = []) {
  const teams = compactUnique(teamLeaders);
  if (!teams.length) {
    return "all";
  }
  return `Team Leader: ${teams.join(", ")}`;
}
...
function parseAuthorityRows(values = []) {
  ...
  for (let index = 1; index < values.length; index += 1) {
    ...
    const telegramUsername = String(row[1] || "").trim();
    const telegramId = normalizeTelegramId(row[2] || "");
    const office = String(row[3] || "").trim();
    const desk = String(row[4] || "").trim();
    const authority = String(row[5] || "").trim();
...
export async function upsertAuthorityUserScope({
  ...
  const rowValues = [
    ...
    composeAuthorityCell(teamLeaders),
  ];

  if (existing) {
    await sheets.spreadsheets.values.update({
      spreadsheetId,
      range: authorityRange(`A${existing.rowNumber}:F${existing.rowNumber}`),
...
export async function removeAuthorityRowByNumber(rowNumber) {
  ...
  await sheets.spreadsheets.values.clear({
    spreadsheetId,
    range: authorityRange(`A${numeric}:F${numeric}`),
  });
...
export function summarizeAuthorityRow(row = {}) {
  ...
  const scope = [`Office: ${row.office || "all"}`, `Desk: ${row.desk || "all"}`, `Authority: ${row.authority || "all"}`];
