...
const COLUMN_ALIAS_GROUPS = {
  ...
  "agent name": ["agent name", "agent", "agent names"],
  agent: ["agent", "agent name", "agent names"],
};
...
export function rowMatchesFilters(row, tabConfig, filters = {}, now = new Date()) {
  ...
  if (!matchesExactAnyField(["office", "desk"], filters.office)) {
    return false;
  }
  if (!matchesContainsAnyField(["office", "desk", "department", "country"], filters.officeContains)) {
    return false;
  }
