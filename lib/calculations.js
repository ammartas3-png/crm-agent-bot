...
export function rowMatchesFilters(row, tabConfig, filters = {}, now = new Date()) {
  ...
  if (filters.department) {
    const department = normalizeText(getRowValue(row, getFieldName(tabConfig, "department")));
    if (department !== normalizeText(filters.department)) {
      return false;
    }
  }
  ...
  if (filters.placement) {
    const placement = normalizeText(getRowValue(row, getFieldName(tabConfig, "placement")));
    if (placement !== normalizeText(filters.placement)) {
      return false;
    }
  }

  if (filters.subCampaign) {
    const subCampaign = normalizeText(getRowValue(row, getFieldName(tabConfig, "subCampaign")));
    if (subCampaign !== normalizeText(filters.subCampaign)) {
      return false;
    }
  }
