...
export function calculateCR(totalFtd, validLeads) {
  return validLeads > 0 ? (totalFtd / validLeads) * 100 : 0;
}
...
export function calculateSummary(rows, tabConfig, filters = {}, now = new Date()) {
  ...
  const { totalLeads, differentMonthLeads, validLeads } = calculateValidLeads(
    leadRows,
    tabConfig,
  );
  const totalFtd = calculateFtdCount(ftdRows, tabConfig);
  const cr = calculateCR(totalFtd, validLeads);

  const targets = contextRows
  ...
  const lateFtd = ftdRows.filter((row) =>
