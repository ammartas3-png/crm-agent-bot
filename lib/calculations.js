...
// Parsing the same date strings runs across every row in multiple passes, so a
// bounded memo cache avoids re-running the regex/Date construction repeatedly.
// Returned Date objects must be treated as read-only by callers.
const DATE_PARSE_CACHE_LIMIT = 20_000;
const dateParseCache = new Map();

function parseDateValueUncached(value) {
...
export function parseDateValue(value) {
  ...
  if (typeof value !== "string" && typeof value !== "number") {
    return parseDateValueUncached(value);
  }

  const cacheKey = `${typeof value}:${value}`;
  if (dateParseCache.has(cacheKey)) {
    return dateParseCache.get(cacheKey);
  }

  const result = parseDateValueUncached(value);
  if (dateParseCache.size >= DATE_PARSE_CACHE_LIMIT) {
...
export function rowMatchesFilters(row, tabConfig, filters = {}, now = new Date()) {
  ...
  if (filters.country) {
    const country = normalizeText(getRowValue(row, getFieldName(tabConfig, "country")));
  ...
  if (filters.office) {
    const office = normalizeText(getRowValue(row, getFieldName(tabConfig, "office")));
    if (office !== normalizeText(filters.office)) {
      return false;
    }
  }

  if (filters.teamLeader) {
    const teamLeader = normalizeText(getRowValue(row, getFieldName(tabConfig, "teamLeader")));
    if (teamLeader !== normalizeText(filters.teamLeader)) {
      return false;
    }
  }
  ...
  if (filters.agent) {
    const agent = normalizeText(
      getRowValue(row, getFieldName(tabConfig, filters.agentField || "agentNames")) ||
  ...
  if (filters.brand) {
    const brand = normalizeText(getRowValue(row, getFieldName(tabConfig, "brand")));
  ...
  if (filters.campaign) {
    const campaign = normalizeText(getRowValue(row, getFieldName(tabConfig, "campaign")));
    if (campaign !== normalizeText(filters.campaign)) {
      return false;
    }
  }

  if (filters.status) {
    const status = normalizeText(getRowValue(row, getFieldName(tabConfig, "status")));
...
export function uniqueValues(rows, tabConfig, fieldKey, limit = 96) {
  const fieldName = getFieldName(tabConfig, fieldKey);
...
export function uniqueValuesForFields(rows, tabConfig, fieldKeys = [], limit = 96) {
  const idField = getFieldName(tabConfig, "id");
  const fieldNames = fieldKeys.map((fieldKey) => [fieldKey, getFieldName(tabConfig, fieldKey)]);
  const sets = new Map(fieldKeys.map((fieldKey) => [fieldKey, new Set()]));

  for (const row of rows) {
    if (!isPresent(getRowValue(row, idField))) {
      continue;
    }
    for (const [fieldKey, fieldName] of fieldNames) {
      const value = String(getRowValue(row, fieldName) || "").trim();
      if (value) {
        sets.get(fieldKey).add(value);
      }
    }
  }

  const result = {};
  for (const [fieldKey, values] of sets) {
