...
export function calculateSummary(rows, tabConfig, filters = {}, now = new Date()) {
  ...
  const totalLeads = scopedRows.filter((row) => isPresent(getRowValue(row, fields.id))).length;
  const totalFtd = scopedRows.filter((row) => isFtd(getRowValue(row, fields.ftd))).length;
...
export function topPerformers(
  ...
  return [...groups.entries()]
    ...
    .map((item) => ({
      label: item.label,
      value: metricKey === "totalLeads" ? item.summary.totalLeads : item.summary.totalFtd,
      cr: item.summary.cr,
    }))
...
export function hourlyDistribution(
  ...
  for (const row of filteredRows(rows, tabConfig, filters, now)) {
    if (metricKey === "totalFtd" && !isFtd(getRowValue(row, getFieldName(tabConfig, "ftd")))) {
      continue;
    }
