...
export function rowMatchesFilters(row, tabConfig, filters = {}, now = new Date()) {
  ...
  const dateValueForField = (dateFieldKey = "created") => {
    if (dateFieldKey === "ftdDate") {
      ...
      if (isPresent(explicitFtdDate) && parseDateValue(explicitFtdDate)) {
  ...
  if (filters.date) {
    ...
    if (!isPresent(dateValue)) {
      ...
      if (filters.dateField === "ftdDate") {
        // For day/range windows (e.g. a daily breakdown or "today"), a real FTD happens
        // on/after the lead is registered, so it belongs strictly to its FTD DATE day:
        // never fall back to the Created/Lead day, otherwise an FTD made later (e.g. an
        // older lead converting today) would also be miscounted on the lead's
        // registration day. Only an implausible FTD DATE that predates registration falls
        // back to the Created date. Month-scoped windows keep the original fallback so
        // monthly totals (which include late converters) are unaffected.
        const explicitFtdDate = getRowValue(row, getFieldName(tabConfig, "ftdDate"));
        const parsedFtdDate = isPresent(explicitFtdDate) ? parseDateValue(explicitFtdDate) : null;
        if (parsedFtdDate && filters.date?.type !== "month") {
          const registrationRaw =
            getRowValue(row, getFieldName(tabConfig, "created")) ||
            getRowValue(row, getFieldName(tabConfig, "leadDate"));
          const parsedRegistration = parseDateValue(registrationRaw);
          const toDayNum = (date) => Date.UTC(date.getUTCFullYear(), date.getUTCMonth(), date.getUTCDate());
          if (!parsedRegistration || toDayNum(parsedFtdDate) >= toDayNum(parsedRegistration)) {
            return false;
          }
        }
        const hasFtdSignal =
        ...
        const fallbackDateValue = dateValueForField("created");
        if (!isPresent(fallbackDateValue) || !dateMatches(fallbackDateValue, filters.date, now)) {
          return false;
        }
      } else {
        return false;
  ...
  if (filters.hourRange) {
    ...
    if (!resolvedDate && filters.dateField === "ftdDate") {
      const hasFtdSignal =
        isPresent(getRowValue(row, getFieldName(tabConfig, "ftdMaker"))) ||
        ftdMetricValue(row, tabConfig) > 0;
      if (hasFtdSignal) {
        resolvedDate = parseDateValue(dateValueForField("created"));
      }
    }
    if (!resolvedDate) {
      return false;
    }
    const hour = resolvedDate.getUTCHours();
...
export function isDifferentMonth(value) {
  ...
  if (["1", "true", "yes", "y", "evet"].includes(normalized)) {
    return true;
  }
...
export function calculateFtdCount(ftdRows, tabConfig) {
  ...
  return ftdRows.reduce((sum, row) => {
    const metricValue = ftdMetricValue(row, tabConfig);
