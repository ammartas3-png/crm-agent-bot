...
const TURKIYE_OFFICE_PATTERN = /turk(?:iye|ey)?\s*office/;

export function officePermissionValuesMatch(filterValue = "", rowValue = "") {
  ...
  if (TURKIYE_OFFICE_PATTERN.test(filter) && TURKIYE_OFFICE_PATTERN.test(row)) {
    return true;
  }
  return false;
}

export function officeNamesEquivalent(left = "", right = "") {
  return officePermissionValuesMatch(left, right);
}
...
export function rowMatchesPermissionFilters(row, tabConfig, permissionFilters = {}) {
  ...
  if (officeValues.length > 0 && !officeValues.some((value) => officePermissionValuesMatch(value, office))) {
    return false;
  }
  if (deskValues.length > 0 && !deskValues.includes(desk)) {
    return false;
  }
  if (teamLeaderValues.length > 0) {
    const matchesTeamScope =
      teamLeaderValues.includes(teamLeader) ||
      teamLeaderValues.includes(agent);
    if (!matchesTeamScope) {
      return false;
    }
  }
