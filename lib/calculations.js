...
export function calculateSummary(rows, tabConfig, filters = {}, now = new Date()) {
  const scopedRows = filteredRows(rows, tabConfig, filters, now);
  const fields = tabConfig.fields || {};

  const totalLeads = scopedRows.length;
  const differentMonthLeads = scopedRows.filter((row) =>
    isPresent(getRowValue(row, fields.differentMonth)),
  ).length;
  const validLeads = Math.max(totalLeads - differentMonthLeads, 0);
  const totalFtd = scopedRows.filter((row) => isPresent(getRowValue(row, fields.ftdMaker))).length;
  const cr = validLeads > 0 ? (totalFtd / validLeads) * 100 : 0;

  const targets = scopedRows
  ...
  const lateFtd = scopedRows.filter((row) =>
    isPresent(getRowValue(row, fields.lateFtdDifference)),
  ).length;
...
export function topPerformers(
  ...
  return [...groups.entries()]
    .map(([label, groupRows]) => ({
      label,
      summary: calculateSummary(groupRows, tabConfig, {}, now),
    }))
    .map((item) => ({
      label: item.label,
      value:
...
export function hourlyDistribution(
  ...
  return [...buckets.entries()]
    .map(([hour, bucketRows]) => {
      const summary = calculateSummary(bucketRows, tabConfig, {}, now);
      return {
