...
// Periods (YYYY-MM) with ingested leads for an office — used when the Offices tab
// is missing spreadsheet IDs but n8n/Redis already has the month synced.
export async function listIngestPeriodsForOffice(office = "", category = "leads") {
  await ensureMetaHydrated();
  const wantedOffice = normOffice(office);
  const wantedCategory = normCategory(category);
  const periods = new Set();
  for (const { meta } of memorySources.values()) {
    if (normCategory(meta.category) !== wantedCategory) {
      continue;
    }
    if (wantedOffice && normOffice(meta.office) !== wantedOffice) {
      continue;
    }
...
export async function getIngestMetaForOfficePeriod(office = "", period = "", category = "leads") {
  await ensureMetaHydrated();
  const wantedOffice = normOffice(office);
  ...
  for (const { meta } of memorySources.values()) {
    ...
    if (wantedPeriod && String(meta.period || "").trim() !== wantedPeriod) {
      continue;
    }
    if (!best || Number(meta.rowCount || 0) >= Number(best.rowCount || 0)) {
      best = meta;
    }
  }
  return best;
