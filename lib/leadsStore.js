...
const sourceRowsInflight = new Map();
let metaHydrated = false;

function mgetBatchSize(env = process.env) {
  const raw = Number(env.LEADS_MGET_BATCH);
...
async function loadSourceRowsByKey(sourceKey) {
  ...
  if (sourceRowsInflight.has(sourceKey)) {
    return sourceRowsInflight.get(sourceKey);
  }
  const loadPromise = (async () => {
    ...
    const values = await mgetBatched(keys, mgetBatchSize());
    let rows = [];
    ...
    sourceRowsInflight.delete(sourceKey);
  });
  sourceRowsInflight.set(sourceKey, loadPromise);
  return loadPromise;
}
