...
import { aiReplyCacheKey, getCachedReply, setCachedReply } from "./aiReplyCache.js";

const DEFAULT_MODEL = "gpt-4o-mini";
const DEFAULT_MAX_TOKENS = 220;

function resolveMaxTokens(env) {
  const parsed = Number(env.OPENAI_MAX_TOKENS);
  ...
  try {
    const response = await fetchImpl(`${baseUrl}/chat/completions`, {
      ...
      body: JSON.stringify({
        model,
        messages: context.messages,
        max_tokens: resolveMaxTokens(env),
        temperature: 0.2,
      }),
      signal,
    });
...
export async function generateAiReply(context, options = {}) {
  ...
  // Serve identical grounded prompts from cache to avoid spending LLM tokens on
  // repeated questions. Misses fall through to the live call below.
  const cacheKey = aiReplyCacheKey(context);
  const cached = getCachedReply(cacheKey, { env });
  if (cached) {
    return cached;
  }
  ...
  if (relayUrl && fetchImpl) {
    ...
    if (relayText) {
      setCachedReply(cacheKey, relayText, { env });
      return relayText;
    }
  }
  ...
  if (fetchImpl) {
    ...
    if (openAiText) {
      setCachedReply(cacheKey, openAiText, { env });
      return openAiText;
    }
  }
