...
function addTargetResultSheet(workbook, report = {}) {
  ...
  const rate = total > 0 ? reached / total : 0;
  ...
  const summaryCell = worksheet.getCell(1, 1);
  summaryCell.value = `${total} agent  ·  ${reached} reached target  ·  ${Math.round(rate * 100)}% reach rate`;
  summaryCell.font = { bold: true, color: { argb: "FF1D4ED8" } };
