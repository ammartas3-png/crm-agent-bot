...
function addTeamRosterSheet(workbook, report = {}) {
  ...
  // Team blocks across the top: each block = 2 columns (Agent, Lang) + a spacer.
  const BLOCK_COLS = 2;
  const SPACER = 1;
  const blockStart = (index) => index * (BLOCK_COLS + SPACER) + 1;
  let maxBottomRow = 1;
  teams.forEach((team, blockIndex) => {
    ...
    // Header (merged across the 2 columns).
    worksheet.mergeCells(1, start, 1, start + 1);
    ...
    worksheet.getCell(1, start + 1).border = border;
    const agents = Array.isArray(team.agents) ? team.agents : [];
    agents.forEach((agent, agentIndex) => {
      const rowIndex = agentIndex + 2;
    ...
    const cntRowIndex = agents.length + 2;
    const cntLabel = worksheet.getCell(cntRowIndex, start);
    cntLabel.value = "Agent Cnt";
    cntLabel.font = { bold: true };
    cntLabel.fill = cntFill;
    cntLabel.border = border;
    const cntValue = worksheet.getCell(cntRowIndex, start + 1);
    cntValue.value = Number(team.count || 0);
    cntValue.font = { bold: true };
    cntValue.fill = cntFill;
    cntValue.alignment = { horizontal: "right" };
    cntValue.border = border;
    if (cntRowIndex > maxBottomRow) {
      maxBottomRow = cntRowIndex;
    }
  });

  // Summary tables below the team blocks (by language + by team, side by side).
  const summaryStart = maxBottomRow + 2;
  const writeSummary = (startCol, title, rows, totalLabel) => {
    worksheet.getColumn(startCol).width = 18;
    worksheet.getColumn(startCol + 1).width = 22;
    worksheet.getColumn(startCol + 2).width = 24;
    const headers = [title, "Agent Cnt (Including TL)", "Agent Cnt (Not Including TL)"];
    headers.forEach((header, index) => {
      const cell = worksheet.getCell(summaryStart, startCol + index);
      cell.value = header;
      cell.font = { bold: true, color: { argb: "FFFFFFFF" } };
      cell.fill = headerFill;
    ...
    rows.forEach((row, index) => {
      ...
      const labelCell = worksheet.getCell(rowIndex, startCol);
      labelCell.value = row.label;
      labelCell.border = border;
      const inclCell = worksheet.getCell(rowIndex, startCol + 1);
      inclCell.value = Number(row.inclTL || 0);
      inclCell.alignment = { horizontal: "right" };
      inclCell.border = border;
      const exclCell = worksheet.getCell(rowIndex, startCol + 2);
      exclCell.value = Number(row.exclTL || 0);
      exclCell.alignment = { horizontal: "right" };
      exclCell.border = border;
    });
    const totalRowIndex = summaryStart + 1 + rows.length;
    const totalLabelCell = worksheet.getCell(totalRowIndex, startCol);
    totalLabelCell.value = totalLabel;
    totalLabelCell.font = { bold: true };
    totalLabelCell.fill = totalFill;
    totalLabelCell.border = border;
    const totalInclCell = worksheet.getCell(totalRowIndex, startCol + 1);
    totalInclCell.value = Number(totals.inclTL || 0);
    totalInclCell.font = { bold: true };
    totalInclCell.fill = totalFill;
    totalInclCell.alignment = { horizontal: "right" };
    totalInclCell.border = border;
    const totalExclCell = worksheet.getCell(totalRowIndex, startCol + 2);
    totalExclCell.value = Number(totals.exclTL || 0);
    totalExclCell.font = { bold: true };
    totalExclCell.fill = totalFill;
    totalExclCell.alignment = { horizontal: "right" };
    totalExclCell.border = border;
  };

  writeSummary(
    1,
    "Language",
    byLanguage.map((row) => ({ label: row.language || "—", inclTL: row.inclTL, exclTL: row.exclTL })),
    "Grand Total",
  );
  writeSummary(
    5,
    "Team",
    byTeam.map((row) => ({ label: row.team, inclTL: row.inclTL, exclTL: row.exclTL })),
    "Total",
  );
}
