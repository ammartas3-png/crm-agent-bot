...
function addTrafficPrioritySheet(workbook, report = {}, query = {}) {
  ...
  const allocation = allocationSequence(ranking.agents, count);
  ...
  const worksheet = workbook.addWorksheet("Traffic Priority");
  [6, 26, 22, 10, 10, 10].forEach((width, index) => {
  ...
  titleCell.value = `Traffic Priority - last ${windowDays} days (blocked = no FTD in last ${blockWindowDays} days)`;
  ...
  rowIndex += 1;
  const summaryHeaders = ["Agent", "Team Leader", "CR", "Count", "Share", "Status"];
  ...
  summaryAgents.forEach((agent) => {
    const values = [
      ...
      agent.blocked ? "-" : agent.allocated,
      agent.blocked ? "-" : percentCell(agent.share),
      agent.blocked ? `Blocked (no FTD ${blockWindowDays}d)` : "Active",
    ];
    values.forEach((value, index) => {
      const cell = worksheet.getCell(rowIndex, index + 1);
      cell.value = value;
