  ...
  const metricRows = [
    {
      ...
      valueForMonth: (values) => {
        const crTarget = Number(values.crTarget || 0);
        const ftdTarget = Number(values.ftdTarget || 0);
        const pspWorking = Number(values.pspWorking || 0);
        const monthDays = Number(values.monthDays || 0);
        if (crTarget <= 0 || ftdTarget <= 0 || pspWorking <= 0 || monthDays <= 0) {
          return "";
        }
        return (ftdTarget / (crTarget / 100)) / pspWorking / monthDays;
      },
      numberFormat: "0.00",
    },
    {
    ...
    {
      label: "PSPs working?",
      valueForMonth: (values) => Number(values.pspWorking || 0),
      numberFormat: "0",
    },
  ];
