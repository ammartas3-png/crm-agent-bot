...
function addLeadSplitterSheet(workbook, report = {}) {
  ...
  const columns = [
    ...
    { key: "agent", header: "Agent", width: 24 },
    { key: "leads", header: "Leads", width: 12 },
    { key: "ftd", header: "FTD", width: 12 },
    { key: "cr", header: "CR", width: 12, type: "percent" },
  ];
  setColumnWidths(worksheet, columns);
  styleHeader(worksheet.getRow(1));
  ...
  for (const row of rows) {
    const isTotal = row.kind === "countryTotal" || row.kind === "deskTotal";
    const added = worksheet.addRow({
      desk: row.desk || "",
      country: row.kind === "deskTotal" ? "" : row.country || "",
    ...
    if (isTotal) {
      added.eachCell((cell) => {
        ...
        cell.fill = {
          type: "pattern",
          pattern: "solid",
          fgColor: { argb: row.kind === "deskTotal" ? "FFDBEAFE" : "FFEEF2FF" },
        };
      });
    }
  }
