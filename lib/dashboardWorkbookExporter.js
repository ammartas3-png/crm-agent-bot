...
function comparisonSelectionsFromQuery(query = {}) {
  const raw = String(query?.comparisonSelections || "").trim();
  if (!raw) {
    return {};
  }
  try {
...
function comparisonRowsForDimension(rows = [], dimension = {}, selections = {}) {
  const dimensionKey = String(dimension?.key || "").trim();
  if (!dimensionKey) {
    return [];
  }
  const filteredRows = rows.filter((row) =>
    COMPARISON_TABLE_DIMENSIONS.every((dimensionItem) => {
      if (dimensionItem.key === dimensionKey) {
        return true;
      }
  ...
  for (const row of filteredRows) {
    ...
    const leads = metricNumber(row?.leads);
    const ftd = metricNumber(row?.ftd);
    const crTarget = metricNumber(row?.crTarget);
    const crTargetReach = metricNumber(row?.crTargetReach);
    bucket.leads += leads;
    bucket.ftd += ftd;
    if (crTarget > 0) {
  ...
  return [...grouped.values()]
    ...
    .sort(
      (left, right) =>
        metricNumber(right.leads) - metricNumber(left.leads) ||
        String(left.label || "").localeCompare(String(right.label || ""), undefined, { numeric: true, sensitivity: "base" }),
    );
}

function addComparisonSheets(workbook, report = {}, query = {}) {
  const cleanRows = Array.isArray(report?.table) ? report.table.filter((row) => row && row.__rowKind !== "total") : [];
  const selections = comparisonSelectionsFromQuery(query);
  for (const dimension of COMPARISON_TABLE_DIMENSIONS) {
    const sheetName = String(dimension.label || "Comparison").slice(0, 31);
    ...
    worksheet.columns = [
      ...
      { key: "ftd", width: 14 },
      { key: "cr", width: 14 },
      { key: "crTargetReach", width: 18 },
    ];
    worksheet.mergeCells(1, 1, 1, 5);
    const titleCell = worksheet.getCell(1, 1);
    titleCell.value = `Comparison Table — ${dimension.label}`;
    ...
    headerRow.values = [dimension.label, "Leads", "FTD", "CR", "CR Target Reach"];
    ...
    const rows = comparisonRowsForDimension(cleanRows, dimension, selections);
    for (const row of rows) {
      worksheet.addRow({
        label: row.label,
        leads: metricNumber(row.leads),
        ftd: metricNumber(row.ftd),
    ...
    applyTableGrid(worksheet, 5, 2);
    for (let rowIndex = 3; rowIndex <= worksheet.rowCount; rowIndex += 1) {
      const reachCell = worksheet.getCell(rowIndex, 5);
      if (reachCell.value === "" || reachCell.value === null || reachCell.value === undefined) {
        continue;
      }
      const reachNumeric = rows[rowIndex - 3]?.crTargetReach;
      if (!Number.isFinite(Number(reachNumeric))) {
        continue;
      }
      applyReachCellStyle(reachCell, Number(reachNumeric));
      reachCell.alignment = { vertical: "middle", horizontal: "center" };
    }
  }
}
...
function addBuilderTableSheet(workbook, report = {}, query = {}) {
  ...
  const orderedRows = last4QuickStyleEnabled ? sortRowsForLast4QuickExport(sourceRows) : sourceRows;
  const dataRowMeta = [];
...
export async function dashboardReportWorkbookBuffer(report = {}, query = {}) {
  ...
  if (asEnabled(query.comparisonMode) && report.tableType === "builder") {
    addComparisonSheets(workbook, report, query);
