...
function addLeadSplitterSheet(workbook, report = {}) {
  ...
  const columns = [
    { key: "desk", header: "Desk", width: 12 },
    { key: "country", header: "Country", width: 22 },
    { key: "agent", header: "Agent", width: 24 },
    { key: "leads", header: "Leads", width: 10 },
    { key: "ftd", header: "FTD", width: 10 },
    { key: "cr", header: "CR", width: 10, type: "percent" },
    { key: "crTarget", header: "CR Target", width: 12, type: "percent" },
    { key: "crTargetReach", header: "CR Target Reach", width: 16, type: "percent" },
  ];
  setColumnWidths(worksheet, columns);

  // Dark navy header with white bold text (matches the reference sheet).
  worksheet.getRow(1).eachCell((cell) => {
    cell.font = { bold: true, color: { argb: "FFFFFFFF" } };
    cell.fill = { type: "pattern", pattern: "solid", fgColor: { argb: "FF1F3864" } };
    cell.alignment = { vertical: "middle", horizontal: "center" };
    cell.border = {
      top: { style: "thin", color: { argb: "FF16294D" } },
      left: { style: "thin", color: { argb: "FF16294D" } },
      bottom: { style: "thin", color: { argb: "FF16294D" } },
      right: { style: "thin", color: { argb: "FF16294D" } },
    };
  });
  ...
  const paintTrafficCell = (cell, isGood, bold) => {
    cell.fill = isGood ? greenFill : redFill;
    cell.font = { ...(cell.font || {}), color: { argb: isGood ? "FF006100" : "FF9C0006" }, bold: Boolean(bold) };
  };

  const rows = Array.isArray(report?.leadSplitter?.rows) ? report.leadSplitter.rows : [];
  for (const row of rows) {
    const isDeskTotal = row.kind === "deskTotal";
    const isCountryTotal = row.kind === "countryTotal";
    const isTotal = isDeskTotal || isCountryTotal;
    const added = worksheet.addRow({
      desk: row.desk || "",
      country: isDeskTotal ? "" : row.country || "",
      agent: row.kind === "agent" ? row.agent || "" : row.label || "",
    ...
    // CR cell: green when > 0, red when 0 (matches the reference sheet).
    paintTrafficCell(added.getCell(6), Number(row.cr) > 0, isTotal);
    // CR Target Reach: green when target reached (>= 100%), red otherwise.
    paintTrafficCell(added.getCell(8), Number(row.crTargetReach) >= 100, isTotal);
  }
  percentFormat(worksheet, 6);
  percentFormat(worksheet, 7);
  percentFormat(worksheet, 8);
  applyTableGrid(worksheet, columns.length, 1);
}
