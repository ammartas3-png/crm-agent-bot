...
function addComparisonReportSheets(workbook, report = {}) {
  const tables = Array.isArray(report.comparisonTables) ? report.comparisonTables : [];
  const columns = COMPARISON_COLUMNS.map((column) => ({
    key: column.key,
    header: column.label,
  ...
  for (const table of tables) {
    const worksheet = workbook.addWorksheet(comparisonSheetName(table.label));
    setColumnWidths(worksheet, columns);
    styleHeader(worksheet.getRow(1));
...
export async function dashboardReportWorkbookBuffer(report = {}, query = {}) {
  ...
  } else if (isComparisonReportQuery(query) && Array.isArray(report.comparisonTables) && report.comparisonTables.length) {
    addComparisonReportSheets(workbook, report);
