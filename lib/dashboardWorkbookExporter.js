...
function addLast4Sheet(workbook, report = {}) {
  ...
  for (const month of monthBlocks) {
    ...
    if (headerColumnIndex > groupStart) {
      ...
      worksheet.getCell(1, groupStart).value = month.label || month.key;
      monthGroupRanges.push({ start: groupStart, end: headerColumnIndex - 1 });
    }
  }
  styleHeader(worksheet.getRow(1));
  styleHeader(worksheet.getRow(2));
  ...
  for (const range of monthGroupRanges) {
    for (let columnIndex = range.start; columnIndex <= range.end; columnIndex += 1) {
      ...
      monthTitleCell.font = { ...(monthTitleCell.font || {}), bold: true, color: { argb: "FFFFFFFF" } };
      ...
      metricTitleCell.font = { ...(metricTitleCell.font || {}), bold: true, color: { argb: "FF1E3A8A" } };
    }
  }
  ...
  for (let rowIndex = 3; rowIndex <= worksheet.rowCount; rowIndex += 1) {
    const positionCell = worksheet.getCell(rowIndex, 1);
    if (String(positionCell.value || "").trim() === "Team Leader") {
  ...
  const metricRows = [
    ...
    },
    {
      ...
      valueForMonth: (values) => {
        ...
        return leads > 0 ? leads / values.monthDays : 0;
      },
      numberFormat: "0",
    },
    {
