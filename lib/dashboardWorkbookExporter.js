...
function addBuilderTableSheet(workbook, report = {}, query = {}) {
  ...
  if (!isColumnPivot) {
    ...
    for (const row of report.table || []) {
      ...
      if (row?.__transferAgent) {
        transferRowNumbers.push(addedRow.number);
      }
    }
    ...
    // Mark mid-month transfer agents (neon green, legible dark bold text). Done
    // after metric coloring so the whole row reads as a transfer row.
    for (const rowNumber of transferRowNumbers) {
      worksheet.getRow(rowNumber).eachCell((cell) => {
        cell.fill = { type: "pattern", pattern: "solid", fgColor: { argb: "FFCCFF00" } };
        cell.font = { ...(cell.font || {}), color: { argb: "FF14310A" }, bold: true };
      });
    }
