...
import { loadLeadRows, scopeFromFilters } from "./dataProvider.js";
...
function inferDynamicFilters(text, rows, tabConfig) {
  ...
  const pendingFields = filterFields.filter(([filterKey]) => !filters[filterKey]);
  const valuesByField = uniqueValuesForFields(
    rows,
    tabConfig,
    pendingFields.map(([, fieldKey]) => fieldKey),
    500,
  );

  for (const [filterKey, fieldKey] of pendingFields) {
    const match = (valuesByField[fieldKey] || []).find((value) =>
...
export async function answerQuery(text, options = {}) {
  ...
  const scope = scopeFromFilters(parsed.filters || {}, now);
  const rows = await readRows(parsed.tabKey, { tabConfig, scope });
  const filters = {
