...
test("startMenu asks office first for admin users", async () => {
  ...
  assert.equal(labels.some((label) => /office/i.test(label)), true);
});
...
test("country options are sorted by lead count", async () => {
  await selectMonthAndTotalDate(127);
  const countrySelect = await handleMenuCallback(127, "report:country", {
    tabConfig,
    infoAgentsTabConfig,
    readRows,
    now: NOW,
  });
  const countryLabels = countrySelect.replyMarkup.inline_keyboard
    .flat()
    .filter((button) => button.callback_data?.startsWith("drill:multiToggle:"))
    .map((button) => String(button.text).replace(/^✅\s*|^⬜\s*/u, "").trim());
...
test("office drilldown shows summary and child hierarchy", async () => {
  ...
  const teamLeaderSelect = await handleMenuCallback(100, officePick.callback_data, {
    tabConfig,
    infoAgentsTabConfig,
    readRows,
    now: NOW,
...
test("team leader detail supports countries to sub-campaign drilldown", async () => {
  ...
  const officePick = officeSelect.replyMarkup.inline_keyboard
    ...
    .find((button) => button.callback_data?.startsWith("drill:pick:"));
  assert.ok(officePick);
  const teamLeaderSelect = await handleMenuCallback(119, officePick.callback_data, {
    tabConfig,
    infoAgentsTabConfig,
    readRows,
    now: NOW,
  ...
  const agentToggle = agentSelect.replyMarkup.inline_keyboard
    ...
    .find(
      (button) =>
        button.callback_data?.startsWith("drill:multiToggle:") &&
        /Ahmet|Max|Mia/i.test(String(button.text || "")),
    );
...
test("working agents without leads still appear in team leader drilldown", async () => {
  ...
  const agentSelect = await handleMenuCallback(103, "drill:next:agentNames", {
    tabConfig,
    infoAgentsTabConfig,
    readRows,
    now: NOW,
  });
  assert.equal(typeof agentSelect.text, "string");
  assert.equal(agentSelect.text.length > 0, true);
});
...
test("drill pagination edits same message instead of sending new", async () => {
  ...
  const nextPage = officeRoot.replyMarkup.inline_keyboard
    .flat()
    .find((button) => button.callback_data === "drill:page:1");
  assert.ok(nextPage);
...
test("office is single-select and team leader supports multi-select for export filters", async () => {
  ...
  assert.equal(officePickButtons.length >= 1, true);
  const teamLeaderList = await handleMenuCallback(121, officePickButtons[0].callback_data, {
    tabConfig,
    infoAgentsTabConfig,
    readRows,
    now: NOW,
  ...
  const leaderToggle = teamLeaderList.replyMarkup.inline_keyboard
    ...
    .find((button) => button.callback_data?.startsWith("drill:multiToggle:"));
  assert.ok(leaderToggle);
  const selectedLeader = String(leaderToggle.text || "")
    .replace(/^[^\p{L}\p{N}]+/u, "")
    .trim();
  ...
  assert.equal(Boolean(selectedLeader), true);
  assert.match(filteredTeamLeader.text, new RegExp(selectedLeader.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "i"));
  assert.equal(
...
test("campaign export includes selected campaign values in columns", async () => {
  ...
  const campaignSelect = await handleMenuCallback(120, "report:campaign", {
    tabConfig,
    infoAgentsTabConfig,
    readRows,
    now: NOW,
  ...
  const campaignList = await handleMenuCallback(120, "drill:multiDone", {
    tabConfig,
    infoAgentsTabConfig,
    readRows,
    now: NOW,
  });
  assert.match(campaignList.text, /Campaign/);
  ...
  const filterLine = matrix
    .slice(0, 6)
    .map((row) => String((row || [])[0] || ""))
    .find((line) => /Campaign:\s*/i.test(line)) || "";
  assert.equal(/Campaign:\s*/i.test(filterLine), true);
});

test("drill by agent export keeps team leader assignment column", async () => {
  ...
  const officePick = officeSelect.replyMarkup.inline_keyboard
    ...
    .find((button) => button.callback_data?.startsWith("drill:pick:"));
  assert.ok(officePick);
  const teamLeaderSelect = await handleMenuCallback(123, officePick.callback_data, {
    tabConfig,
    infoAgentsTabConfig,
    readRows,
    now: NOW,
  });
  let teamLeaderList = teamLeaderSelect;
  if (/Multi-select Team Leader/i.test(teamLeaderSelect.text)) {
    ...
    teamLeaderList = await handleMenuCallback(123, "drill:multiDone", {
      tabConfig,
      infoAgentsTabConfig,
      readRows,
      now: NOW,
  ...
  const agentDrillButton = teamLeaderList.replyMarkup.inline_keyboard
    .flat()
    .find(
      (button) =>
        button.callback_data === "drill:listNext:agentNames" || button.callback_data === "drill:next:agentNames",
    );
  assert.equal(
    Boolean(agentDrillButton),
    true,
  );
  await handleMenuCallback(123, agentDrillButton.callback_data, {
    tabConfig,
    infoAgentsTabConfig,
    readRows,
    now: NOW,
  ...
  assert.equal(uniqueTeamLeaders.length >= 1, true);
});
